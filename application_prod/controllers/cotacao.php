...
class Cotacao extends CI_Controller {
	...
	function gerenciar(){
		...
		$this->load->model('usuarios_model');		 
         ...
         $config['base_url'] = base_url().'index.php/cotacao/gerenciar/';
		...
		$primeiro22 = true;
		 ...
		 if (!empty($idOs) || !empty($idPedidoCotacao)   || !empty($idStatuscompras) || !empty($query_usuario) ) {
         ...
         $this->data['results'] = $this->cotacao_model->getWhereLikeos($idOs, $idPedidoCotacao,$idStatuscompras,$query_usuario);
            ...
            $config['total_rows'] = count($this->cotacao_model->getdistribuidorcount('distribuir_os','*','statuscompras.idStatuscompras in(1,7) and distribuir_os.solicitacao = 1'));
             $config['per_page'] = 100;
             $config['next_link'] = 'Próxima';
             $config['prev_link'] = 'Anterior';
             $config['full_tag_open'] = '<div class="pagination alternate"><ul>';
         ...
         $this->data['results'] = $this->cotacao_model->getdistribuidor('distribuir_os','*','statuscompras.idStatuscompras in(1,7) and distribuir_os.solicitacao = 1',$config['per_page'],$this->uri->segment(3));
        ...
        */
	    ...
	    $this->data['view'] = 'cotacao/cotacao';
    ...
    //$idCotacaoItens= $this->input->post('idCotacaoItens');
        ...
        if ($this->input->post('id_item_pc1') == '') {
			...
			$this->session->set_flashdata('error','Erro ao editar!');
                redirect(base_url() . 'index.php/cotacao/cotacao');
...
$data3 = array(
	...
	function visualizarimprimir(){
		...
		{
			 redirect(base_url() . 'index.php/cotacao/gerenciar');
		}
        
        $this->data['results'] = $this->cotacao_model->getWhereLikeos('', $idPedidoCotacao,'','');
 ...
 $data3 = array(
			...
			//verifica se existe o numero
			 if(count($this->cotacao_model->getqtditens($idPedidoCotacao_n)) > 0)
			{	
	...
	if(!empty($this->uri->segment(3)))
			 ...
			 {
				redirect(base_url() . 'index.php/cotacao/gerenciar/'.$idPedidoCotacao_n);
			 }
			...
			{
				$this->session->set_flashdata('error','Número de Cotação não encontrada!');
	if(!empty($this->uri->segment(3)))
			 {
 ...
 $data3 = array(
	...
	$this->session->set_flashdata('success','Status editado com sucesso!');
				...
				redirect(base_url() . 'index.php/cotacao/gerenciar/'.$idPedidoCotacao);
	...
	function excluir_itemcotacao(){
	 ...
	 $this->cotacao_model->edit('distribuir_os', $data3, 'idDistribuir', $idDistribuir);
        ...
        redirect(base_url() . 'index.php/cotacao/gerenciar/'.$idPedidoCotacao);
			...
			{
				  redirect(base_url() . 'index.php/cotacao');
			}
			}
		}
		else
