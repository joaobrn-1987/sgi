    ...
    public function verificarLogin(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($ajax == true){
                ...
                $this->session->set_flashdata('error','Os dados de acesso estão incorretoss.');
                //redirect($this->login);
				redirect('mapos/login');
            }
        } 
            ...
            if(count($usuario) > 0){
                ...
                if($ajax == true){
				 ...
				 //echo $url = $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']."-";
				  ...
				  //nome computador
				  //$computador = getenv('COMPUTERNAME');
				  //echo getenv('REMOTE_ADDR');
				  //exec('wmic COMPUTERSYSTEM Get UserName', $user2);
...
//echo getenv('COMPUTERNAME');
 ...
 $computador = getenv("USERNAME");
 
 //$computador = $user2[1];
 //$computador = $_ENV['USERNAME'];
 //getenv("REMOTE_ADDR");
