...
class Pedidocompraalmox extends CI_Controller {
    ...
    function __construct() {
		...
		$this->load->model('pedidocompraalmox_model','',TRUE);
		$this->data['menuPedidoCompraalmox'] = 'Almoxarifado P.C.';
	}	
	...
	function gerenciar(){
		...
		$this->load->model('pedidocompra_model');		 
         ...
         $config['base_url'] = base_url().'index.php/pedidocompraalmox/gerenciar/';
...
$this->data['dados_statusgrupo'] = $this->pedidocompra_model->getstatus_grupo('');		 
		...
		$primeiro = true;
         ...
         $this->data['results'] = $this->pedidocompraalmox_model->getWhereLikeos($idOs, $idPedidoCotacao,$query_statuscompra,$idPedidoCompra,$fornecedor_id,$nf_fornecedor,$query_idgrupo);
 
             $config['total_rows'] = $this->pedidocompraalmox_model->numrowsWhereLikeos($idOs, $idPedidoCotacao,$query_statuscompra,$idPedidoCompra,$fornecedor_id,$nf_fornecedor,$query_idgrupo);
            ...
            //$config['total_rows'] = $this->pedidocompraalmox_model->count('pedido_cotacaoitens');
            $config['total_rows'] = count( $this->pedidocompraalmox_model->getdistribuidorcount('pedido_cotacaoitens','*','statuscompras.idStatuscompras  in(2,7)  and distribuir_os.solicitacao = 2 '));
         ...
         $this->data['results'] = $this->pedidocompraalmox_model->getdistribuidor('pedido_cotacaoitens','*','statuscompras.idStatuscompras  in(2,7) and distribuir_os.solicitacao = 2',$config['per_page'],$this->uri->segment(3));
        ...
        */
	    ...
	    $this->data['view'] = 'pedidocompraalmox/pedidocompraalmox';
	 ...
	 public function imprimir_pedido(){
        ...
        $this->load->model('orcamentos_model');
         ...
         $data['result'] = $this->pedidocompraalmox_model->pedidoCustom($this->uri->segment(3));
       

	
				$this->load->helper('mpdf');
           echo $html = $this->load->view('pedidocompraalmox/imprimir/imprimir_pedido',$data,true);
...
$idStatuscompras2 = $this->input->post('idStatuscompras2');
	...
	$this->data['dist'] = $this->pedidocompraalmox_model->gettable('pedido_cotacaoitens','idPedidoCompraItens ='. $this->input->post('idPedidoCompraItensipi')[$x]);
		 ...
		 $dist = $this->data['dist']->idDistribuir;
			...
			$this->pedidocompra_model->edit('distribuir_os', $data4, 'idDistribuir', $dist);
                ...
                redirect(base_url() . 'index.php/pedidocompraalmox/editarpedido/'.$idPedidoCompraipi);
	}
   ...
   $contador=count($this->input->post('idCotacaoItens_'));
        ...
        if ($this->input->post('idCotacaoItens_')[0] < 1) {
			...
			$this->session->set_flashdata('error','Selecione pelo menos um item!');
                redirect(base_url() . 'index.php/pedidocompraalmox');
...
$data = array(
  ...
  if (is_numeric($id = $this->pedidocompraalmox_model->add('pedido_compras', $data, true)) ) {
...
$this->data['dadoscot'] = $this->pedidocompraalmox_model->gettable('pedido_cotacaoitens','idCotacaoItens ='. $this->input->post('idCotacaoItens_')[$x]);
        
		 $distri = $this->data['dadoscot']->idDistribuir;

		 
			//verifica se esta reservado estoque
				$this->data['status_atual'] = $this->pedidocompraalmox_model->gettable('distribuir_os','idDistribuir ='. $distri);
		  ...
		  else
		 {
			...
			$data3 = array(
				...
				$id_pci = $this->pedidocompraalmox_model->add('pedido_comprasitens', $data2, true);
		...
		$this->pedidocompraalmox_model->edit('distribuir_os', $data3, 'idDistribuir', $distri);
					
				$this->pedidocompraalmox_model->edit('pedido_cotacaoitens', $data5, 'idCotacaoItens', $this->input->post('idCotacaoItens_')[$x]);
 ...
 $this->data['dadosped2'] = $this->pedidocompra_model->gettable('pedido_cotacaoitens','idCotacaoItens ='. $this->input->post('idCotacaoItens')[$x]);
        ...
        $idDistribuir =  $this->data['dadosped2']->idDistribuir;
			...
			$this->pedidocompra_model->edit('distribuir_os', $data3, 'idDistribuir', $idDistribuir);	
               ...
               redirect(base_url() . 'index.php/pedidocompraalmox/editarpedido/'.$this->input->post('idPedidoCompra'));
...
$data3 = array(
	...
	function editarpedido(){
		...
		$this->load->model('cotacao_model');		 
         ...
         $config['base_url'] = base_url().'index.php/pedidocompraalmox/editarpedido/';
...
$this->data['dados_statuscondicao'] = $this->pedidocompra_model->getstatus_cond_pg('');
        ...
        */
	    ...
	    $this->data['view'] = 'pedidocompraalmox/editarpedido';
       	$this->load->view('tema/topo',$this->data);
       //$this->load->view('tema/rodape');
...
else
{
 ...
 $data3 = array(
	...
	$this->session->set_flashdata('success','Pedido alterado com sucesso!');
				...
				redirect(base_url() . 'index.php/pedidocompraalmox/editarpedido/'.$idPedidoCompra);
  ...
  $this->load->model('pedidocompra_model');
        ...
        if(count($this->pedidocompra_model->getqtditens($idPedidoCompra_n)) == 0 || $this->input->post('idPedidoCompra_n') == '') {
			...
			$this->session->set_flashdata('error','Informe o número válido do pedido!');
                redirect(base_url() . 'index.php/pedidocompraalmox/pedidocompraalmox');
	...
	function excluir_itempedido(){
        ...
        if ($idCotacaoItens == null){
            ...
            $this->session->set_flashdata('error','Erro ao tentar excluir item.'); 
			  redirect(base_url() . 'index.php/pedidocompraalmox/gerenciar/'.$idPedidoCompra);
	...
	public function cad_desenho() {
	 ...
	 function do_upload(){
        ...
        $this->upload_config = array(
            ...
            'allowed_types' => 'png|jpg|jpeg|bmp|dwg|pdf|docx',
            'max_size'      => 10240,
            'detect_mime'   => TRUE,
            'remove_space'  => TRUE,
            'encrypt_name'  => TRUE,
        ...
        if (!$this->upload->do_upload()) {
            ...
            log_message('error', 'Upload error (desenho): ' . strip_tags($upload_error));
