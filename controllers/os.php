...
class Os extends CI_Controller
{
    ...
    function gerenciar()
    {
        ...
        $this->data['status_peritagem'] = $this->os_model->getStatusPeritagem();
        ...
        if (!empty($idStatusOs)) {
            $ids_int = implode(',', array_filter(array_map('intval', (array)$idStatusOs)));
            if ($ids_int !== '') $query_status_producao = "($ids_int)";
        }
        $query_unid_execucao = "";
        if (!empty($unid_execucao)) {
            $ids_int = implode(',', array_filter(array_map('intval', (array)$unid_execucao)));
            if ($ids_int !== '') $query_unid_execucao = "($ids_int)";
        }
        $query_unid_faturamento = "";
        if (!empty($unid_faturamento)) {
            $ids_int = implode(',', array_filter(array_map('intval', (array)$unid_faturamento)));
            if ($ids_int !== '') $query_unid_faturamento = "($ids_int)";
        }
        $query_tipoos = "";
        if (!empty($id_tipo)) {
            $ids_int = implode(',', array_filter(array_map('intval', (array)$id_tipo)));
            if ($ids_int !== '') $query_tipoos = "($ids_int)";
        }
        $query_desenho = "";
        if (!empty($desenho)) {
            $desenho_ints = implode(',', array_filter(array_map('intval', (array)$desenho)));
            if ($desenho_ints !== '') $query_desenho = " and os.statusDesenho IN($desenho_ints)";
        }
        $query_verifControl = "";
        if (!empty($verificacaoControle)) {
            $vc_ints = implode(',', array_filter(array_map('intval', (array)$verificacaoControle)));
            if ($vc_ints !== '') $query_verifControl = " and os.idVerificacaoControle IN($vc_ints)";
        }
        $status_peritagem = "";
        if(!empty($this->input->post("selectStatusPeritagem"))){
            $status_peritagem = $this->input->post("selectStatusPeritagem");
        }
        ...
        if (!empty($idOs) || !empty($cod_orc) || !empty($clientes_id) || !empty($idProdutos) || !empty($numpedido_os) || !empty($tag) || !empty($numero_nf) || !empty($descricao_item) || !empty($unidade_execucao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($query_tipoos) || !empty($query_status_producao) || !empty($query_desenho || !empty($query_verifControl)) || !empty($numero_nffab) || !empty($numero_nfserv) || !empty($status_peritagem)) {




            $this->data['results'] = $this->os_model->getWhereLikeos($idOs, $cod_orc, $clientes_id, $idProdutos, $numpedido_os, $tag, $numero_nf, $descricao_item, 'os', "", $query_status_producao, $query_unid_execucao, $query_unid_faturamento, "", "", "", "", "", $query_tipoos, $query_desenho,'',$query_verifControl, $numero_nffab, $numero_nfserv, null, $status_peritagem);
    ...
    function carteiraservico()
    {
        ...
        if(!empty($idOs) || !empty($cod_orc) || !empty($idProdutos) || !empty($query_status_producao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($querydataentrega) || !empty($querydatacadastro) || !empty($querydatareagendada) || !empty($query_clientes) || !empty($queryentrega_reagendada) || !empty($descricao_item) || !empty($query_tipoos) || !empty($unidade_execucao || !empty($query_clientes3) || !empty($query_desenho)  || !empty($query_verifControl) || !empty($query_vendedor) || !empty($status_peritagem))){
            //APROVADOS
            $this->data['data_entrega']= (!empty($data_entrega)?$data_entrega:null);
            $result1 = $this->os_model->getOsAberta($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_peritagem);
            //PENDENTES
            $result2 = $this->os_model->getOsPendente($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro2, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_peritagem);
        ...
        if (!empty($idOs) || !empty($cod_orc) ||  !empty($idProdutos) || !empty($query_status_producao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($querydataentrega) || !empty($querydatacadastro) || !empty($querydatareagendada) || !empty($query_clientes) || !empty($queryentrega_reagendada) || !empty($descricao_item) || !empty($query_tipoos) || !empty($unidade_execucao || !empty($query_clientes3) || !empty($query_desenho)  || !empty($query_verifControl) || !empty($query_vendedor) || !empty($status_peritagem))) {
            ...
            $this->data['results'] = $this->os_model->getWhereLikeos($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_peritagem);
    ...
    function alterarStatusDesenho()
    {
        ...
        if($objOrcamentoItem->tipoOrc == "serv"){
            $data3 = array();
            $data4 = array("idStatusPeritagem"=>2);
    ...
    function do_upload()
    {
        ...
        $this->upload_config = array(
            ...
            'allowed_types' => 'png|jpg|jpeg|bmp|dwg|pdf|docx|xls|xlsx|txt',
            'max_size'      => 10240,
            'detect_mime'   => TRUE,
            'remove_space'  => TRUE,
            'encrypt_name'  => TRUE,
    ...
    public function anexar()
    {
        ...
        $upload_conf = array(
            ...
            'allowed_types' => 'png|jpg|jpeg|bmp|dwg|pdf|docx|xls|xlsx|txt', // formatos permitidos para anexos de os
            'max_size'      => 10240,
            'detect_mime'   => TRUE,
        );
    ...
    function alterarconjuntoos(){
        ...
        foreach ($id_osreag as $os_reag) {
            ...
            if($orcamentoItem->status_item == 1){
                $this->session->set_flashdata('error', 'Não é possivel alterar informações de O.S. que não foram aprovadas');
                redirect(base_url() . 'index.php/os');
            }
   ...
   function send($to,$assunto,$msg) {
    ...
    function ordemservico_pcp(){
        ...
        if (!empty($idOs) || !empty($cod_orc) || !empty($clientes_id) || !empty($idProdutos) || !empty($numpedido_os) || !empty($tag) || !empty($numero_nf) || !empty($descricao_item) || !empty($unidade_execucao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($query_tipoos) || !empty($query_status_producao)  || !empty($query_verifControl) || !empty($numero_nffab) ) {
            ...
            $this->data['results'] = $this->os_model->getos('os', 'os.data_abertura_real,os.data_insert,verificacao_controle.*,os.statusDesenho,os.obsDesenho,os.desconto_os,os.val_unit_os,os.numpedido_os,os.tag,os.val_ipi_os,os.idOs,os.`data_abertura`,os.`subtot_os`,os.`data_entrega`,os.`data_reagendada`,os.`idOrcamentos`,clientes.nomeCliente,produtos.pn,orcamento_item.descricao_item,os.qtd_os,unidade_execucao.status_execucao,unidade_faturamento.status_faturamento,os_tipo.nome_tipo,(SELECT anexo_desenho.idAnexo from anexo_desenho where  anexo_desenho.idOs = os.idOs and (anexo_desenho.user_proprietario =12 OR anexo_desenho.user_proprietario =13) LIMIT 1) as desenhoTrue,status_os.nomeStatusOs ', 'os.statusDesenho = 3', 'os', $config['per_page'], $this->uri->segment(3));
        }
