...
class Desenho extends CI_Controller {
    ...
    function aguardandodesenho(){
        ...
        if(!empty($this->uri->segment(3))){
            ...
            if(!empty($this->input->post('idOrcamento'))){
                $idOrcamento = (int) $this->input->post('idOrcamento');
                if($primeiro){
                    $where = " WHERE orcamento.idOrcamentos = $idOrcamento";
                    $primeiro = false;
                }else{
            ...
            if(!empty($this->input->post('idOrcamentoItem'))){
                $idOrcamentoItem = (int) $this->input->post('idOrcamentoItem');
                if($primeiro){
                    $where = " WHERE orcamento_item.idOrcamento_item = $idOrcamentoItem";
                    $primeiro = false;
                }else{
                    $where .= " and orcamento_item.idOrcamento_item = $idOrcamentoItem";
                }
            }
            if(!empty($this->input->post('idOs'))){
                $idOs = (int) $this->input->post('idOs');
                if($primeiro){
                    $where = " WHERE os.idOs = $idOs";
                    $primeiro = false;
                }else{
            ...
            if(!empty($this->input->post('descricaoOrc'))){
                $descricaoOrc = $this->db->escape_like_str($this->input->post('descricaoOrc'));
                if($primeiro){
                    $where = " WHERE orcamento_item.descricao_item like '%{$descricaoOrc}%'";
                    $primeiro = false;
                }else{
            ...
            if(!empty($this->input->post('statusDesenho'))){
                $statusDesenho = (int) $this->input->post('statusDesenho');
                if($primeiro){
                    $where = " WHERE orcamento_item.statusDesenho = $statusDesenho";
                    $primeiro = false;
                }else{
                    $where .= " and orcamento_item.statusDesenho = $statusDesenho";
                }
            }
            if(!empty($this->input->post('pn'))){
                $pn = $this->db->escape_like_str($this->input->post('pn'));
                if($primeiro){
                    $where = " WHERE produtos.pn like '%{$pn}%'";
                    $primeiro = false;
                }else{
                    ...
                    if(!empty($catalogo)){
                        for($y=0;$y<count($novoEscopoIdProduto);$y++){
                            if(!empty($novoEscopoIdProduto[$y])){
                                ...
                                $catalogoItem = $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($novoEscopoIdProduto[$y],$catalogo->idCatalogoProduto);
            ...
            for($x=0;$x<count($idOs);$x++){
                ...
                for($y=0;$y<count($novoCatalogoIdProduto);$y++){
                    ...
                    if(!empty($idCatalogoItem)){
                        ...
                        $this->peritagem_model->add("os_sub",$dataSubOs);
                    }
                }
    ...
    function cadastrarDesenhoNoItem (){
        ...
        if(!empty($_FILES['file']["name"])){
            ...
            $_FILES['file']["name"] = $this->generateRandomString().".".$imageFileType;
    ...
    function finalizardesenho(){
        ...
        $idOs = $this->input->post('idOs');
        $data = array("idStatusPeritagem"=>2);
