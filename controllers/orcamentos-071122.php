	...
	function gerenciar(){
        ...
        $descricao_item = $this->input->post('descricao_item');
         ...
         if (!empty($cod_orc) || !empty($clientes_id) || !empty($idstatusOrcamento) || !empty($idGrupoServico) || !empty($idNatOperacao) || !empty($referencia) || !empty($num_pedido) || !empty($num_nf) || !empty($status <> '') || !empty($idProdutos <> '') || !empty($descricao_item <> '')) {
            ...
            $this->data['results'] = $this->orcamentos_model->getWhereLikeorc2($cod_orc, $clientes_id, $idstatusOrcamento,$idGrupoServico, $idNatOperacao,$referencia, $num_pedido,$num_nf,$status,$idProdutos,$descricao_item);
	...
	public function aprovar_os(){
        ...
        if($this->input->post('item')[0] > 0)
        {
            ...
            for($x=0;$x<$contador;$x++)
            {
                                ...
                                if(count($listaMateriais)>0){
                                    foreach($listaMateriais as $d){
                                        $data2 = array(
                                            ...
                                            "volume"=>$d->volume,
                                            "peso"=>$d->peso,
                                            "dimensoesL"=>$d->dimensoesL,
                                            "dimensoesC"=>$d->dimensoesC,
                                            "dimensoesA"=>$d->dimensoesA,
                                            "dimensoes"=>$d->dimensoes,
                                            "id_status_grupo"=>$d->id_status_grupo,
                                            "obs"=>$d->obs
                                        );           
                ...
                /*
                if($this->input->post('unid_execucao') == '2' && $this->input->post('idStatusOs') == 5){
                    ...
                    if($response->status == "success"){
                        ...
                        $response3 = json_decode($this->simplexCriarAtividade($response->response->id,$itemOrc->descricao_item,$nova_os,'1652280851846x229444076177981440',$dataAbertura,$data_entrega));
                ...
                }else if($this->input->post('unid_execucao') == '2'){
                    $response = json_decode($this->simplexCriarOS($os->idSimplexCliente,$os->documento,$dataAbertura,$data_entrega,$nova_os));
                    if($response->status == "success"){
                        echo("<script>console.log('sucesso');</script>");
                        $data2 = array("idSimplexOs"=>$response->response->id);
                        ...
                        $response3 = json_decode($this->simplexCriarAtividade($response->response->id,$itemOrc->descricao_item,$nova_os,'1650891880589x672038920761376800',$dataAbertura,$data_entrega));
                        if($response3->status == "success"){
                            $data2 = array("idSimplexAtividade"=>$response3->response->id);
                            $this->orcamentos_model->edit('os', $data2, 'idOs', $nova_os);
                        }
...
else
{
    ...
    public function simplexCriarAtividade($idSimplexOs,$descricao,$idOs,$lista,$inicio,$entrega){
        $curl = curl_init();
