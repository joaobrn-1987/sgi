...
class Os_model extends CI_Model {
	...
	public function OSCustom($idOs){
		...
		$query = "SELECT emitente.url_logo as url_logoemi, emitente.imagem as imagememi,emitente.nome as nomeemi, clientes.idClientes as idcli, clientes.nomeCliente as nomecli , produtos.idProdutos,os.idSimplexAtividade , produtos.descricao , produtos.pn , orcamento_item.detalhe,grupo_servico.nome as grupo,status_os.nomeStatusOs,os.obs_os,os.numpedido_os,os.tag,os.obs_controle,unidade_execucao.status_execucao, unidade_faturamento.status_faturamento, os_tipo.nome_tipo, os.`idOs`, os.`data_abertura`,os.`data_entrega`,os.`idStatusOs`,os.`data_reagendada`,os.`tiposervico`,os.`idOrcamentos`, os.qtd_os,orcamento_item.descricao_item,(SELECT anexo_desenho.idAnexo from anexo_desenho where anexo_desenho.idOs = os.idOs and (anexo_desenho.user_proprietario =12 OR anexo_desenho.user_proprietario =13) LIMIT 1) as desenhoTrue FROM (`os`)
			...
			left join unidade_faturamento on unidade_faturamento.id_unid_fat = os.unid_faturamento left join os_tipo on os_tipo.id_tipo = os.id_tipo left join anexo_desenho on anexo_desenho.idOs = os.idOs where os.idOs = $idOs group by anexo_desenho.idOs";
	...
	public function getdesenho_os2($id){
		$id = (int) $id;
	 	$query = "SELECT * FROM `os`
			...
			WHERE os.idOs = $id and (anexo_desenho.statusAnexo = 2 or anexo_desenho.statusAnexo is null) group by anexo_desenho.imagem";
        return $this->db->query($query)->result();
    }
	...
	public function autoCompleteDistribuir($q){
		...
		$query2 = 'SELECT insumos.*, (select SUM(quantidade) from almo_estoque where almo_estoque.idProduto = insumos.idInsumos and almo_estoque.idEmitente = 1 ) as qtdEst FROM `insumos` where insumos.descricaoInsumo like "%'.$q_escaped.'%"';
	...
	public function getmaterial_dist($id,$ordertipo='',$campo = '',$limit = '',$where = ''){
       	...
       	$this->db->select('insumos.*,distribuir_os.*,distribuir_os.data_cadastro as datacadastro,produtos.*,status_grupo_compra.*,pedido_comprasitens.previsao_entrega,pedido_comprasitens.valor_unitario,pedido_comprasitens.valor_total,pedido_comprasitens.ipi_valor ,usuarios.*,statuscompras.*,pedido_comprasitens.idPedidoCompra');
	...
	function getos($table,$fields,$where='',$tipo = '',$perpage=0,$start=0,$one=false,$array='array')
	{
		...
		$this->db->join('status_peritagem', 'status_peritagem.idStatusPeritagem = orc_servico_escopo.idStatusPeritagem','left');
	...
	function getos2($perpage = 5,$start = 0)
	{
		$query = "
		...
		LEFT JOIN anexo_nfcliente ON anexo_nfcliente.idOs = os.idOs  GROUP BY os.idOs ORDER BY `os`.`idOs` LIMIT $start,$perpage";
	...
	public function getWhereLikeos($idOs = '', $cod_orc = '', $clientes_id = '', $idProdutos = '', $numpedido_os = '', $tag = '',$numero_nf = '',$descricao_item = '',$tipo = '',$unidade_execucao = '',$query_status_producao = '',$query_unid_execucao = '',$query_unid_faturamento = '',$querydataentrega = '',$querydatacadastro = '',$querydatareagendada = '',$query_clientes = '' ,$queryentrega_reagendada = '', $query_tipoos='',$query_desenhoTrue = '',$query_clientes3 = '',$query_verifControl = '',$numero_nffab = '', $numero_nfserv = '', $vendedor = '', $status_peritagem = '')
    {
		...
		if($status_peritagem <> ''){
			$status_per = " and orc_servico_escopo.idStatusPeritagem in (".$status_peritagem.")";
		}else{
			$status_per = "";
		}
		...
		verificacao_controle.idVerificacaoControle,
		os.numpedido_os,
		os.val_ipi_os,os.tag,os.idOs,os.`data_abertura`,os.`data_entrega`,os.`data_reagendada`, os.statusDesenho, os.obsDesenho, os.nf_venda_dev, os.numero_nf, os.nf_venda_dev, os.nf_canhoto,
		...
		LEFT JOIN anexo_nfcliente ON anexo_nfcliente.idOs = os.idOs
			...
			$query_tipoos1 $unidade_execucao1 $query_desenhoTrue $query_clientes3 $query_verifControl $numero_nffab1 $numero_nfserv1 $vendedor $status_per  GROUP BY os.idOs ORDER BY `os`.`idOs` $order";
	...
	function getSubOsByIdOs($id){
		$query = "SELECT os_sub.*, produtos.*, classe_item_escopo.nomeClasse, classe_item_escopo.descricaoClasse FROM os_sub left join produtos on produtos.idProdutos = os_sub.idProduto_sub join classe_item_escopo on classe_item_escopo.idClasse = os_sub.idClasse WHERE idOs = $id";
		return $this->db->query($query)->result();
	}
	function getSubOsByIdOs2($id){
		$query = "SELECT os_sub.*, produtos.*,orc_servico_escopo_itens.dimenExt,orc_servico_escopo_itens.dimenInt,orc_servico_escopo_itens.dimenComp,orc_servico_escopo_itens.obs, classe_item_escopo.nomeClasse, classe_item_escopo.descricaoClasse FROM os_sub left join produtos on produtos.idProdutos = os_sub.idProduto_sub join classe_item_escopo on classe_item_escopo.idClasse = os_sub.idClasse left join orc_servico_escopo_itens on orc_servico_escopo_itens.idOrcServicoEscopoItens = os_sub.idOrcServicoEscopoItens WHERE idOs = $id and os_sub.ativo = 1";
		return $this->db->query($query)->result();
	}
	function getSubOsByIdOrcamentoItem($id){
		$query = "SELECT os_sub.*, produtos.*, orcamento_item.tipoOrc, classe_item_escopo.nomeClasse , classe_item_escopo.descricaoClasse FROM os_sub left join produtos on produtos.idProdutos = os_sub.idProduto_sub join classe_item_escopo on classe_item_escopo.idClasse = os_sub.idClasse join os on os.idOs = os_sub.idOs join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item WHERE os.idOrcamento_item = $id  and os_sub.ativo = 1";
		return $this->db->query($query)->result();
	}
	function getSubOsByIdOrcamentoItemByPosicao($id,$posicao){
		$query = "SELECT os_sub.*, produtos.*, orcamento_item.tipoOrc, classe_item_escopo.nomeClasse , classe_item_escopo.descricaoClasse FROM os_sub left join produtos on produtos.idProdutos = os_sub.idProduto_sub join classe_item_escopo on classe_item_escopo.idClasse = os_sub.idClasse join os on os.idOs = os_sub.idOs join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item WHERE os.idOrcamento_item = $id  and os_sub.ativo = 1 and os_sub.posicao = $posicao";
		return $this->db->query($query)->result();
	}
	function getSubOsByIdOrcamentoItem2($id){
		$query = "SELECT os_sub.*, produtos.*, orcamento_item.tipoOrc, classe_item_escopo.nomeClasse , classe_item_escopo.descricaoClasse FROM os_sub left join produtos on produtos.idProdutos = os_sub.idProduto_sub join classe_item_escopo on classe_item_escopo.idClasse = os_sub.idClasse join os on os.idOs = os_sub.idOs join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item WHERE os.idOrcamento_item = $id  and os_sub.ativo = 1 GROUP BY produtos.idProdutos";
		return $this->db->query($query)->result();
	}
	function getSubOsByIdProduto($idProduto){
        $query = "SELECT * FROM os_sub join os on os.idOs = os_sub.idOs  WHERE idProduto_sub = $idProduto  order by os_sub.idOsSub desc limit 1";
		return $this->db->query($query)->row();
    }
	function getMaterialByIdOsSub($idOsSub){
        $query = "SELECT * FROM distribuir_os where idOsSub = $idOsSub and idStatuscompras = 5";
		return $this->db->query($query)->result();
    }
	function getSugestaoOsSub($idOsSub){
		$query = "SELECT * FROM distribuir_os_sugestao join insumos on insumos.idInsumos = distribuir_os_sugestao.idInsumos join status_grupo_compra on status_grupo_compra.idgrupo = distribuir_os_sugestao.id_status_grupo WHERE idOsSub = $idOsSub and respondeu = 0";
		return $this->db->query($query)->result();
	}
	function getInforDistribuirSugestao($idDistribuirSugestao){
		$query = "SELECT distribuir_os_sugestao.*, insumos.*,status_grupo_compra.*,os_sub.posicao FROM distribuir_os_sugestao join os_sub on os_sub.idOsSub = distribuir_os_sugestao.idOsSub join insumos on insumos.idInsumos = distribuir_os_sugestao.idInsumos join status_grupo_compra on status_grupo_compra.idgrupo = distribuir_os_sugestao.id_status_grupo WHERE idDistribuirSugestao = $idDistribuirSugestao";
		return $this->db->query($query)->row();
	}
	function getStatusF(){
	...
	public function getOrcamentoItemByIdOs($idOs){
		$query = "SELECT orcamento_item.* FROM `orcamento_item` join os on os.idOrcamento_item = orcamento_item.idOrcamento_item WHERE os.idOs = $idOs";

		return $this->db->query($query)->row();
	}
	public function getOsSubSemSub($where = ''){
		$query = "SELECT os.*, orcamento_item.descricao_item, orcamento_item.idProdutos FROM `os` left join os_sub on os_sub.idOs = os.idOs join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item WHERE os_sub.idOsSub is null $where";
		return $this->db->query($query)->result();
	}

	
	public function getStatusPeritagem(){
		$query = "SELECT * FROM status_peritagem order by ordem asc";
		return $this->db->query($query)->result();
	}

	public function getDistribuirByIdOs($idOs){
		$query = "SELECT * FROM distribuir_os WHERE idOs = $idOs ORDER BY data_cadastro asc";
		return $this->db->query($query)->result();
	}
	...
	function getOsPendente($idOs = '', $cod_orc = '', $clientes_id = '', $idProdutos = '', $numpedido_os = '', $tag = '',$numero_nf = '',$descricao_item = '',$tipo = '',$unidade_execucao = '',$query_status_producao = '',$query_unid_execucao = '',$query_unid_faturamento = '',$querydataentrega = '',$querydatacadastro = '',$querydatareagendada = '',$query_clientes = '' ,$queryentrega_reagendada = '', $query_tipoos='',$query_desenhoTrue = '',$query_clientes3 = '',$query_verifControl = '',$numero_nffab = '', $numero_nfserv = '', $vendedor = '', $status_peritagem = ''){
		...
		left join status_peritagem on status_peritagem.idStatusPeritagem = orc_servico_escopo.idStatusPeritagem
		LEFT JOIN anexo_nfcliente ON anexo_nfcliente.idOs = os.idOs
			where 1=1 and orcamento_item.status_item = 0 and orcamento_item.tipoOrc = 'serv' $idOs1 $cod_orc1 $clientes_id1 $idProdutos1 $numpedido_os1 $tag1 $numero_nf1 $query_status_producao1 $query_unid_execucao1 
			$query_unid_faturamento1 $querydataentrega1 $querydatacadastro1 $querydatareagendada1 $query_clientes1 $queryentrega_reagendada1 $descricao_item1
			$query_tipoos1 $unidade_execucao1 $query_desenhoTrue $query_clientes3 $query_verifControl $numero_nffab1 $numero_nfserv1 $vendedor $status_per  GROUP BY os.idOs ORDER BY `os`.`idOs` $order";
