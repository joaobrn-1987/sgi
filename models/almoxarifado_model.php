...
class Almoxarifado_model extends CI_Model {
    ...
    public function getEmpresaUsuario($idUser){
        $query = "SELECT emitente.id,emitente.nome 
    ...
    function getEstoqueByIdEmitenteAndIdInsumo($idEmitente,$idInsumos){
        $query = "SELECT * FROM `almo_estoque` WHERE idProduto = $idInsumos and idEmitente = $idEmitente order by quantidade desc";
        return $this->db->query($query)->result();
    }
    function getItensReservadoOs($where = ""){
        $query = "SELECT almo_estoque.*, insumos.descricaoInsumo, status_os.nomeStatusOs 
            FROM almo_estoque 
            join insumos on almo_estoque.idProduto = insumos.idInsumos 
    ...
    public function getPecaMortaEstoque($id){
        $query = "SELECT * from pecas_mortas_estoque 
            ...
            where pecas_mortas_estoque.idPecasMortasEstoque = $id";
    ...
    public function getValeByOs($idOs){
        $query = "SELECT hist_vale.*, usuarios.nome, insumos.* FROM `hist_vale` join insumos on insumos.idInsumos = hist_vale.idInsumo join usuarios on usuarios.idUsuarios = hist_vale.idUser WHERE idOsOrig = $idOs or idOsDest = $idOs ";
        return $this->db->query($query)->result();   
    }
    public function getValeByOsDestAndIdInsumo($idOs,$idInsumo){
        $query = "SELECT * FROM `hist_vale` join insumos on insumos.idInsumos = hist_vale.idInsumo WHERE idOsDest = $idOs and hist_vale.idInsumo = $idInsumo group by idOsDest order by data_insert asc";
        return $this->db->query($query)->result();   
    }
    public function getOnlyValeByOsOrigem($idOs){
        $query = "SELECT * FROM `hist_vale` WHERE idOsOrig = $idOs";
        return $this->db->query($query)->result();   
    }
    ...
    public function compareEntradaSaida(){
        $query ="SELECT insumos.idInsumos, 
        insumos.descricaoInsumo, emitente.nome,emitente.id, insumos.pn_insumo,almo_estoque_departamento.descricaoDepartamento,almo_estoque_departamento.idAlmoEstoqueDep,
        SUM(almo_estoque.quantidade) as quantidade_total, 
         (if((select sum(almo_estoque_entrada.quantidade)
             from almo_estoque_entrada 
             where almo_estoque_entrada.idProduto = almo_estoque.idProduto)is null,0,(select sum(almo_estoque_entrada.quantidade)
        ...
        join emitente on emitente.id = almo_estoque.idEmitente 
        join almo_estoque_departamento on almo_estoque_departamento.idAlmoEstoqueDep = almo_estoque.idDepartamento 
        group by insumos.idInsumos HAVING quantidade_total != subtracao
        order by insumos.descricaoInsumo asc";
        return $this->db->query($query)->result();
    }
}