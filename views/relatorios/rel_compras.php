                                ...
                                <form class="form-inline" action="<?php echo base_url() ?>index.php/relatorios/relcompras/1" method="post" name="form1" id="form1">
                                    <div class="span12" style="padding: 1%; margin-left: 0">
                                        <div class="span4" class="control-group">
                                            <label for="dataInicial" class="control-label">Data da compra</label></br>
                                    ...
                                    <div class="span12" style="margin-left: 0px; display: flex; flex-wrap: wrap; gap: 10px;">
                                        ...
                                        <div class="span3" style="display:<?php if(isset($unidExec2) && $unidExec2){echo 'block';}else{echo 'none';}?>;" id="divUnidExec">
                                            <label class="control-label">Unidade de Execução:</label>
                                            <div class="card card-body" style="overflow-y: scroll;height: 200px">
                                        ...
                                        <div class="span3" style="display:<?php if(isset($statusExec2) && $statusExec2){echo 'block';}else{echo 'none';}?>;" id="divStatusExec">
                                            <label class="control-label">Status de Execução:</label>
                                            <div class="card card-body" style="overflow-y: scroll;height: 200px">
...
<div class="widget-box">
    ...
    <div class="widget-content nopadding" id="divRelatorio">
        ...
        <table class="table table-bordered" id="table_relatorio" style="border-collapse: collapse;font-family:Arial, Helvetica, sans-serif;font-size:10px;" border="1">
            <thead>
                <tr>
                <?php if(!empty($relatorio)): $arrayKeys = array_keys((array)$relatorio[0]); foreach($arrayKeys as $r){ echo '<th>'.$r.'</th>'; } endif; ?>
                </tr>
            </thead>
            <tbody>
                <?php
                if(!empty($relatorio)){
                    $somaTotal = 0;
                    $ocs_processadas = array(); // Array para controle de duplicação de soma

                    foreach($relatorio as $r){
                        echo '<tr>';
                        
                        // Pega o número da OC (Use o nome exato que vem do Model)
                        $numeroOC = isset($r->{'Nº OC'}) ? $r->{'Nº OC'} : uniqid();

                        foreach($arrayKeys as $d){
                            if($d == 'Total das compras realizadas' || $d == 'ICMS' || $d == 'IPI'){
                                echo '<td>R$ '. number_format((float)$r->$d, 2, ',', '.').'</td>';
                                
                                // Só soma se a OC ainda não foi processada
                                if (!in_array($numeroOC, $ocs_processadas)) {
                                    if($d == 'Total das compras realizadas'){ 
                                        $somaTotal += $r->$d; 
                                    }
                                    // Não somamos IPI e ICMS aqui pois já estão incluídos no Total do Model
                                }
                            } else {
                                echo '<td>'.$r->$d.'</td>';
                            }
                        ...
                        // Marca a OC como processada
                        if (!in_array($numeroOC, $ocs_processadas)) {
                            $ocs_processadas[] = $numeroOC;
                        }
                    }
                } else {
                    echo '<tr><td colspan="1"><h4 style="text-align:center">Nenhum resultado encontrado.</h4></td></tr>';
                }
                ?>
            </tbody>
            ...
            <tfoot>
                <tr>
                    <td colspan="<?php echo count($arrayKeys); ?>" style="text-align:right; font-weight: bold;">TOTAL GERAL (Produtos + Impostos + Frete - Desc): R$ <?php echo number_format((float)$somaTotal, 2, ',', '.'); ?></td>
                </tr>
            </tfoot>
            <?php endif; ?>
        </table>
...
$(document).ready(function() {
    ...
    function PrintElem(elem) {
        ...
        mywindow.document.write('</head><body><h1>Relatório de Compras</h1>');
    ...
    function exportTableToCSV($button, $table, filename) {
        ...
        // ADICIONA O BOM (\uFEFF) PARA FORÇAR O EXCEL A LER EM UTF-8
