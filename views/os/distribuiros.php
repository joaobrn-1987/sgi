																...
																<?php foreach($subOs as $a){
																	...
																	echo '<tr class="trpai'.$a->idOsSub.'">';
																		...
																		//if($result->statusDesenho == 3){
																			if (($this->permission->checkPermission($this->session->userdata('permissao'), 'eDistribuirOs') && $result->fechadoPCP == 1)||$result->idOs == 1||$result->idOs == 2||$result->idOs == 3||$result->idOs == 6) {
																		...
																		if(count($a->distribuiros)>0){
																			echo '<div style="margin: 20px;margin-top: 0px;">';
																				echo '<table class="table table-bordered ">';
																					echo '<thead>';
																					echo '<tr>';
																						echo '<th colspan=15>Materiais Cadastrados</th>';
																					echo '</tr>';
																					echo '<tr>';
																						echo '<th>ID</th>';
																						echo '<th>Qtd.</th>';
...
// --- Revisão PCP (ícone + Ver Obs) ---
echo '<td>';
if ((int)$d->idStatuscompras === 22) {
    $hasObs = isset($d->observacao_revisao_pcp) && trim((string)$d->observacao_revisao_pcp) !== '';
    $mid = 'modalObs' . $d->idDistribuir;

    // ícone de revisão
    echo '<span title="Revisão PCP"><i class="fa fa-check-circle text-success"></i></span> ';

    // botão "Ver Obs" (se houver observação)
    if ($hasObs) {
        $obs = htmlspecialchars((string)$d->observacao_revisao_pcp, ENT_QUOTES, 'UTF-8');
        echo '<a href="#' . $mid . '" data-toggle="modal" class="btn btn-info btn-mini" style="font-size:10px; padding:2px 6px; margin-left:6px;">Ver Obs</a>';

        // modal
        echo '<div id="' . $mid . '" class="modal hide fade" tabindex="-1" role="dialog" aria-hidden="true">'
            . '<div class="modal-header">'
                . '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>'
                . '<h5>Observação da Revisão PCP</h5>'
            . '</div>'
            . '<div class="modal-body"><pre style="white-space:pre-wrap; word-break:break-word; margin:0;">' . $obs . '</pre></div>'
        . '</div>';
    }
}
echo '</td>';
...
// --- Data Ped. Revisão ---
echo '<td>';
if ((int)$d->idStatuscompras === 22 && !empty($d->data_revisao_pcp)) {
    echo date('d/m/Y H:i', strtotime($d->data_revisao_pcp));
}
echo '</td>';
				
				
																								if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eDistribuirOs')) {
																									if ($d->idStatuscompras == 1 || $d->idStatuscompras == 2 || $d->idStatuscompras == 22) {
																									?>
																										<td>
													...
													<div class="modal-body">	
														<div class="control-group">
															...
															<div class="livre<?php echo $dist->idDistribuir; ?>" >
																<div class="control-group">
																	Dimensões:
																	<input class="" class="form-control" id="livre<?php echo $dist->idDistribuir; ?>" type="text" name="livre" value="<?php echo $dist->dimensoes; ?>" />
																</div>
															</div>
															<div class="tamanho<?php echo $dist->idDistribuir; ?>" >
																<div class="control-group">
																	Comprimento(cm):
																	<input class="" class="form-control" id="tamanho<?php echo $dist->idDistribuir; ?>" type="text" name="tamanho" value="<?php echo $dist->comprimento; ?>" />
																</div>
															</div>
															<div class="volume<?php echo $dist->idDistribuir; ?>">
																<div class="control-group">
																	Volume(ml):
																	<input class="" class="form-control" id="volume<?php echo $dist->idDistribuir; ?>" type="text" name="volume" value="<?php echo $dist->volume; ?>" />
																</div>
															</div>
															<div class="peso<?php echo $dist->idDistribuir; ?>">
																<div class="control-group"><br>
																	Peso(g):
																	<input class="" class="form-control" id="peso<?php echo $dist->idDistribuir; ?>" type="text" name="peso" value="<?php echo $dist->peso; ?>" />
																</div>
															</div>
															<div class="dimensoes<?php echo $dist->idDistribuir; ?>">
																<div class="control-group"><br>
																	Dimensões(mm):
																	<div class="">
																		<input class="" class="form-control" id="dimensoesL<?php echo $dist->idDistribuir; ?>" type="text" name="dimensoesL" value="<?php echo $dist->dimensoesL; ?>" placeholder='Largura' />
																		<input class="" class="form-control" id="dimensoesC<?php echo $dist->idDistribuir; ?>" type="text" name="dimensoesC" value="<?php echo $dist->dimensoesC; ?>" placeholder='Comp.' />
																		<input class="" class="form-control" id="dimensoesA<?php echo $dist->idDistribuir; ?>" type="text" name="dimensoesA" value="<?php echo $dist->dimensoesA; ?>" placeholder='Altura' />
																	</div>
																</div>
															</div>
															<!--
...
<div id="modalinserir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">	
	<form action="<?php echo base_url(); ?>index.php/os/cad_distribuir" id="formCadastrar" enctype="multipart/form-data" method="post" class="form-horizontal">
		...
		<div class="modal-body">
			...
			<div class="control-group">
				Quantidade em Estoque:
				<input id="estoque" type="text" name="estoque" value="0" readonly /><!--
				 - Compra PCP: <input type="checkbox" name="checkbox" id="checkbox" value="1"><br> -->
			</div>
			...
			<div class="tamanho0">
				<div class="control-group"><br>
					Comprimento(cm):
					<input class="" class="form-control" id="tamanho0" type="text" name="tamanho" value="" />
				</div>
			</div>
			<div class="volume0">
				<div class="control-group"><br>
					Volume(ml):
					<input class="" class="form-control" id="volume0" type="text" name="volume" value="" />
				</div>
			</div>
			<div class="peso0">
				<div class="control-group"><br>
					Peso(g):
					<input class="" class="form-control" id="peso0" type="text" name="peso" value="" />
				</div>
			</div>
			<div class="dimensoes0">
				<div class="control-group"><br>
					Dimensões(mm):
					<div class="">
						<input class="" class="form-control" id="dimensoesL0" type="text" name="dimensoesL" value="" placeholder='Largura' />
						<input class="" class="form-control" id="dimensoesC0" type="text" name="dimensoesC" value="" placeholder='Comp.' />
						<input class="" class="form-control" id="dimensoesA0" type="text" name="dimensoesA" value="" placeholder='Altura' />
					</div>
