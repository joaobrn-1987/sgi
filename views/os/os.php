...
?>
    ...
    <div class="span3 control-group">
        <label for="dataInicialcad" class="control-label">Data OS Inicial:</label>
        <input type="date" class="span12" id="dataInicialcad" name="dataInicialcad"
               value="<?php
                        // Converte DD/MM/YYYY para YYYY-MM-DD se a data existir
                        echo isset($data_filtro['data_ini']) && !empty($data_filtro['data_ini']) ? DateTime::createFromFormat('d/m/Y', $data_filtro['data_ini'])->format('Y-m-d') : '';
                      ?>"
               placeholder="YYYY-MM-DD">
    </div>
    <div class="span3 control-group">
        <label for="dataFinalcad" class="control-label">Data OS Final:</label>
        <input type="date" class="span12" id="dataFinalcad" name="dataFinalcad"
               value="<?php
                        // Converte DD/MM/YYYY para YYYY-MM-DD se a data existir
                        echo isset($data_filtro['data_final']) && !empty($data_filtro['data_final']) ? DateTime::createFromFormat('d/m/Y', $data_filtro['data_final'])->format('Y-m-d') : '';
                      ?>"
               placeholder="YYYY-MM-DD">
    </div>
    <div class="span2 control-group">
        <label>&nbsp;</label> <button type="submit" name="filter" value="Filtrar" class="btn btn-primary span12"><i class="icon-search icon-white"></i> Filtrar</button>
    </div>


<script type="text/javascript">
    $(document).ready(function() {
        // Remova ou comente a linha que aplica máscara aos inputs de data
        // Exemplo: Se você tinha jQuery(".data").mask("99/99/9999");
        // Comente-a ou remova a classe 'data' dos inputs type="date" acima.
        // Se a máscara for aplicada a outros campos, ajuste o seletor:
        jQuery("input[type=text].data").mask("99/99/9999"); // Exemplo ajustado
    });
</script>											
