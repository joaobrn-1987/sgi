...
class Os extends CI_Controller
{
    ...
    function gerenciar()
    {
        ...
        //$this->data['status_escopo'] = $this->os_model->getStatusPeritagem();
        ...
        $status_escopo = "";
        if(!empty($this->input->post("selectStatusPeritagem"))){
            $status_escopo = $this->input->post("selectStatusPeritagem");
        }
        ...
        if (!empty($idOs) || !empty($cod_orc) || !empty($clientes_id) || !empty($idProdutos) || !empty($numpedido_os) || !empty($tag) || !empty($numero_nf) || !empty($descricao_item) || !empty($unidade_execucao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($query_tipoos) || !empty($query_status_producao) || !empty($query_desenho || !empty($query_verifControl)) || !empty($numero_nffab) || !empty($numero_nfserv) || !empty($status_escopo) ) {
            ...
            $this->data['results'] = $this->os_model->getWhereLikeos($idOs, $cod_orc, $clientes_id, $idProdutos, $numpedido_os, $tag, $numero_nf, $descricao_item, 'os', "", $query_status_producao, $query_unid_execucao, $query_unid_faturamento, "", "", "", "", "", $query_tipoos, $query_desenho,'',$query_verifControl, $numero_nffab, $numero_nfserv, null, $status_escopo, $abertura);
...
function planejadoPCP() {
    ...
    // Verifica se o idOs já existe no banco de dados
			if (!empty($this->os_model->getos($idOs))) {
			
				$data = array(

				"planejadoPCP" => $planejadopcp,
			...
			$this->os_model->add('os', $data);
		
				$this->session->set_flashdata('success', 'O.S. Planejado com sucesso.');
			} else {
				$this->session->set_flashdata('error', 'O.S. não encontrada.');
			}
			// Redireciona para a página de distribuição de O.S.
    ...
    function carteiraservico()
    {
        ...
        $unidade_execucao = $this->input->get('unidade_execucao');
        $verificacaoControle = $this->input->get('verificacaoControle');
        $desenho = $this->input->get('desenho');
        ...
        if (!empty($dataInicialcad) && !empty($dataFinalcad)) {
            ...
            $querydatacadastro2 = " and os.data_insert BETWEEN '$dataInicialcad2 00:00:00' AND '$dataFinalcad2 23:59:59'";
        }


        $dataInicialentrega = $this->input->get('dataInicialentrega');
        $dataFinalentrega = $this->input->get('dataFinalentrega');
        ...
        $dataInicialreag = $this->input->get('dataInicialreag');
        $dataFinalreag = $this->input->get('dataFinalreag');
        ...
        $status_escopo = "";
        if(!empty($this->input->get("selectStatusPeritagem"))){
            $status_escopo = $this->input->get("selectStatusPeritagem");
        }
        $idGrupoServico = $this->input->get('idGrupoServico');
        if(!empty($idOs) || !empty($cod_orc) || !empty($idProdutos) || !empty($query_status_producao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($querydataentrega) || !empty($querydatacadastro) || !empty($querydatareagendada) || !empty($query_clientes) || !empty($queryentrega_reagendada) || !empty($descricao_item) || !empty($query_tipoos) || !empty($unidade_execucao || !empty($query_clientes3) || !empty($query_desenho)  || !empty($query_verifControl) || !empty($query_vendedor) || !empty($status_escopo) || !empty($idGrupoServico))){
            //APROVADOS
            $this->data['data_entrega']= (!empty($data_entrega)?$data_entrega:null);
            $result1 = $this->os_model->getOsAberta($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_escopo,$idGrupoServico);
            //PENDENTES
            $result2 = $this->os_model->getOsPendente($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro2, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_escopo,$idGrupoServico);
            $this->data['results'] = array_merge($result1,$result2);
            $result_status1 = $this->os_model->getWhereLikeos_status($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro, $querydatareagendada, $query_clientes, $queryentrega_reagendada, '', $query_tipoos, '', $query_clientes3,$query_verifControl,null,null,$query_vendedor,$status_escopo,$idGrupoServico);
            $result_status2 = $this->os_model->getWhereLikeos_status2($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro2, $querydatareagendada, $query_clientes, $queryentrega_reagendada, '', $query_tipoos, '', $query_clientes3,$query_verifControl,null,null,$query_vendedor,$status_escopo,$idGrupoServico);
            $this->data['result_status'] = array_merge($result_status1,$result_status2);
        }else{
            ...
            //APROVADOS
            $result1 = $this->os_model->getOsAberta($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_escopo);
            //PENDENTES
            $result2 = $this->os_model->getOsPendente($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro2, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_escopo);
            $this->data['results'] = array_merge($result1,$result2);
            $result_status1 = $this->os_model->getWhereLikeos_status($this->uri->segment(3), '', '', '', '', '', '', '', 'c', '', '', '', '', '', '', '', '', 'and status_os.carteirapadrao = 1', '');
            $result_status2 = $this->os_model->getWhereLikeos_status2($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro2, $querydatareagendada, $query_clientes, $queryentrega_reagendada, '', $query_tipoos, '', $query_clientes3);
            $this->data['result_status'] = array_merge($result_status1,$result_status2);
        }
        ...
        /*
        if (!empty($idOs) || !empty($cod_orc) ||  !empty($idProdutos) || !empty($query_status_producao) || !empty($query_unid_execucao) || !empty($query_unid_faturamento) || !empty($querydataentrega) || !empty($querydatacadastro) || !empty($querydatareagendada) || !empty($query_clientes) || !empty($queryentrega_reagendada) || !empty($descricao_item) || !empty($query_tipoos) || !empty($unidade_execucao || !empty($query_clientes3) || !empty($query_desenho)  || !empty($query_verifControl) || !empty($query_vendedor) || !empty($status_escopo))) {


            $this->data['results'] = $this->os_model->getWhereLikeos($idOs, $cod_orc, '', $idProdutos, '', '', '', $descricao_item, 'c', $unidade_execucao, $query_status_producao, $query_unid_execucao, $query_unid_faturamento, $querydataentrega, $querydatacadastro, $querydatareagendada, $query_clientes, $queryentrega_reagendada, $query_tipoos, $query_desenho, $query_clientes3, $query_verifControl,null,null,$query_vendedor,$status_escopo);
    ...
    public function visualizar()
    {
        ...
        $this->data['anexo_desenho'] = $this->os_model->getdesenho_os2($this->uri->segment(3));
        ...
        $this->data['status_os'] = $this->os_model->getstatus_os();
        $this->data['escopo'] = $this->peritagem_model->getOrcEscopoItemByIdOs($this->uri->segment(3));
        ...
        $this->data['tipo_os'] = $this->os_model->get_table('os_tipo');
        $this->data['verificacao_controle'] = $this->os_model->getVerificacaoControle();
        $this->data['saidas_almoxarifado'] = $this->almoxarifado_model->getSaida2(' WHERE (almo_estoque.idOs is null or almo_estoque.idOs = 0 or almo_estoque.idOs = "") and almo_estoque_saida.idOs = '.$this->uri->segment(3));
        /*   
        ...
        foreach($resultAlteracoesStatus as $r){
            ...
            if($conttDEntr == 0){
                $anteriorDEntr = $r->data_entrega;
                array_push($arrayAlteracoesDataEntrega,$r);
                $conttDEntr = 1;
            }else{
                if($anteriorDEntr != $r->data_entrega){
                    array_push($arrayAlteracoesDataEntrega,$r);
                    $anteriorDEntr = $r->data_entrega;
                }else{
                    $arrayAlteracoesDataEntrega[count($arrayAlteracoesDataEntrega)-1] = $r;
                }
            }
        ...
        $this->data['alteracoesDataReagendada'] = $arrayAlteracoesDataReprogramada;
        $this->data['alteracoesDesenho'] = $arrayAlteracoesDesenho;
        $this->data['alteracoesCont'] = $arrayAlteracoesCont;
        $this->data['subOs'] = $this->os_model->getSubOsByIdOs($this->uri->segment(3));
    ...
    function editar_datareagendada(){
        ...
        if(!empty($this->input->post('datareag'))){
			...
			/************************ envio email alter euripedes*/
            $email = array(
                
				'euripedes@ubertec.ind.br'
				
            );
				 
				 //$email = "euripedes@ubertec.ind.br";
				  
                     //echo json_encode($email);
                    //exit;
                    try{
                        //echo '<script>console.log("'.$email.'")</script>';
                        $this->send($email,"OS reagendada para: ".$this->input->post('datareag'),"OS: ".$idOs, "Descrição: ".$desc);
                    }catch(Exception $e){}
			/*************************/
        
        $vari = $this->os_model->edit('os', $data, 'idOs', $idOs);
        if($vari){
            $os = $this->os_model->getByid_table($idOs, 'os', 'idOs', 'descricao_item');
                $this->os_model->insertOSHis($os);
    ...
    public function editar_distribuiros(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($objDistribuir->idStatuscompras == 19){
                $data2 = array_merge($data2,array('idStatuscompras'=>1,'rejeitado'=>0,'idUserRejeitado '=>NULL,'data_rejeitado'=>NULL));
            }
            ...
            //echo $this->os_model->edit('distribuir_os', $data3, 'idDistribuir', $this->input->post('idDistribuir'))." Distribuir: ".$this->input->post('idDistribuir');
            //return;
