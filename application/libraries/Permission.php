...
class Permission{
    ...
    private function loadPermission($id = null){
        ...
        if($id != null){
            ...
            if(count($array) > 0){
                // SEGURANÇA: allowed_classes=false evita PHP Object Injection via dados corrompidos no banco
                $raw = $array[$this->select];
                $decoded = @unserialize($raw, ['allowed_classes' => false]);
                if ($decoded === false && $raw !== serialize(false)) {
                    log_message('error', 'Permission::loadPermission - falha ao deserializar permissao id='.$id);
                    return false;
                }
                $array = $decoded;
