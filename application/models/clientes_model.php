...
class Clientes_model extends CI_Model {
    ...
    {
        $allowed = ['nomeCliente','email','telefone','celular','cpf','cnpj','cidade','estado','documento'];
    ...
    {
        $allowed = ['nomeCliente','email','telefone'];
        if (!in_array($field, $allowed, true)) return [];
        $search_safe = $this->db->escape_like_str($search);
        $query = "
            SELECT * FROM clientes_solicitantes, clientes
            WHERE clientes.idClientes = clientes_solicitantes.idClientes and $field LIKE '%{$search_safe}%'
            ORDER BY $field
        ";
