...
class Mapos_model extends CI_Model {
    ...
    public function alterarSenha($senha, $oldSenha, $id){
        ...
        $this->db->where('idUsuarios', (int)$id);
        $this->db->limit(1);
        ...
        // SEGURANÇA: suporte a bcrypt (novos) e SHA1 (legado) na verificação
        $hash = $usuario->senha;
        $old_ok = false;
        if (strlen($hash) === 40 && ctype_xdigit($hash)) {
            // Hash SHA1 legado
            $old_ok = hash_equals($hash, sha1($oldSenha));
        } else {
            // Hash bcrypt
            $old_ok = password_verify($oldSenha, $hash);
        }
        ...
        // SEGURANÇA: salvar nova senha como bcrypt
    ...
    public function addEmitente($nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf,$telefone,$email, $logo,$caminho,$email_compras){
       ...
       $this->db->set('email_compras', $email_compras);
       $this->db->set('url_logo', $caminho);
       $this->db->set('imagem', $logo);
       return $this->db->insert('emitente');
    }


    public function editEmitente($id, $nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf,$telefone,$email,$email_compras){
    ...
    public function editLogo($id,$caminho, $logo){
    ...
    public function getBackupLast1Hours($datainico, $datafim){
        // SEGURANÇA: parâmetros via bind para evitar SQL Injection
        $query = "SELECT * FROM backup_hist WHERE data_backup BETWEEN ? AND ?";
        return $this->db->query($query, [$datainico, $datafim])->row();
    }

    public function getOSSimplexPendente(){
        $query = "SELECT os.idOs, os.idSimplexAtividade,os.qtd_os, os.data_abertura,os.data_entrega,orcamento_item.descricao_item, clientes.idClientes,clientes.cep,clientes.nomeCliente, clientes.documento,clientes.idSimplexCliente FROM `os` join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item join orcamento on orcamento.idOrcamentos = orcamento_item.idOrcamentos join clientes on clientes.idClientes = orcamento.idClientes join status_os on status_os.idStatusOs = os.idStatusOs WHERE orcamento_item.status_item = 2 and status_os.nomeStatusOs not like '%(F)%' and os.idSimplexAtividade is null";
