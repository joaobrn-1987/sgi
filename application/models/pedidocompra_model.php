...
function getdistribuidor($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array')
	 {
        ...
        /* --- CAMPOS ADICIONADOS PARA O CÁLCULO FUNCIONAR NA LISTA --- */
	...
	function getDistribuirById($id){
		$this->db->where('idDistribuir', (int)$id);
		return $this->db->get('distribuir_os')->row();
	}
...
/*	function getpedidocomprafornece($id,$group = '',$statuscompra = '')
	{
		...
		if(!empty($statuscompra))
		{
			...
			if($pegacampo[0] == 'st')
			{
				$statuscompra1 = " and statuscompras.idStatuscompras = ".(int)$pegacampo[1];
			}
			elseif($pegacampo[0] == 'os')
			{
				$statuscompra1 = " and distribuir_os.idOs = ".(int)$pegacampo[1];

			}
			else
			{
				$statuscompra1  = " and pedido_comprasitens.notafiscal = '".$this->db->escape_str($pegacampo[1])."'";
			}
		}
		else
		{
...
function getpedidocomprafornece($id, $group = '', $statuscompra = '')
    {
        ...
        {
            // CORREÇÃO AQUI: Deixamos vazio para listar TODOS os itens do pedido
            // Como já removemos o agrupamento por data, o pedido não vai quebrar.
            $group1 = ""; 
        }
        ...
        $query = "
            ...
            inner join distribuir_os on distribuir_os.idDistribuir = pedido_cotacaoitens.idDistribuir  
            ...
            where pedido_comprasitens.idPedidoCompra = ? $statuscompra1 $group1
            ORDER BY pedido_compras.idPedidoCompra desc
        ";
    ...
    function verificaos($idDistribuir){
		...
		WHERE
		statuscompras.idStatuscompras not in(0) AND distribuir_os.idDistribuir = ?
	";

   		return $this->db->query($query, [(int)$idDistribuir])->result();
	}
...
public function getWhereLikeos($idOs, $idStatuscompras, $idPedidoCotacao ='',$idPedidoCompra='',$fornecedor_id='',$nf_fornecedor='',$query_idgrupo='',$solicitacao='',$query_usuario='', $numPedido='', $empresaNum1='', $empresaNum2='', $descricao='',$retirarOs='',$dataEntrega='',$autorizacaoPCP='', $unid_exec = '', $responsabilidadePCP = '',$usuario_orc = '')
    {
        // --- FILTROS ---
        $query_usuario1   = ($query_usuario <> '') ? " AND distribuir_os.usuario_cadastro IN(".implode(',', array_map('intval', explode(',', $query_usuario))).")" : '';
        $idOs1            = ($idOs <> '') ? " AND distribuir_os.idOs IN(".implode(',', array_map('intval', explode(',', $idOs))).")" : '';
        $idPedidoCompra1  = ($idPedidoCompra <> '') ? " AND pedido_cotacaoitens.idPedidoCompra IN(".implode(',', array_map('intval', explode(',', $idPedidoCompra))).")" : '';
        $idPedidoCotacao1 = ($idPedidoCotacao <> '') ? " AND pedido_cotacaoitens.idPedidoCotacao IN(".implode(',', array_map('intval', explode(',', $idPedidoCotacao))).")" : '';
        $idStatuscompras1 = ($idStatuscompras <> '') ? " AND distribuir_os.idStatuscompras IN (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $idStatuscompras)))).")" : '';
        $query_idgrupo1   = ($query_idgrupo <> '') ? " AND distribuir_os.id_status_grupo IN (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $query_idgrupo)))).")" : '';
        $fornecedor_id1   = ($fornecedor_id <> '') ? " AND fornecedores.idFornecedores = ".(int)$fornecedor_id : '';
        $nf_fornecedor1   = ($nf_fornecedor <> '') ? " AND pedido_comprasitens.notafiscal IN('".implode("','", array_map([$this->db,'escape_str'], explode(',', $nf_fornecedor)))."')" : '';
        $numPedido1       = ($numPedido <> '') ? " AND pedido_cotacaoitens.idPedidoCotacao = ".(int)$numPedido : '';
        $empresaNum       = ($empresaNum1 <> '' && $empresaNum2 <> '') ? " AND os.unid_faturamento BETWEEN ".(int)$empresaNum1." AND ".(int)$empresaNum2 : '';
        $descricao        = ($descricao !== '') ? " AND insumos.descricaoInsumo LIKE '%" . $this->db->escape_like_str($descricao) . "%'" : '';
        $autorizacaoPCP   = ($autorizacaoPCP <> '') ? " AND distribuir_os.finalizado = '".$this->db->escape_str($autorizacaoPCP)."'" : '';
        $unid_exec1       = (!empty($unid_exec)) ? " AND os.unid_execucao IN (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $unid_exec)))).")" : '';
        $usuario_orc      = ($usuario_orc <> '') ? " AND distribuir_os.idUser_aguardandoOrc IN (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $usuario_orc)))).")" : '';
        // ---------------

        $query = "
            SELECT
                distribuir_os.idDistribuir, distribuir_os.idOs, usuarios.user, distribuir_os.quantidade, insumos.idInsumos,
                pedido_comprasitens.idPedidoCompraItens, pedido_comprasitens.valor_unitario, 
                
                /* CAMPOS FINANCEIROS (CORRIGIDO PARA APARECER EM TODAS AS LISTAS) */
                pedido_comprasitens.outros,
                pedido_comprasitens.desconto,
                pedido_comprasitens.ipi_valor,
                pedido_comprasitens.icms,
                ...
                pedido_compras.idEmitente,
                fornecedores.nomeFornecedor, pedido_comprasitens.notafiscal, pedido_comprasitens.obs as obscompras,
                distribuir_os.data_revisao_pcp, distribuir_os.observacao_revisao_pcp, distribuir_os.data_alteracao,
                distribuir_os.data_cadastro as datacadastrodist, distribuir_os.dimensoes, distribuir_os.histo_alteracao,
                distribuir_os.obs, distribuir_os.metrica, distribuir_os.comprimento, distribuir_os.volume, distribuir_os.peso,
                distribuir_os.cod_fornecedor, distribuir_os.dimensoesL, distribuir_os.dimensoesA, distribuir_os.data_limite,
                distribuir_os.dimensoesC, usuarios3.nome as nomePCP, usuarios4.nome as nomeSUP, usuarios5.nome as nomeDirTec,
                usuarios6.nome as nomeDir, distribuir_os.data_autorizacaoPCP, distribuir_os.data_autorizacaoDir,
                distribuir_os.data_autorizacaoSUP, status_grupo_compra.nomegrupo, status_grupo_compra.idgrupo,
                pedido_cotacaoitens.data_cadastro, pedido_compras.data_cadastro as cadpedgerado, pedido_comprasitens.data_autorizacao,
                statuscompras.nomeStatus, statuscompras.cor, statuscompras.idStatuscompras, almo_permissao_suprimentos.statusPermissao,
                almo_permissao_suprimentos.idAlmoPermSupr, pedido_comprasitens.datastatusentregue, pedido_cotacaoitens.idPedidoCompra,
                pedido_cotacaoitens.idPedidoCompraItens, pedido_cotacaoitens.idCotacaoItens, pedido_cotacaoitens.idPedidoCotacao,
                usuarios2.nome as nomeAgOrc, insumos.descricaoInsumo, unidade_execucao.status_execucao
            FROM
                distribuir_os
                
                /* JOIN SEGURO: Cotação -> Item -> Pedido (Garante Frete) */
                ...
                LEFT JOIN usuarios ON distribuir_os.usuario_cadastro = usuarios.idUsuarios
                ...
                LEFT JOIN almo_permissao_suprimentos ON almo_permissao_suprimentos.idInsumo = distribuir_os.idInsumos
            ...
            WHERE
                statuscompras.idStatuscompras not in(0) {$idOs1} {$idPedidoCotacao1} {$idStatuscompras1} {$query_idgrupo1} {$idPedidoCompra1} {$fornecedor_id1} {$nf_fornecedor1} {$numPedido1} {$empresaNum} {$descricao} {$query_usuario1} {$retirarOs} {$dataEntrega} {$unid_exec1} {$usuario_orc}
            GROUP BY
                distribuir_os.idDistribuir
            ORDER BY
                distribuir_os.data_alteracao desc
        ";
    ...
    public function numrowsWhereLikeos($idOs, $idPedidoCotacao,$idStatuscompras,$idPedidoCompra,$fornecedor_id,$nf_fornecedor,$query_idgrupo,$solicitacao,$query_usuario,$numPedido, $empresaNum1, $empresaNum2)
    {
		if($query_usuario <> '')
		{
			$query_usuario1 = " and  distribuir_os.usuario_cadastro in(".implode(',', array_map('intval', explode(',', $query_usuario))).")";
		...
		if($nf_fornecedor <> '')
		{
			$nf_fornecedor1  = " and  pedido_comprasitens.notafiscal in('".implode("','", array_map([$this->db,'escape_str'], explode(',', $nf_fornecedor)))."')";
		}
		else
		{
			$nf_fornecedor1 = '';
      ...
      if($idOs <> '')
		{
         ...
         $idOs1 = " and distribuir_os.idOs in(".implode(',', array_map('intval', explode(',', $idOs))).")";
		}
		else
		{
			$idOs1 = '';
		...
		if($idPedidoCompra <> '')
		{
			$idPedidoCompra1 = " and  pedido_cotacaoitens.idPedidoCompra in(".implode(',', array_map('intval', explode(',', $idPedidoCompra))).")";
		}
		else
		{
			$idPedidoCompra1 = '';
		...
		if($idPedidoCotacao <> '')
		{
			$idPedidoCotacao1 = " and  pedido_cotacaoitens.idPedidoCotacao in (".implode(',', array_map('intval', explode(',', $idPedidoCotacao))).")";
		}
		else
		{
			$idPedidoCotacao1 = '';
		...
		if($idStatuscompras <> '')
		{
			$idStatuscompras1  = " and  distribuir_os.idStatuscompras in (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $idStatuscompras)))).")";
		}
		else
		{
			$idStatuscompras1 = '';
		...
		if($query_idgrupo <> '')
		{
			$query_idgrupo1  = " and  distribuir_os.id_status_grupo in (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $query_idgrupo)))).")";
		}
		else
		{
			$query_idgrupo1 = '';
		...
		if($fornecedor_id <> '')
		{
			$fornecedor_id1  = " and  fornecedores.idFornecedores = ".(int)$fornecedor_id;
		}
		else
		{
			$fornecedor_id1 = '';
	...
	public function getWhereLikeos_editar($idOs, $idPedidoCotacao,$idStatuscompras,$idPedidoCompra,$query_usuario)
    {
		...
		if($idPedidoCompra <> '')
		{
			$idPedidoCompra1 = " and  pedido_cotacaoitens.idPedidoCompra = ".(int)$idPedidoCompra;
		}
		else
		{
			$idPedidoCompra1 = '';
...
pedido_cotacaoitens.idCotacaoItens,
 ...
 insumos.descricaoInsumo FROM pedido_cotacaoitens inner join distribuir_os on distribuir_os.idDistribuir = pedido_cotacaoitens.idDistribuir inner join status_grupo_compra on status_grupo_compra.idgrupo = distribuir_os.id_status_grupo inner join insumos on insumos.idInsumos = distribuir_os.idInsumos  inner join statuscompras on statuscompras.idStatuscompras = distribuir_os.idStatuscompras inner join pedido_comprasitens on pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens  where 1 = 1 $idOs1 $idPedidoCotacao1 $idStatuscompras1 $idPedidoCompra1 ORDER BY pedido_cotacaoitens.idPedidoCompra desc
    ...
    public function pedidoCustom($id='',$itens='',$statuscompra = ''){
	   ...
	   if(!empty($id))
		 {
			 $tes1 = implode(',', array_map('intval', explode(',', str_replace(".",",",$id))));
	  ...
	  if(!empty($statuscompra))
		 {
			 ...
			 {
				 $tes = implode(',', array_map('intval', explode(',', str_replace(".",",",$pegacampo[1]))));
	   ...
	   if(!empty($itens))
	   {
		   $itens_safe = implode(',', array_map('intval', explode(',', str_replace(" ", "", $itens))));
		   $itens1 = "and pedido_comprasitens.idPedidoCompraItens in(".$itens_safe.")";
	   }
	   else
	   {
		   $itens1 = '';
	...
	public function getWhereLikeos2($parPedidoCotacao = ''){
		...
		if($parPedidoCotacao <> '')
		{
			...
			$parPedidoCotacao1 = " and distribuir_os.idDistribuir in (".implode(',', array_map('intval', explode(',', $parPedidoCotacao))).")";
		}
		else
		{
			$parPedidoCotacao1 = '';
	...
	public function cotacaoCustom($idEmitente = '', $itensimprimir = ''){
    	...
    	$itensimprimir_safe = implode(',', array_map('intval', explode(',', str_replace(" ", "", $itensimprimir))));
    	$imprimir = "distribuir_os.idDistribuir in(".$itensimprimir_safe.")";
...
public function pedidoCustom3($itens = '')
    {
        if (!empty($itens)) {
            $itens = implode(',', array_map('intval', explode(',', str_replace(" ", "", $itens))));
        } else {
            return [];
        }
        ...
        $query = "
        SELECT 
            distribuir_os.idDistribuir, distribuir_os.idOs, distribuir_os.quantidade, 
            distribuir_os.dimensoes, distribuir_os.comprimento, distribuir_os.volume,
            distribuir_os.peso, distribuir_os.cod_fornecedor, distribuir_os.dimensoesL,
            distribuir_os.dimensoesA, distribuir_os.dimensoesC, distribuir_os.histo_alteracao, 
            distribuir_os.obs, statuscompras.nomeStatus, statuscompras.idStatuscompras, 
            
            pedido_cotacaoitens.idPedidoCompra, pedido_cotacaoitens.idPedidoCompraItens, 
            pedido_cotacaoitens.idCotacaoItens, pedido_cotacaoitens.idPedidoCotacao, 
            
            /* CAMPOS DO ITEM (Valores Individuais) */
            pedido_comprasitens.ipi_valor, pedido_comprasitens.valor_unitario, 
            pedido_comprasitens.valor_total, pedido_comprasitens.quantidade as qtdrecebida, 
            pedido_comprasitens.previsao_entrega, pedido_comprasitens.prazo_entrega, 
            pedido_comprasitens.idCondPgto, pedido_comprasitens.cod_pgto, 
            pedido_comprasitens.obs as obscompras, pedido_comprasitens.desconto, 
            pedido_comprasitens.outros, pedido_comprasitens.icms, 
            pedido_comprasitens.porcentagem, pedido_comprasitens.icmsPorcentagem,

            /* CAMPOS DO CABEÇALHO (Frete Global) */
            pedido_compras.idFornecedores, pedido_compras.idEmitente, 
            pedido_compras.data_cadastro, pedido_compras.frete, 
            
            /* DADOS EMITENTE E FORNECEDOR */
            emitente.nome, emitente.id, emitente.url_logo, emitente.imagem, 
            emitente.cnpj, emitente.rua, emitente.numero, emitente.cidade, 
            emitente.email_compras, emitente.ie, emitente.bairro, emitente.uf, emitente.telefone, 
            
            fornecedores.nomeFornecedor, fornecedores.idFornecedores, fornecedores.documento, 
            fornecedores.telefone as telforne, fornecedores.rua as ruaforne, fornecedores.bairro as bforne, 
            
            insumos.descricaoInsumo, status_os.nomeStatusOs, unidade_execucao.status_execucao

        FROM distribuir_os
        
        /* JOINs AJUSTADOS PARA GARANTIR INTEGRIDADE */
        LEFT JOIN pedido_cotacaoitens ON distribuir_os.idDistribuir = pedido_cotacaoitens.idDistribuir
        LEFT JOIN pedido_comprasitens ON pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens  
        LEFT JOIN pedido_compras ON pedido_compras.idPedidoCompra = pedido_comprasitens.idPedidoCompra
        ...
        WHERE statuscompras.idStatuscompras NOT IN (0) AND distribuir_os.idDistribuir IN ($itens)
        ";
	...
	function getByIdOrdemCompra($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			WHERE pedido_compras.idPedidoCompra = ? and distribuir_os.idStatuscompras in (5,4,14) and distribuir_os.entradaMp = 0";
		return $this->db->query($query, [(int)$id])->result();
	}

	function getByIdOrdemCompraAndNotRecebido($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			distribuir_os.idOs,
			pedido_comprasitens.idPedidoCompraItens,
			distribuir_os.idDistribuir,
			...
			join status_os on status_os.idStatusOs = os.idStatusOs
			join unidade_execucao on unidade_execucao.id_unid_exec = os.unid_execucao
			WHERE pedido_compras.idPedidoCompra = ? and distribuir_os.idStatuscompras in (5,4,14) and distribuir_os.recebido = 0";
		return $this->db->query($query, [(int)$id])->result();
	}
	
	function getByIdOrdemCompraAndRecebido($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			distribuir_os.idOs,
			pedido_comprasitens.idPedidoCompraItens,
			distribuir_os.idDistribuir,
			...
			join status_os on status_os.idStatusOs = os.idStatusOs
			join unidade_execucao on unidade_execucao.id_unid_exec = os.unid_execucao
			WHERE pedido_compras.idPedidoCompra = ? and distribuir_os.idStatuscompras in (5,4,14,11) and distribuir_os.recebido = 1 and distribuir_os.entradaMp = 0";
		return $this->db->query($query, [(int)$id])->result();
	}
	function getByIdDistribuir($id){
		$query = "SELECT * FROM `distribuir_os` WHERE distribuir_os.idDistribuir = ?";
		return $this->db->query($query, [(int)$id])->row();
	}
	function getbyIdPedidoComprasItens($id){
		$query = "SELECT * FROM `pedido_comprasitens` WHERE idPedidoCompraItens = ?";
		return $this->db->query($query, [(int)$id])->row();
	}
	function getValorInsumoByIdInsumo($id){
		...
		join insumos on insumos.idInsumos = distribuir_os.idInsumos 
		WHERE distribuir_os.idInsumos = ? and distribuir_os.idStatuscompras = 5 ORDER by distribuir_os.data_cadastro desc limit 1";
		return $this->db->query($query, [(int)$id])->row();
	}
	function getDistribuirByIdOs($idOs){
		...
		JOIN pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
		join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens and pedido_cotacaoitens.idPedidoCompraItens = pedido_comprasitens.idPedidoCompraItens
		where distribuir_os.idStatuscompras = 5 and distribuir_os.idOs = ?";
	 ...
	 function getPedidoCompraItensByIdDistribuir($id){
		$query = "SELECT * FROM pedido_cotacaoitens WHERE idDistribuir = ?";
		return $this->db->query($query, [(int)$id])->result();
	 }
	 function getPedidoCompraItensByIdCotacaoItem($id){
		$query = "SELECT * FROM pedido_comprasitens WHERE idCotacaoItens = ?";
		return $this->db->query($query, [(int)$id])->result();
	 }
	 function getdistribuidorById($id){
		...
		join usuarios on usuarios.idUsuarios = distribuir_os.usuario_cadastro where 1 = 1 and distribuir_os.idDistribuir = ?";
		return $this->db->query($query, [(int)$id])->row();
	 }
	 function getComprasRespPCP($idOs, $idStatuscompras, $idPedidoCotacao ='',$idPedidoCompra='',$fornecedor_id='',$nf_fornecedor='',$query_idgrupo='',$solicitacao='',$query_usuario='', $numPedido='', $empresaNum1='', $empresaNum2='', $descricao='',$retirarOs='',$dataEntrega='',$autorizacaoPCP='', $unid_exec = ''){
		if($query_usuario <> '')
		{
			$query_usuario1 = " and distribuir_os.usuario_cadastro in(".implode(',', array_map('intval', explode(',', $query_usuario))).")";
		}
		...
		if($numPedido <> '')
		{
			$numPedido1  = " and  pedido_cotacaoitens.idPedidoCotacao = ".(int)$numPedido;
		}
		else
		{
			$numPedido1 = '';
		...
		{
			$empresaNum  = " and os.unid_faturamento BETWEEN ".(int)$empresaNum1. " and ". (int)$empresaNum2;
		}
		else
		{
			$empresaNum = '';
		...
		if($descricao <> ''){
			$descricao = " and insumos.descricaoInsumo LIKE '%" . $this->db->escape_like_str($descricao) . "%'";
		}else{
			$descricao = '';
		}/*
		if($dataEntrega <> ''){
		...
		if($autorizacaoPCP <> ''){
			$autorizacaoPCP = " and distribuir_os.finalizado = '".$this->db->escape_str($autorizacaoPCP)."'";
		}else{
			$autorizacaoPCP = '';
		}
		if(!empty($unid_exec))
		{
			$unid_exec1  = " and  os.unid_execucao in (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $unid_exec)))).")";
		}else{
			$unid_exec1 = '';
		}
	 ...
	 public function getPermissaoCompraByIdPermissao($id){
		$query = "SELECT * FROM permissao_compra where idPermissao = ?";
		return $this->db->query($query, [(int)$id])->row();
	}	 
	...
	public function itensautorizacaocompra2($min,$max, $where2){
		if(!empty($max)){
		...
		left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
		left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
		left join usuarios on usuarios.idUsuarios = distribuir_os.idUser_aguardandoOrc
			where ((distribuir_os.idStatuscompras = 10 or distribuir_os.idStatuscompras = 12 or distribuir_os.idStatuscompras = 17  or distribuir_os.idStatuscompras = 13 or distribuir_os.idStatuscompras = 16) or distribuir_os.aprovacaoSUP = 1)
			and (pedido_comprasitens.valor_unitario is not null 
			and pedido_comprasitens.valor_unitario != '' 
			and pedido_comprasitens.valor_unitario != 0) 
...
public function itensautorizacaocompra2_where($min, $max, $where2) { 
    ...
    $query = "
        SELECT 
            SUM(pedido_comprasitens.valor_unitario * pedido_comprasitens.quantidade) AS soma,
            GROUP_CONCAT(pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') AS idPedidoCompraItens,
            GROUP_CONCAT(distribuir_os.idDistribuir SEPARATOR ';') AS idDistribuir,
            GROUP_CONCAT(distribuir_os.idOs SEPARATOR ';') AS idOs,
            GROUP_CONCAT(insumos.descricaoInsumo SEPARATOR ';') AS descricaoInsumo,
        ...
        JOIN pedido_cotacaoitens ON pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir 
        JOIN pedido_comprasitens ON pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens 
            AND pedido_cotacaoitens.idPedidoCompraItens = pedido_comprasitens.idPedidoCompraItens 
        JOIN insumos ON insumos.idInsumos = distribuir_os.idInsumos
        JOIN pedido_compras ON pedido_compras.idPedidoCompra = pedido_comprasitens.idPedidoCompra 
        JOIN os ON os.idOs = distribuir_os.idOs
        LEFT JOIN fornecedores ON fornecedores.idFornecedores = pedido_compras.idFornecedores
        ...
        LEFT JOIN usuarios ON usuarios.idUsuarios = distribuir_os.idUser_aguardandoOrc
        LEFT JOIN unidade_execucao ON unidade_execucao.id_unid_exec = os.unid_execucao
        LEFT JOIN emitente ON emitente.id = pedido_compras.idEmitente
        WHERE (
            distribuir_os.idStatuscompras IN (10, 12, 13, 16, 17)
            OR distribuir_os.aprovacaoSUP = 1
        )
        AND (
            pedido_comprasitens.valor_unitario IS NOT NULL 
            AND pedido_comprasitens.valor_unitario != '' 
            AND pedido_comprasitens.valor_unitario != 0
        ) 
        $where2
        GROUP BY pedido_comprasitens.idPedidoCompra 
        HAVING aprovacaoDir NOT LIKE '%0%' 
            AND aprovacaoPCP NOT LIKE '%0%' 
            AND aprovacaoSUP2 LIKE '%0%' 
        $where
    ";
	...
	public function itensautorizacaocompra4($min,$max,$where2 = ''){
		...
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			...
			GROUP_CONCAT(distribuir_os.idStatuscompras SEPARATOR ';') as status2,
			status_cond_pgt.nome_status_cond_pgt as nomePgto,
			pedido_comprasitens.cod_pgto as condPgto,
			pedido_comprasitens.idPedidoCompra,
			fornecedores.nomeFornecedor,
			distribuir_os.data_rejeitado,
			distribuir_os.data_cadastro, 
			distribuir_os.aprovacaoSUP,
			distribuir_os.aprovacaoDir,
			pedido_comprasitens.data_autorizacao,
			pedido_comprasitens.autorizadoCompra,
			usuarios.nome as nomeRejeitado			
			from distribuir_os 
			join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir 
			join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens and pedido_cotacaoitens.idPedidoCompraItens = pedido_comprasitens.idPedidoCompraItens 
			join insumos on insumos.idInsumos = distribuir_os.idInsumos
			join pedido_compras on pedido_compras.idPedidoCompra = pedido_comprasitens.idPedidoCompra
			join os on os.idOs = distribuir_os.idOs
			LEFT JOIN usuarios ON usuarios.idUsuarios = distribuir_os.idUserRejeitado
			left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
			left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
			where ((distribuir_os.idStatuscompras = 10 or distribuir_os.idStatuscompras = 12 or distribuir_os.idStatuscompras = 17  or distribuir_os.idStatuscompras = 13 or distribuir_os.idStatuscompras = 16) or distribuir_os.aprovacaoDir = 1)
			and os.unid_execucao in (1,2,4) $where2
			GROUP by pedido_comprasitens.idPedidoCompra having aprovacaoPCP not like \"%0%\" and aprovacaoDir2 like \"%0%\" and status2 not like \"%3%\" $where";
		return $this->db->query($query)->result();
	}
	...
	public function itensreprovadoscompra4_2($where2 = ''){
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			pedido_comprasitens.quantidade) as soma,
			GROUP_CONCAT(pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') as idPedidoCompraItens,
			GROUP_CONCAT(distribuir_os.idDistribuir  SEPARATOR ';') as idDistribuir,
			...
			left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
			left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
			where distribuir_os.idStatuscompras = 15
			and (os.unid_execucao in (3) or distribuir_os.idOs in (2,11)) $where2
			GROUP by pedido_comprasitens.idPedidoCompra";
		return $this->db->query($query)->result();
	}
	public function itensautorizacaocompra4_3($min,$max,$where2 = ''){
		...
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			...
			LEFT JOIN usuarios ON usuarios.idUsuarios = distribuir_os.idUserRejeitado
			where ((distribuir_os.idStatuscompras = 10 or distribuir_os.idStatuscompras = 12 or distribuir_os.idStatuscompras = 17  or distribuir_os.idStatuscompras = 13 or distribuir_os.idStatuscompras = 16) or distribuir_os.aprovacaoDir = 1)
			and (os.unid_execucao in (5,8) or distribuir_os.idOs in (6,13,14)) $where2
			GROUP by pedido_comprasitens.idPedidoCompra having aprovacaoPCP not like \"%0%\" and aprovacaoDir2 like \"%0%\" and status2 not like \"%3%\" $where";
		return $this->db->query($query)->result();
	}

	public function itensreprovadoscompra4_3($where2 = ''){
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			pedido_comprasitens.quantidade) as soma,
			GROUP_CONCAT(pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') as idPedidoCompraItens,
			GROUP_CONCAT(distribuir_os.idDistribuir  SEPARATOR ';') as idDistribuir,
			...
			left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
			left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
			where distribuir_os.idStatuscompras = 15 and (os.unid_execucao in (5) or distribuir_os.idOs in (6,13)) $where2
			GROUP by pedido_comprasitens.idPedidoCompra having aprovacaoPCP not like \"%0%\" and aprovacaoDir2 like \"%0%\" and status2 not like \"%3%\" ";
		return $this->db->query($query)->result();
	}
	...
	public function itensreprovadoscompra4_4($min='',$max='',$where2 = ''){
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			pedido_comprasitens.quantidade) as soma,
			GROUP_CONCAT(pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') as idPedidoCompraItens,
			GROUP_CONCAT(distribuir_os.idDistribuir  SEPARATOR ';') as idDistribuir,
			...
			join pedido_compras on pedido_compras.idPedidoCompra = pedido_comprasitens.idPedidoCompra
			join os on os.idOs  = distribuir_os.idOs
			join usuarios on usuarios.idUsuarios = distribuir_os.idUserRejeitado
			left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
			left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
			where distribuir_os.idStatuscompras = 15 and os.unid_execucao in (6,1)  $where2
			GROUP by pedido_comprasitens.idPedidoCompra";
		return $this->db->query($query)->result();
	}
	public function itensautorizacaocompra4Admin($min,$max,$where2 = ''){
		...
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			...
			left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
			left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
			where ((distribuir_os.idStatuscompras = 10 or distribuir_os.idStatuscompras = 12 or distribuir_os.idStatuscompras = 17  or distribuir_os.idStatuscompras = 13 or distribuir_os.idStatuscompras = 16 or distribuir_os.idStatuscompras = 3) or distribuir_os.aprovacaoDir = 1)
			 $where2
			GROUP by pedido_comprasitens.idPedidoCompra having aprovacaoPCP not like \"%0%\" and aprovacaoDir2 like \"%0%\" and status2 not like \"%3%\" $where";
			//echo $query;
			//exit();
	...
	public function itensreprovado4Admin($where2 = ''){
		$query = "select sum(pedido_comprasitens.valor_unitario* 
			pedido_comprasitens.quantidade) as soma,
			GROUP_CONCAT(pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') as idPedidoCompraItens,
			GROUP_CONCAT(distribuir_os.idDistribuir  SEPARATOR ';') as idDistribuir,
	...
	public function itensautorizacaocompra3($min,$max,$where2 = ''){
		...
		left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
		left join status_cond_pgt on status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
		where (distribuir_os.idStatuscompras = 10 or distribuir_os.idStatuscompras = 12 or distribuir_os.idStatuscompras = 17  or distribuir_os.idStatuscompras = 13 or distribuir_os.idStatuscompras = 14)
		and (pedido_comprasitens.valor_unitario is not null
		and pedido_comprasitens.valor_unitario != ''
	...
	public function getComprasParaAprovarPcp3(){
		$query = "SELECT os.idOs, (select sum(distribuir_os.quantidade*pedido_comprasitens.valor_unitario) FROM distribuir_os join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir join pedido_comprasitens on pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens and pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens join statuscompras on statuscompras.idStatuscompras = distribuir_os.idStatuscompras where os.idOs = distribuir_os.idOs and statuscompras.etapa = 3 and distribuir_os.aprovacaoPCP = 0 and os.unid_execucao in (5,8)) as valorTotal,
		os.qtd_os,			
        status_os.nomeStatusOs,
		orcamento_item.descricao_item		
		FROM os 
		join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item
        join status_os on status_os.idStatusOs = os.idStatusOs
		where  os.idOs not in (10,11,12,13,14) 
		group by os.idOs 
		having valorTotal is not null
		order by os.idOs asc";
		return $this->db->query($query)->result();
	...
	public function getComprasParaAprovarPcp4(){
		$query = "SELECT os.idOs, (select sum(distribuir_os.quantidade*pedido_comprasitens.valor_unitario) FROM distribuir_os join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir join pedido_comprasitens on pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens and pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens join statuscompras on statuscompras.idStatuscompras = distribuir_os.idStatuscompras where os.idOs = distribuir_os.idOs and statuscompras.etapa = 3 and distribuir_os.aprovacaoPCP = 0 and os.unid_execucao in (1,2,4,6,7,9)) as valorTotal,
		os.qtd_os,			
        status_os.nomeStatusOs,
		orcamento_item.descricao_item		
		FROM os 
		join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item
        join status_os on status_os.idStatusOs = os.idStatusOs
		where  os.idOs not in (10,11,12,13) and os.unid_execucao in (1,2,4,6,7,9)
		group by os.idOs 
		having valorTotal is not null
		order by os.idOs asc";
		return $this->db->query($query)->result();
	...
	public function getHistAprovacaoPCP4(){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where  distribuir_os.idOs not in (10,11,12,13,1,2,3,6) and distribuir_os.data_autorizacaoPCP is not null and os.unid_execucao in (1,2,4,6,7,9)
		group by distribuir_os.idOs 
		order by distribuir_os.data_autorizacaoPCP desc limit 50";
		$this->db->simple_query('SET SESSION group_concat_max_len=50000');
		return $this->db->query($query)->result();
	...
	public function getItensByIdos($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and os.unid_execucao in (1,2,4) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdosAdmin($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdos2($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and (os.unid_execucao in (3) or distribuir_os.idOs in (2,11)) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdos3($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and (os.unid_execucao in (5) or distribuir_os.idOs in (2,11)) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdos4($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and os.unid_execucao in (1,2,4,6,7,9) and distribuir_os.aprovacaoPCP = 0 order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdosLimited($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and distribuir_os.aprovacaoPCP = 0 and os.unid_execucao in (1,2,4) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdosLimitedAdmin($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and distribuir_os.aprovacaoPCP = 0 order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdosLimited2($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and distribuir_os.aprovacaoPCP = 0 and (os.unid_execucao in (3) or distribuir_os.idOs in (2,11)) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdosLimited3($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and distribuir_os.aprovacaoPCP = 0 and (os.unid_execucao in (5) or distribuir_os.idOs in (2,11)) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
	public function getItensByIdosLimited4($idOs){
        ...
        left join pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
        left join pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
		where distribuir_os.idOs = ? and distribuir_os.aprovacaoPCP = 0 and os.unid_execucao in (1,2,4,6,7,9) order by distribuir_os.data_enviadopcp desc,distribuir_os.aprovacaoPCP asc";
		return $this->db->query($query, [(int)$idOs])->result();
	}
// SUBSTITUA A FUNÇÃO DA LINHA 1058 POR ESTA:
	function getByIdOrdemCompra2($id, $unid_execucao = null, $filtroStatus = null){ // <-- 1. PARÂMETROS ADICIONADOS
		
        // 2. Início da Lógica de Filtro
        $where = "WHERE pedido_compras.idPedidoCompra = " . (int)$id; // (Melhoria de segurança)
		
        if ($unid_execucao !== null && is_numeric($unid_execucao)) {
			$where .= " AND os.unid_execucao = " . (int)$unid_execucao; // (Melhoria de segurança)
		}

        // 3. ADIÇÃO DO FILTRO DE STATUS (A CORREÇÃO PRINCIPAL)
        if ($filtroStatus !== null && is_numeric($filtroStatus)) { 
            $where .= " AND distribuir_os.idStatuscompras = " . (int)$filtroStatus;
        }
		...
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			$where"; // 4. USA A NOVA VARIÁVEL $where
		return $this->db->query($query)->result();
	}
	...
	function getByIdOrdemCompra3($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
			left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
			WHERE pedido_compras.idPedidoCompra = ? and os.unid_execucao in (1,2,4) ";
		return $this->db->query($query, [(int)$id])->result();
	}
	...
	function getByIdOrdemCompra3_2($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
			left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
			WHERE pedido_compras.idPedidoCompra = ? and (os.unid_execucao in (3) or distribuir_os.idOs in (2,11))";
		return $this->db->query($query, [(int)$id])->result();
	}
	...
	function getByIdOrdemCompra3_3($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
			left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
			WHERE pedido_compras.idPedidoCompra = ? and (os.unid_execucao in (5) or distribuir_os.idOs in (6,13))";
		return $this->db->query($query, [(int)$id])->result();
	}
	...
	function getByIdOrdemCompra3_4($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
			left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
			WHERE pedido_compras.idPedidoCompra = ? and os.unid_execucao in (6,1)";
		return $this->db->query($query, [(int)$id])->result();
	}
	...
	function getByIdOrdemCompra3Admin($id){
		$query = "SELECT pedido_compras.idPedidoCompra,
			...
			join status_os on status_os.idStatusOs = os.idStatusOs
			join unidade_execucao on unidade_execucao.id_unid_exec = os.unid_execucao
			left join usuarios as userORc on userORc.idUsuarios = distribuir_os.idUser_aguardandoOrc
			left join usuarios on usuarios.idUsuarios = distribuir_os.usuario_cadastro
			left join usuarios as user2 on user2.idUsuarios = distribuir_os.idUserAprovacao
			left join usuarios as user3 on user3.idUsuarios = distribuir_os.idUserAprovacaoSUP
			left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
			left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
			WHERE pedido_compras.idPedidoCompra = ?";
		return $this->db->query($query, [(int)$id])->result();
	}
...
public function getRelatorioPorInsumoEmpresa($idInsumos, $idOs){
        ...
        // 1. PROTEÇÃO DE ENTRADA: Se faltar ID, cria um objeto zerado para a View não quebrar
        if(empty($idInsumos) || empty($idOs)) {
            $dummy = new stdClass();
            $dummy->idInsumos = 0;
            $dummy->nome = '-';
            $dummy->descricaoInsumo = '-';
            $dummy->id = 0;
            $dummy->quantidade_total = 0;
            $dummy->quantidade_reservada = 0;
            
            // Retorna um array contendo esse objeto, pois a View espera um result() (lista)
            return array($dummy);
        }
        
        $idInsumos = (int)$idInsumos;
        $idOs = (int)$idOs;
        ...
        emitente.nome,
        emitente.id,
        sum(IF(almo_estoque.idOs is null or almo_estoque.idOs = '',0,almo_estoque.quantidade)) as quantidade_total,
        sum(IF(almo_estoque.idOs = $idOs,0,almo_estoque.quantidade)) as quantidade_reservada
        ...
        where insumos.idInsumos =  $idInsumos and almo_estoque.quantidade > 0
        group by insumos.idInsumos,emitente.id";
        
        $result = $this->db->query($query)->result();
        
        // 2. PROTEÇÃO DE RESULTADO: Se o banco não achar nada, retorna o objeto zerado também
        if (empty($result)) {
            $dummy = new stdClass();
            $dummy->idInsumos = $idInsumos;
            $dummy->nome = '-';
            $dummy->descricaoInsumo = '-';
            $dummy->id = 0;
            $dummy->quantidade_total = 0;
	...
	function getDistribuirByIdComprasItens($id){
		$query = "SELECT distribuir_os.* FROM pedido_comprasitens join pedido_cotacaoitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens join distribuir_os on distribuir_os.idDistribuir = pedido_cotacaoitens.idDistribuir  where pedido_comprasitens.idPedidoCompraItens = ?";
		return $this->db->query($query, [(int)$id])->row();
	 }
	 
	 function getDistribuirDetails($id){
		$query = "SELECT pedido_comprasitens.idPedidoCompra,pedido_comprasitens.idPedidoCompraItens,pedido_comprasitens.idCotacaoItens, pedido_cotacaoitens.idDistribuir FROM pedido_cotacaoitens join pedido_comprasitens on pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens and pedido_cotacaoitens.idCotacaoItens = pedido_comprasitens.idCotacaoItens where pedido_cotacaoitens.idDistribuir = ?";
		return $this->db->query($query, [(int)$id])->row();
	 }
	 ...
	 function getByIdOrdemCompra2Hist($id){
		...
		left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
		left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
		WHERE pedido_comprasitens.autorizadoCompra = 1 AND pedido_compras.idPedidoCompra = ?";
		return $this->db->query($query, [(int)$id])->result();
	 }
	 ...
	 function getByIdOrdemCompra3Hist($id){
		...
		left join usuarios as user4 on user4.idUsuarios = pedido_comprasitens.idUsuarioAutorizacao
		left join usuarios as user5 on user5.idUsuarios = distribuir_os.idUserAprovacaoDir
		WHERE distribuir_os.aprovacaoDir = 1 and pedido_compras.idPedidoCompra = ?
		and os.unid_execucao in (1,2,4)";
		return $this->db->query($query, [(int)$id])->result();
	 }
...
function getHistoricoAprovacaoDiretoria(){
		$query = "select sum(pedido_comprasitens.valor_unitario* pedido_comprasitens.quantidade) as soma,
		WEEK(pedido_comprasitens.data_autorizacao, 1) as semana_aprovacao,
		GROUP_CONCAT(pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') as idPedidoCompraItens,
		GROUP_CONCAT(distribuir_os.idDistribuir  SEPARATOR ';') as idDistribuir,
		GROUP_CONCAT(distribuir_os.idOs SEPARATOR ';') as idOs,
		GROUP_CONCAT(insumos.descricaoInsumo SEPARATOR ';') as descricaoInsumo,
		...
		left join fornecedores on fornecedores.idFornecedores = pedido_compras.idFornecedores
		where 
		pedido_comprasitens.autorizadoCompra = 1 /* <-- CORRIGIDO AQUI */
		and pedido_comprasitens.idUsuarioAutorizacao != 51 /* <-- CORRIGIDO AQUI */
		and distribuir_os.data_cadastro > '2022-03-25'   
		GROUP by pedido_comprasitens.idPedidoCompra 
		order by pedido_comprasitens.data_autorizacao desc limit 20"; /* <-- CORRIGIDO AQUI */
		return $this->db->query($query)->result();
	 }
	 ...
	 function getdistribuidorByIdPedidoCompra($id){
		...
		join orcamento_item on orcamento_item.idOrcamento_item = os.idOrcamento_item 
		join produtos on produtos.idProdutos = orcamento_item.idProdutos
		join usuarios on usuarios.idUsuarios = distribuir_os.usuario_cadastro where 1 = 1 and pedido_compras.idPedidoCompra = ?";
		return $this->db->query($query, [(int)$id])->result();
	 }
	 ...
	 function getUltimosOrc($where){
        // Sanitiza o fragmento $where: substitui valores numéricos após "= " por cast intval/floatval
        $where = preg_replace_callback(
            '/(\b(?:idInsumos|idDistribuir|idOs|idStatuscompras)\s*=\s*)(-?[\d]+)/i',
            function($m){ return $m[1].(int)$m[2]; },
            $where
        );
        $where = preg_replace_callback(
            '/(\b(?:dimensoesL|dimensoesC|dimensoesA|comprimento|volume|peso)\s*=\s*)(-?[\d.]+)/i',
            function($m){ return $m[1].(float)$m[2]; },
            $where
        );
		$query = "SELECT distribuir_os.idDistribuir,
		distribuir_os.idOs,
	 ...
	 function getAnexoCotacaoSuprimentosByIdPedidoCompra($id){
		$query = "SELECT * FROM anexo_cotacao_suprimentos join pedido_compras on pedido_compras.idPedidoCompra = anexo_cotacao_suprimentos.idPedidoCompra where pedido_compras.idPedidoCompra = ?";
		return $this->db->query($query, [(int)$id])->result();
	 }
	 
	function getAllComprasByIdInsumo($idInsumo,$where = ""){
        $query = "SELECT sum(distribuir_os.quantidade) as quantidade, sum(distribuir_os.quantidade*pedido_comprasitens.valor_unitario) as valor_total FROM `distribuir_os` JOIN pedido_cotacaoitens on pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir JOIN pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens and pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens WHERE distribuir_os.idStatuscompras = 5 and distribuir_os.idInsumos = ? $where";
		return $this->db->query($query, [(int)$idInsumo])->row();
    }

	function getAnexoCotacao($idAnexo){
		$query = "SELECT * FROM anexo_cotacao_suprimentos WHERE idAnexo = ?";
		return $this->db->query($query, [(int)$idAnexo])->row();
	}

	function getObservacaoRejeicao($idOc){
		$query = "SELECT distribuir_os.obs_rejeitado FROM distribuir_os JOIN pedido_cotacaoitens ON pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir JOIN pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens and pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens WHERE pedido_comprasitens.idPedidoCompra = ? and distribuir_os.obs_rejeitado is not null LIMIT 1";
		return $this->db->query($query, [(int)$idOc])->row();

	}
	function getItensRejeitadosByIdOC($idOc){
		$query = "SELECT distribuir_os.idDistribuir, pedido_comprasitens.idPedidoCompraItens FROM distribuir_os JOIN pedido_cotacaoitens ON pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir JOIN pedido_comprasitens on pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens and pedido_comprasitens.idPedidoCompraItens = pedido_cotacaoitens.idPedidoCompraItens WHERE pedido_comprasitens.idPedidoCompra = ? and distribuir_os.rejeitado = 1";
		return $this->db->query($query, [(int)$idOc])->result();
	}
// Cole esta função dentro da classe Pedidocompra_model
    /**
     * Busca Ordens de Compra agrupadas que estão aguardando Aprovação Final (Status 23).
     * Similar a itensautorizacaocompra2_where, mas focada no status 23.
     *
     * @param string $min Valor mínimo (opcional, se a aprovação final for baseada em valor)
     * @param string $max Valor máximo (opcional)
     * @param string $where2 String SQL com filtros adicionais (já inclui status 23 e rejeitado != 1)
     * @return array Array de objetos com os dados das OCs.
     */
    public function itensAprovacaoFinal($min = "", $max = "", $where2 = "") {

        // Define o filtro de valor (será usado no HAVING) - Opcional
        $whereValor = "";
        if ($min !== "" && is_numeric($min)) {
            if ($max !== "" && is_numeric($max)) {
                // Usa escape para segurança
                $whereValor = " HAVING SUM(pedido_comprasitens.valor_unitario * pedido_comprasitens.quantidade) BETWEEN " . $this->db->escape($min) . " AND " . $this->db->escape($max);
            } else {
                $whereValor = " HAVING SUM(pedido_comprasitens.valor_unitario * pedido_comprasitens.quantidade) >= " . $this->db->escape($min);
            }
        }

        // Aumenta o limite do group_concat
        $this->db->query("SET SESSION group_concat_max_len = 10000");

        // Query principal focada no status 23
        $query = "
            SELECT
                SUM(pedido_comprasitens.valor_unitario * pedido_comprasitens.quantidade) AS soma,
                GROUP_CONCAT(DISTINCT pedido_comprasitens.idPedidoCompraItens SEPARATOR ';') AS idPedidoCompraItens,
                GROUP_CONCAT(DISTINCT distribuir_os.idDistribuir SEPARATOR ';') AS idDistribuir,
                GROUP_CONCAT(DISTINCT distribuir_os.idOs SEPARATOR ';') AS idOs,
                GROUP_CONCAT(DISTINCT insumos.descricaoInsumo SEPARATOR ';') AS descricaoInsumo,
                GROUP_CONCAT(DISTINCT os.unid_execucao SEPARATOR ';') as unid_execucao,
                status_cond_pgt.nome_status_cond_pgt AS nomePgto,
                pedido_comprasitens.cod_pgto AS condPgto,
                pedido_comprasitens.idPedidoCompra,
                fornecedores.nomeFornecedor,
                distribuir_os.data_cadastro, /* Data da solicitação original */
                MAX(distribuir_os.data_alteracao) as data_alteracao, /* Última alteração (quando chegou no status 23) */
                usuarios.nome AS nomeUserOrc /* Comprador/Solicitante */
                /* Flags de aprovação intermediária podem ser omitidas aqui se não forem exibidas na tela final */
            FROM distribuir_os
            JOIN pedido_cotacaoitens ON pedido_cotacaoitens.idDistribuir = distribuir_os.idDistribuir
            JOIN pedido_comprasitens ON pedido_comprasitens.idCotacaoItens = pedido_cotacaoitens.idCotacaoItens
                AND pedido_cotacaoitens.idPedidoCompraItens = pedido_comprasitens.idPedidoCompraItens
            ...
            LEFT JOIN fornecedores ON fornecedores.idFornecedores = pedido_compras.idFornecedores
            LEFT JOIN status_cond_pgt ON status_cond_pgt.id_status_cond_pgt = pedido_comprasitens.idCondPgto
            LEFT JOIN usuarios ON usuarios.idUsuarios = distribuir_os.idUser_aguardandoOrc /* Ou usuario_cadastro */
            WHERE 1=1
             /* $where2 já contém: AND distribuir_os.idStatuscompras = 23 AND distribuir_os.rejeitado != 1 */
             /* E outros filtros vindos do controller */
             $where2

             /* Garante valor unitário válido */
             AND pedido_comprasitens.valor_unitario IS NOT NULL
             AND pedido_comprasitens.valor_unitario > 0

            GROUP BY pedido_comprasitens.idPedidoCompra /* Agrupa por Ordem de Compra */
            $whereValor /* Aplica filtro de valor total da OC, se houver */
            ORDER BY data_alteracao ASC /* Ordena pela data que chegou para aprovação final */
        ";

        // Debug (opcional):
         // log_message('debug', "SQL Query (itensAprovacaoFinal):\n" . $query);

        return $this->db->query($query)->result();
    } // Fim da função itensAprovacaoFinal	
	
/**
     * Busca todos os itens (idDistribuir, idPedidoCompraItens, idPedidoCompra)
     * que estão no status 23 (Aguardando Aprovação Final)
     * a partir de uma lista de IDs de OS.
     *
     * @param array $arrayOs Lista de IDs de OS
     * @return array
     */
    public function getItensParaAprovacaoLote($arrayOs) {
        if (empty($arrayOs)) {
            return [];
        }
        
        $ids = implode(',', array_map('intval', $arrayOs));

        $query = "
            SELECT DISTINCT 
                d.idDistribuir, 
                pci.idPedidoCompra,
                pci.idPedidoCompraItens
            FROM distribuir_os d
            JOIN pedido_cotacaoitens pcot ON d.idDistribuir = pcot.idDistribuir
            JOIN pedido_comprasitens pci ON pcot.idCotacaoItens = pci.idCotacaoItens 
                 AND pcot.idPedidoCompraItens = pci.idPedidoCompraItens
            WHERE d.idOs IN ($ids)
              AND d.idStatuscompras = 23
              AND d.rejeitado != 1
        ";
        return $this->db->query($query)->result();
    }
    ...
    /**
     * Busca TODOS os itens (idDistribuir, idPedidoCompraItens, valor)
     * que pertencem a uma lista de OCs (idPedidoCompra)
     * e que estão no status 23 (Aguardando Aprovação Final).
     *
     * @param array $arrayIdPedidoCompra Lista de IDs de Pedido de Compra
     * @return array
     */
    public function getTodosItensDasOCsParaAprovar($arrayIdPedidoCompra) {
        if (empty($arrayIdPedidoCompra)) {
            return [];
        }
        
        $ids = implode(',', array_map('intval', $arrayIdPedidoCompra));

        $query = "
            SELECT 
                d.idDistribuir, 
                pci.idPedidoCompraItens,
                (pci.valor_unitario * d.quantidade) as valor_item
            FROM distribuir_os d
            JOIN pedido_cotacaoitens pcot ON d.idDistribuir = pcot.idDistribuir
            JOIN pedido_comprasitens pci ON pcot.idCotacaoItens = pci.idCotacaoItens 
                 AND pcot.idPedidoCompraItens = pci.idPedidoCompraItens
            WHERE pci.idPedidoCompra IN ($ids)
              AND d.idStatuscompras = 23
              AND d.rejeitado != 1
        ";
        return $this->db->query($query)->result();
    ...
    /**
     * Adiciona um registro ao histórico de aprovação em lote.
     *
     * @param array $data Dados para inserir
     * @return int ID inserido
     */
    public function addHistoricoAprovacaoLote($data) {
        $this->db->insert('aprovacao_lote_historico', $data);
        return $this->db->insert_id();
    }

    /**
     * Busca os registros do histórico de aprovação em lote.
     *
     * @return array
     */
    public function getHistoricoAprovacaoLote() {
        $this->db->from('aprovacao_lote_historico');
        $this->db->order_by('data_aprovacao', 'DESC');
        $this->db->limit(100); // Limita aos últimos 100 lotes
