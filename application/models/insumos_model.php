...
class Insumos_model extends CI_Model {
    ...
    {
        $allowed = ['descricaoInsumo','pn_insumo','referencia'];
        if (!in_array($field, $allowed, true)) return [];
        $search_safe = $this->db->escape_like_str($search);
        $query = "
            SELECT * FROM insumos,subcategoriaInsumo, categoriaInsumos
            WHERE insumos.idSubcategoria = subcategoriaInsumo.idSubcategoria and subcategoriaInsumo.idCategoria = categoriaInsumos.idCategoria and $field LIKE '%{$search_safe}%'
            ORDER BY $field
        ";
 ...
 function getBydescricaoSubCategoria_editar($descricaoSubcategoria,$idCategoria = ''){
    ...
    public function addsubCategoria($descricaoSubcategoria){
     ...
     {
         $allowed = ['descricaoSubcategoria','descricaoCategoria'];
         if (!in_array($field, $allowed, true)) return [];
         $search_safe = $this->db->escape_like_str($search);
         $query = "
             SELECT * FROM subcategoriaInsumo, categoriaInsumos
             WHERE subcategoriaInsumo.idCategoria = categoriaInsumos.idCategoria and $field LIKE '%{$search_safe}%'
    ...
    function getInsumo2($cod){
       $this->db->where('idInsumos', (int)$cod);
       return $this->db->get('insumos')->row();
    }
    ...
    public function getCategoriaByIdSubcategoria($id){
        ...
        $query = "SELECT * FROM categoriaInsumos JOIN subcategoriaInsumo ON subcategoriaInsumo.idCategoria = categoriaInsumos.idCategoria WHERE idSubcategoria = ?";
