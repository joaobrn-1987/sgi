 ...
 $this->db->join('estoque_entrada', $table.'.idInsumos = estoque_entrada.id_produto');
   ...
   $this->db->join('estoque_saida', 'estoque_entrada.id_estoque_saida = estoque_saida.idestoque_saida', 'left');  
	...
	public function getWhereLikeos($idInsumos = '',$cat = '', $querydatacadastro = '', $codigo='')
    {
		if($codigo <> '')
		{
			$query_codigo = " and insumos.pn_insumo like '%".$this->db->escape_like_str($codigo)."%'";
		}
		else
		{
			$query_codigo = '';
		...
		if($idInsumos <> '')
		{
			$query_insumo = " and estoque_entrada.id_produto = ".(int)$idInsumos;
		}
		else
		{
			$query_insumo = '';
		...
		if($cat <> '')
		{
			$query_cat = " and categoriaInsumos.idCategoria in (".implode(',', array_map('intval', explode(',', preg_replace('/[()]/', '', $cat)))).")";
		}
		else
		{
			$query_cat = '';
 ...
 $this->db->join('estoque_entrada', $table.'.idInsumos = estoque_entrada.id_produto');
  ...
  $this->db->join('estoque_saida','estoque_saida.idestoque_saida = estoque_entrada.id_estoque_saida','left');
	...
	public function getestoquesaida($idInsumos)
    {
		if($idInsumos <> '')
		{
			$query_insumo = " and estoque_saida.id_produto = ".(int)$idInsumos;
		}
		else
		{
			$query_insumo = '';
