...
class Orcamentos extends CI_Controller
{
    ...
    function gerenciar()
    {
        ...
        if($data_inicio && $data_fim){
            $data_inicio= converterData($data_inicio);
            $data_fim= converterData($data_fim);
            if (!preg_match('/^\d{4}-\d{2}-\d{2}/', $data_inicio) || !preg_match('/^\d{4}-\d{2}-\d{2}/', $data_fim)) {
                $data_inicio = null;
                $data_fim = null;
            }
            if ($data_inicio && $data_fim) {
                $abertura = " and orcamento.data_abertura between '$data_inicio' and '$data_fim'";
            }
        }else{
            $abertura = " and (orcamento.data_abertura > '2023-06-01 00:00:00'  or orcamento_item.status_item = 2)";
        }*/
        
        $abertura = '';
        $abertura2 = '';
        $exibirVenc = $this->input->post('exibirVenc');
        $this->data['exibirVenc'] = $exibirVenc;
        if($exibirVenc){                
            $abertura = "";
            $abertura2 = '';
        }else{
            $abertura = " and orcamento.idstatusOrcamento != 14";
            $abertura2 = 'orcamento.idstatusOrcamento != 14';
        }
        if($this->input->post('idstatusOrcamento')){
            $abertura = "";
            $abertura2 = '';
            $this->data['exibirVenc'] = 1;            
        }
        ...
        if (!empty($cod_orc) || !empty($clientes_id) || !empty($idstatusOrcamento) || !empty($idGrupoServico) || !empty($idNatOperacao) || !empty($referencia) || !empty($num_pedido) || !empty($num_nf) || !empty($status <> '') || !empty($idProdutos <> '') || !empty($descricao_item <> '') || !empty($os <> '') ) {
            ...
            $config['total_rows'] = $this->orcamentos_model->numrowsWhereLikeorc2($cod_orc, $clientes_id, $idstatusOrcamento, $idGrupoServico, $idNatOperacao, $referencia, $num_pedido, $num_nf, $status, $idProdutos, $descricao_item, $os, $abertura);
            $config['per_page'] = 10;
            ...
            $this->data['results'] = $this->orcamentos_model->getWhereLikeorc2($cod_orc, $clientes_id, $idstatusOrcamento, $idGrupoServico, $idNatOperacao, $referencia, $num_pedido, $num_nf, $status, $idProdutos, $descricao_item, $os, $abertura);
            ...
            $this->data['results'] = $this->orcamentos_model->getorc3('orcamento', '*, orcamento.idOrcamentos as id_Orcam, grupo_servico.nome as nomeGrupoServ', $abertura2, $config['per_page'], $this->uri->segment(3));
        }
...
public function carregarVendedoresAuxiliares() {
    ...
    $auxiliaresAuxiliares = $this->Orcamentos_model->buscarAuxiliares($idVendedor);

    // Adicionando logs no CodeIgniter
    log_message('debug', 'ID Vendedor: ' . $idVendedor);
    log_message('debug', 'Resultado Busca: ' . print_r($auxiliaresAuxiliares, true));

    echo json_encode($auxiliaresAuxiliares);
}
    ...
    function adicionar()
    {
        ...
        $this->data['dados_gruposervico'] = $this->orcamentos_model->getGrupoServico();
        $this->data['dados_natoperacao'] = $this->orcamentos_model->getNatOperacao();
        $this->data['dados_tipoqtd'] = $this->orcamentos_model->getAllTipoQtd();
        $this->data['dados_tipoentrega'] = $this->orcamentos_model->getAllTipoEntrega();
        ...
        if ($this->form_validation->run('orcamento') == false) {
			...
			$data_envio = (!empty($data_envio) && $data_envio != '0') ? $data_envio : NULL;
					
			$idVendedorAuxiliar = $this->input->post('idVendedorAuxiliar');
			$idVendedorAuxiliar = (!empty($idVendedorAuxiliar) && $idVendedorAuxiliar != '') ? $idVendedorAuxiliar : NULL;
            ...
            $data = array(
				...
				'idVendedorAuxiliar' => $idVendedorAuxiliar,	
				'enviado_cliente' => $this->input->post('enviado_cliente'),				
				...
				'planejadoPCP' => 0 // Valor padrão para planejadoPCP
                ...
                for ($x = 0; $x < $contador; $x++) {
                    ...
                    if ($this->input->post('orc')[$x] == "serv") {
                        $data2 = array(
                            ...
                            //'statusDesenho' => 1,
                            'idStatusPeritagem' => 1,
                            'idTipoQtd' => $this->input->post('tipoqtd')[$x],
                    ...
                    $idOs2 = $this->criarOs($idOrcItem);/*
                    $desenhoRepositorio = $this->desenho_model->getDesenhoRepositoryByIdProduto($id_produto);
                    if($desenhoRepositorio){
                        $dataRepos = array("idOs"=>$idOs2,
                            "idOrcamento_item"=>$idOrcItem,
                            "idDesenho"=>$desenhoRepositorio->idDesenhos,
                            "idVinculadoPor"=>51,
                            "data_vinculado"=>date('Y-m-d H:i:s'));

                    }$this->desenho_model->add("os_desenho",$dataRepos);*/
                    $this->orcamentos_model->edit('os', array("unid_execucao"=>$this->input->post('unex')[$x],"id_tipo"=>$this->input->post('tipoos')[$x]), "idOs", $idOs2);
                    if ($this->input->post('orc')[$x] == 'fab') {
                        if (!empty($data4)) {
                        ...
                        if (!empty($escopo)) {
                            ...
                            foreach ($escopoItens as $r) {
                                ...
                                if (!empty($r->idProduto)) {
                                    //$desenho = $this->desenho_model->getDesenhoRepositoryByIdProduto();
    ...
    function criarOs($idOrcamentoItem)
    {
        ...
        $this->orcamentos_model->add("os_sub", $dataSubOs,true);
        return $nova_os;
    ...
    function editar($idOrc = null){
        ...
        $this->data['result'] = $this->orcamentos_model->getById3('orcamento', (!empty($this->uri->segment(3))?$this->uri->segment(3):$idOrc));

        $objOrca = $this->orcamentos_model->getOnlyOrcamento($this->uri->segment(3));
        $this->load->model('peritagem_model');
        $this->load->model('produtos_model');
        $this->load->model('producao_model');
        $this->load->model('os_model');
        ...
        $were = 'clientes.idClientes = ' . $objOrca->idClientes;
        ...
        $this->data['dados_gruposervico'] = $this->orcamentos_model->getGrupoServico();
        $this->data['dados_natoperacao'] = $this->orcamentos_model->getNatOperacao();
        $this->data['dados_item'] = $this->orcamentos_model->getorc_itemBC((!empty($this->uri->segment(3))?$this->uri->segment(3):$idOrc));
        $this->data['dados_classe'] = $this->peritagem_model->getTypeClass();
        ...
        $this->load->library('form_validation');
        $this->data['custom_error'] = '';
        $this->data['redirecionarNovoOrc'] = $this->uri->segment(4);
        ...
        if ($this->form_validation->run('orcamento') == false) {
            ...
            $data = array(
                ...
                'valorCotacao' => str_replace(",",".",str_replace(".","",$this->input->post('valor_cotacao'))),
                'moedaCotacao' => $this->input->post('moeda_cotacao')
            ...
            if ($this->orcamentos_model->edit('orcamento', $data, 'idOrcamentos', $this->input->post('idOrcamentos')) == TRUE) {
                ...
                for ($x = 0; $x < $contador; $x++) {
                    ...
                    if(!empty($os2)){
                        $this->orcamentos_model->edit("os", array("unid_execucao"=>($this->input->post('unex')[$x]?$this->input->post('unex')[$x]:0),"id_tipo"=>($this->input->post('tipoos')[$x])?$this->input->post('tipoos')[$x]:0),"idOs",$os2->idOs);
                    }
                    ...
                    if($this->input->post('idstatusOrcamento') == 14){
                        if($orcamento_item->status_item != 2){
                            $this->orcamentos_model->edit("os",array("idStatusOs"=>212),"idOs",$os2->idOs);
                        }
                    ...
                    if (!empty($this->input->post('orc')[$x]) && !empty($this->input->post('tipo_prod')[$x])) {
                        if ($this->input->post('orc')[$x] == 'serv') {
                            ...
                            if (!empty($controleEtapa)) {
                                ...
                                $controleEtapa->idOs = ($this->input->post('idOs2')[$x]?$this->input->post('idOs2')[$x]:null);
                                $this->producao_model->edit("controle_etapa", $controleEtapa, "idControleEtapa", $controleEtapa->idControleEtapa);
                            }else{
                                $dataControle = array(
                                    "idOrcamento" => $this->input->post('idOrcamentos'),
                                    "idOrcamento_item" => $this->input->post('id_orc_item')[$x],
                                    "idProduto" => $this->input->post('idProdutos')[$x],
                                    "idOs"=>($this->input->post('idOs2')[$x]?$this->input->post('idOs2')[$x]:null),
                                    "descricaoItem" => $this->input->post('descricao_item')[$x],
                                    "quantidade" => $this->input->post('qtd')[$x],
                                    "idStatusEtapaServico" => 1,
                                    "data_cadastro" => date('Y-m-d H:i:s')
                                );
    ...
    function excluir_item()
    {
        ...
        if ($this->orcamentos_model->getitemos2($id) == false) {
            $this->orcamentos_model->delete('orcamento_item', 'idOrcamento_item', $id);
            $this->orcamentos_model->delete('os', 'idOrcamento_item', $id);
    ...
    public function visualizar()
    {
        ...
        $this->data['result'] = $this->orcamentos_model->getById3('orcamento', $this->uri->segment(3));
        ...
        $this->data['itens_orcamento'] = $this->orcamentos_model->getorc_itemBC($this->data['result']->idOrcamentos);
    ...
    public function aprovar()
    {
        ...
        foreach($this->data['itens_orcamento'] as $b){
            if(!$b->unid_execucao || !$b->id_tipo){
                $this->session->set_flashdata('error', 'Não é permitido a aprovação de orçamentos sem unidade de execução e/ou tipo O.S. definidos.');
                redirect(base_url() . 'index.php/orcamentos/editar/' . $this->uri->segment(3));
            }
        }
        ...
        if ($this->input->post('item')[0] > 0) {
            for ($x = 0; $x < $contador; $x++) {
                ...
                if ($itemOrc->tipoOrc == 'serv') {
                    ...
                    if (empty($OrcEscopo) || $itemOrc->idStatusPeritagem != 4) {
            ...
            for ($x = 0; $x < $contador; $x++) {
                ...
                if($response->status == "success"){
                    ...
                    $response3 = json_decode($this->simplexCriarAtividade($response->response->id,$itemOrc->descricao_item,$nova_os,'1697560863836x689668401219240000',$dataAbertura,$data_entrega));
                ...
                if ($itemOrc->tipoOrc == 'fab') {
					...
					log_message('debug', 'itemOrc JSON: ' . json_encode($itemOrc));
    ...
    public function orcCustom()
    {
        ...
        if($this->input->get('orcCustomIngles')){
            $this->orcCustomInt();
            return;
        }
        $data['exibirvalor'] = $this->input->get('exibirValor');
        ...
        $data['result'] = $this->orcamentos_model->orcCustom2($idOrcamentos, $dataimpri);
    ...
    public function orcCustomInt()
    {
        ...
        $data['exibirvalor'] = $this->input->get('exibirValor');
        $dataInicial = $this->input->get('dataInicial');
		$data['data_envio']= $this->input->get('data_envio');
        ...
        */
        ...
        $this->load->helper('mpdf');
        echo $html = $this->load->view('orcamentos/imprimir/imprimirOrcIngles', $data, true);
    ...
    function excluir()
    {
        ...
        $this->orcamentos_model->edit('os', array("idStatusOs"=>44), 'idOrcamentos', $idorc);
    ...
    public function escopoPrint()
    {
        ...
        $data['result'] = $this->orcamentos_model->orcItemCustom($idOrcamentos);
    ...
    public function escopoPrintTodos()
    {
        ...
        foreach ($escopo as $r) {
            ...
            $result = $this->orcamentos_model->orcItemCustom($r->idOrcItem);
        ...
        //exit;
        $data['result'] = $this->orcamentos_model->orcCustom($idOrcamentos);
        //$data['descricoes'] =  $this->orcamentos_model->getOrcItemWithOs($idOrcamentos);
        $data['escopo'] = $escopo2;
        $this->load->helper('mpdf');
    ...
    function desmembrarorcamento(){
        $this->load->model('producao_model');
        $idOrcItem = $this->input->post("checkDesmembrar");
        $idOrc = $this->input->post("idOrcDesmembrar");
        $objOrc = $this->orcamentos_model->getOnlyOrcamento($idOrc);
        $novoOrc = clone $objOrc;
        $novoOrc->idOrcamentos = NULL;
        $novoOrc->data_abertura = date('Y-m-d H:i:s');
        $novoIdOrc = $this->orcamentos_model->add("orcamento", $novoOrc,true);
        if(empty($idOrcItem)){
            $this->session->set_flashdata('error', 'Selecione um item para desmembrar.');
            redirect('orcamentos/editar/' . $idOrc);
        }
        foreach($idOrcItem as $r){
            $this->orcamentos_model->edit("os",array("idOrcamentos"=>$novoIdOrc),"idOrcamento_item",$r);
            $this->orcamentos_model->edit("orcamento_item",array("idOrcamentos"=>$novoIdOrc),"idOrcamento_item",$r);
            $this->orcamentos_model->addHistTransfOrc(array("idOrcamento_item"=>$r,"idOrcamentos_origem"=>$idOrc,"idOrcamentos_destino"=>$novoIdOrc,"tipo"=>"desmembrar"));
            $controleEtapa = $this->producao_model->getControleEtapaByIdOrcamento_item($r);
            $controleEtapa->idOrcamento = $novoIdOrc;
            $this->orcamentos_model->edit("controle_etapa",$controleEtapa,"idControleEtapa",$controleEtapa->idControleEtapa);
        }
        $itens = $this->orcamentos_model->getitemorc($idOrc);
        if(empty($itens)){
            $this->orcamentos_model->edit("orcamento",array("idstatusOrcamento"=>12),"idOrcamentos",$idOrc);
        }
        //$this->load->view('cotacao/imprimir/imprimir_cotacao',$data,true);
        //echo "<script type='text/javascript'>window.open('".base_url()."index.php/orcamentos/editar/".$idOrc."');window.open('".base_url()."index.php/orcamentos/editar/".$novoIdOrc."','_blank')</script>";
        //echo "<script type='text/javascript'>window.open('".base_url()."index.php/orcamentos/editar/".$novoIdOrc."','_blank')</script>";
        
        $this->session->set_flashdata('success', 'Os itens do orçamento foram desmembrados. O orçamento criado é <b>'. $novoIdOrc.'</b>');
        //$this->editar($idOrc);
        redirect('orcamentos/editar/' . $idOrc.'/'.$novoIdOrc);
    }

    function transferiritens(){
        $this->load->model('producao_model');
        $idOrcItem = $this->input->post("checkTransferir");
        $idOrc = $this->input->post("idOrcTransferir");
        $idOrcDestino = $this->input->post("idOrcTransferirDestino");
        $novoOrc = $this->orcamentos_model->getOnlyOrcamento($idOrcDestino);
        if(empty($idOrcItem)){
            $this->session->set_flashdata('error', 'Selecione um item para desmembrar.');
            redirect('orcamentos/editar/' . $idOrc);
        }
        if($novoOrc){
            $novoIdOrc = $novoOrc->idOrcamentos;
            foreach($idOrcItem as $r){
                $this->orcamentos_model->edit("os",array("idOrcamentos"=>$novoIdOrc),"idOrcamento_item",$r);
                $this->orcamentos_model->edit("orcamento_item",array("idOrcamentos"=>$novoIdOrc),"idOrcamento_item",$r);
                $this->orcamentos_model->addHistTransfOrc(array("idOrcamento_item"=>$r,"idOrcamentos_origem"=>$idOrc,"idOrcamentos_destino"=>$novoIdOrc,"tipo"=>"transferir"));
                $controleEtapa = $this->producao_model->getControleEtapaByIdOrcamento_item($r);
                if($controleEtapa){
                    $controleEtapa->idOrcamento = $novoIdOrc;
                    $this->orcamentos_model->edit("controle_etapa",$controleEtapa,"idControleEtapa",$controleEtapa->idControleEtapa);
            ...
            $this->session->set_flashdata('success', 'Os itens selecionados foram transferidos com sucesso para o orçamento <b>'. $novoIdOrc.'</b>');
            redirect('orcamentos/editar/' . $idOrc.'/'.$novoIdOrc);
        }else{
            $this->session->set_flashdata('error', 'O orçamento <b>'.$this->input->post("idOrcTransferirDestino").'</b> não existe');
            redirect('orcamentos/editar/' . $idOrc);
        }
    ...
    function editarDescricaoImprimir(){
        $idOrcamentoItem = $this->input->post("idOrcItem");
        $descricao = $this->input->post("descricao");
        $data = array("descricao_item_imprimir"=>$descricao);
        $this->orcamentos_model->edit("orcamento_item",$data,"idOrcamento_item",$idOrcamentoItem);
        echo json_encode(array("result"=>true));
    }
	private function resolveStatusInicialOrcamento($postedStatusId = null)
	{
		// >>>>> EDITE ESTA LISTA COM OS IDS DOS USUÁRIOS/VENDEDORES QUE DEVEM INICIAR EM "PARA ORÇAR" (15)
		$VENDEDORES_STATUS_INICIAL_15 = [1, 182, 273 /*, 85, 87, 98, 6 ... */];

		$idUsuario = (int) $this->session->userdata('idUsuarios');

		if (in_array($idUsuario, $VENDEDORES_STATUS_INICIAL_15, true)) {
			return 15; // "PARA ORÇAR"
		}

		// Se veio no POST, mantém o que o usuário selecionou; senão, padrão 11 ("ORÇAMENTO PENDENTE")
		$postedStatusId = is_null($postedStatusId) ? $this->input->post('idstatusOrcamento') : $postedStatusId;
		if (!empty($postedStatusId) && ctype_digit((string)$postedStatusId)) {
			return (int)$postedStatusId;
		}

		return 11; // fallback seguro para quem não está na lista
	}	
}
