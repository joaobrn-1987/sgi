...
echo $ret = json_encode($retorno, JSON_PRETTY_PRINT);
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run('clientes') == false) {
            ...
            $data = array(
                ...
                'nomeClienteCart' => strtoupper($this->input->post('nomeClienteCart')),
   ...
   //salvar log da açao
    ...
    public function cadastrarSolicitante2(){
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'aCliente')){
            //$this->session->set_flashdata('error','Você não tem permissão para cadastrar solicitante.');
            echo json_encode(array("result"=>false,"msg"=>"Você não tem permissão para cadastrar solicitante."));
            return;
        }
        $data = array(
            'nome' => strtoupper($this->input->post('solicitante')),
            'idClientes' => $this->input->post('cliente'),
            'email_solici' => strtoupper($this->input->post('email'))
        );
        if( $this->clientes_model->getBysolici_cliente($this->input->post('email_solici'),$this->input->post('idClientes')) == true){
            $this->session->set_flashdata('error','Esse email e cliente ja possui cadastro.');
        }
        $retorno = $this->clientes_model->add('clientes_solicitantes',$data,true);
        echo json_encode(array('result'=>true,'msg'=>"Solicitante cadastrado com sucesso.",'id'=>$retorno));
    }

    public function cadastrarCliente(){
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'aCliente')){
            //$this->session->set_flashdata('error','Você não tem permissão para cadastrar solicitante.');
            echo json_encode(array("result"=>false,"msg"=>"Você não tem permissão para cadastrar clientes."));
            return;
        }
        $data = array(
            'nomeCliente' => strtoupper($this->input->post('nomeCliente')),
        ...
        if( $this->clientes_model->getByDocumento($this->input->post('documento')) == true)
        {
        ...
        if (is_numeric($id = $this->clientes_model->add('clientes', $data, true)) ) {
            /*
            $response = json_decode($this->simplexCadastroCliente(strtoupper($this->input->post('nomeCliente')))); 
            if($response->status == "success"){
                $data2 = array("idSimplexCliente"=>$response->response->id);
                $this->clientes_model->edit('clientes', $data2, 'idClientes', $id);
            }*/
            
            echo json_encode(array('result'=>true,'msg'=>"Cliente cadastrado com sucesso.",'id'=>$id));
        }
    }
