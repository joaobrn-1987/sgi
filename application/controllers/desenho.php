...
class Desenho extends CI_Controller {
    ...
    function aguardandodesenho(){
        ...
        if(!empty($this->uri->segment(3))){
            ...
            if(!empty($this->input->post('idOrcamento'))){
                if($primeiro){
                    $where = " WHERE orcamento.idOrcamentos = ".(int)$this->input->post('idOrcamento');
                    $primeiro = false;
                }else{
                    $where = " and orcamento.idOrcamentos = ".(int)$this->input->post('idOrcamento');
                }
            }
            if(!empty($this->input->post('idOrcamentoItem'))){
                if($primeiro){
                    $where = " WHERE orcamento_item.idOrcamento_item = ".(int)$this->input->post('idOrcamentoItem');
                    $primeiro = false;
                }else{
                    $where = " and orcamento_item.idOrcamento_item = ".(int)$this->input->post('idOrcamentoItem');
                }
            }
            if(!empty($this->input->post('idOs'))){
                if($primeiro){
                    $where = " WHERE os.idOs = ".(int)$this->input->post('idOs');
                    $primeiro = false;
                }else{
            ...
            if(!empty($this->input->post('descricaoOrc'))){
                $_descricaoOrc = $this->db->escape_like_str($this->input->post('descricaoOrc'));
                if($primeiro){
                    $where = " WHERE orcamento_item.descricao_item like '%".$_descricaoOrc."%'";
                    $primeiro = false;
                }else{
                    $where = " and orcamento_item.descricao_item like '%".$_descricaoOrc."%'";
                }
            }
            if(!empty($this->input->post('statusDesenho'))){
                if($primeiro){
                    $where = " WHERE orcamento_item.statusDesenho = ".(int)$this->input->post('statusDesenho');
                    $primeiro = false;
                }else{
                    $where = " and orcamento_item.statusDesenho = ".(int)$this->input->post('statusDesenho');
                }
            }
            if(!empty($this->input->post('pn'))){
                $_pn = $this->db->escape_like_str($this->input->post('pn'));
                if($primeiro){
                    $where = " WHERE produtos.pn like '%".$_pn."%'";
                    $primeiro = false;
                }else{
                    $where = " and produtos.pn like '%".$_pn."%'";
                }
            }
            if(!empty($where)){
                $this->data['result'] = $this->orcamentos_model->getOrçAguardandoDesenho3($where.' and orcamento.idstatusOrcamento != 12 ');
                foreach($this->data['result'] as $c){
                    if($c->unid_execucao){
                        $c->objOrcamento = $this->orcamentos_model->getOrçAguardandoDesenho2($where.' and orcamento.idstatusOrcamento != 12 and os.unid_execucao = '.$c->unid_execucao);
                    }else{
                        $c->objOrcamento = $this->orcamentos_model->getOrçAguardandoDesenho2($where.' and orcamento.idstatusOrcamento != 12 and (os.unid_execucao is null or os.unid_execucao = "")');
                    }
                    
                }
            }else{
                $where = " where (orcamento_item.statusDesenho = 1 or orcamento_item.statusDesenho = 2) and orcamento.idstatusOrcamento != 12";
                $this->data['result'] = $this->orcamentos_model->getOrçAguardandoDesenho3($where);
                foreach($this->data['result'] as $c){
                    if($c->unid_execucao){
                        $c->objOrcamento = $this->orcamentos_model->getOrçAguardandoDesenho2($where.' and os.unid_execucao = '.$c->unid_execucao);
                    }else{
                        $c->objOrcamento = $this->orcamentos_model->getOrçAguardandoDesenho2($where.' and (os.unid_execucao is null or os.unid_execucao = "")');
                    }
    ...
    function visualizardesenhos(){
        ...
        //$this->data['result'] = $this->orcamentos_model->getorc_item5( $this->uri->segment(3));
        $this->data['result'] = $this->orcamentos_model->getorc_item5list( str_replace("_",",",$this->uri->segment(4)));
        $this->data['view'] = 'desenho/visualizardesenhos';
       	$this->load->view('tema/topo',$this->data);
    }/*
    ...
    function finalizar(){
        $this->load->model('os_model');
        $buttonSalvar = $this->input->post("btnSalvar");
        if(!empty($buttonSalvar)){
            $this->salvarchecklist();
            return;
        }
        $idOrcItem = $this->input->post("idOrcItem2");
        $orcItem = $this->orcamentos_model->getitem($idOrcItem);
        if($orcItem->tipoOrc == "fab"){
            $data2 = array(
                "fechadoPCP"=>1
            );
            $this->desenho_model->edit('orcamento_item',$data2,'idOrcamento_item',$idOrcItem);
        }
        $data = array(
            "statusDesenho"=>3
        );
        $this->desenho_model->edit('os',$data,'idOrcamento_item',$idOrcItem);
        $os = $this->os_model->getByid_table($idOrcItem, 'os', 'idOrcamento_item');
        $this->os_model->insertOSHis($os);
        $data = array(
            "statusDesenho"=>3,
            "idUserFinalizarDesenho"=>$this->session->userdata('idUsuarios'),
            "data_finalizado_desenho"=>date('Y-m-d H:i:s')
        );
        ...
        redirect(base_url().'index.php/desenho/visualizardesenhos/'.$idOrcItem.'/'.$this->input->post('idOrcItemRedirect'));
    }
    function salvarchecklist(){
        ...
        $idOrc = $this->input->post("idOrcamentoSalvar");

        $orcItem = $this->input->post("idOrcamentoItemChecklist");
        //Serviços
        if(!empty($orcItem)){
            for($x=0;$x<count($orcItem);$x++){
                //Itens que existem no escopo
                $orcservescitens = $this->input->post("idOrcServicoEscopoItens_".$orcItem[$x]);
                $pn = $this->input->post("pn_".$orcItem[$x]);
                $idpn = $this->input->post("idpn_".$orcItem[$x]);
                $contagemNovo = $this->input->post("contagemNovo".$orcItem[$x]);

                $escopoOrc = $this->peritagem_model->getOrcEscopoActiveByOrcItem($orcItem[$x]);

                //Itens para acrescentar no escopo
                $novoEscopoPN = $this->input->post("novoEscopoPN_".$orcItem[$x]);
                $novoEscopoIdProduto = $this->input->post("novoEscopoIdProduto_".$orcItem[$x]);
                
                $novoEscopoDescProd = $this->input->post("novoEscopoDescProd_".$orcItem[$x]);
                $novoEscopoIdClasse = $this->input->post("novoEscopoIdClasse_".$orcItem[$x]);
                $novoEscopoQtd = $this->input->post("novoEscopoQtd_".$orcItem[$x]);
                //echo json_encode($idpn);

                
                //Atualiza os PN do escopo do orçamento item ($orcItem[$x])
                if(!empty($escopoOrc)){
                    $escopo = $this->peritagem_model->getOrcEscopo2($escopoOrc->idOrcServicoEscopo);
                    $catalogo = $this->peritagem_model->getCatalogoAtivosByIdProduto2($escopo->idProduto);
                    if(!empty($orcservescitens)){
                        for($y=0;$y<count($orcservescitens);$y++){
                            $idProd = $this->peritagem_model->getEscopoItemByIdOrcEscopoItem($orcservescitens[$y]);
                            if(!empty($idpn[$y])&& $idpn[$y] != "" && $idpn[$y]){
                                $data = array(
                                    "idProduto"=>$idpn[$y]
                                );
    
                                $this->peritagem_model->edit("servico_escopo_itens",$data,"idServicoEscopoItens",$idProd->idServicoEscopoItens);
                            }else if(!empty($pn[$y]) && $pn[$y] != "" && $pn[$y]){
                                $produtos = $this->produtos_model->getByPn2($pn[$y]);
                                if(!empty($produtos)){
                                    //echo "2";
                                    $data = array(
                            ...
                            if(!empty($idpn[$y]) && !empty($catalogo)){
                                if(!$this->peritagem_model->getItemCatalogoItemByIdProdutoByIdCatalogo($idpn[$y],$catalogo->idCatalogoProduto)){
                                    
                                    $data = array(
                                        "idCatalogoProduto"=>$catalogo->idCatalogoProduto,
                                        "idProduto"=>$idpn[$y],
                                        "quantidade"=>$this->input->post("pnQtd_".$orcservescitens[$y]),
                                        "tipoProd"=>$this->input->post("pnIdClasse_".$orcservescitens[$y]),
                                        "data_cadastro"=>date('Y-m-d H:i:s'),
                                        "idUsuario"=>$this->session->userdata('idUsuarios'),
                                        "ativo"=>1
                                    );
                                    $this->peritagem_model->add("catalogo_produto_itens",$data);/**/
                                }
                            }                        
                        }
                    }
                    //Atualiza o catalogo adicionando os PN novos)
                    if(!empty($catalogo)){
                        for($y=0;$y<count($novoEscopoIdProduto);$y++){
                            if(!empty($novoEscopoIdProduto[$y])){
                                $produtos = $this->produtos_model->getById($novoEscopoIdProduto[$y]);
                                $catalogoItem = !isset($catalogo->idCatalogoProduto) ? null : $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($novoEscopoIdProduto[$y],$catalogo->idCatalogoProduto);
                                if(empty($catalogoItem)){
                                    //cria novo itens para o catalogo
                                    $data = array(
                                        "idCatalogoProduto"=>$catalogo->idCatalogoProduto,
                                        "idProduto"=>$novoEscopoIdProduto[$y],
                                        "quantidade"=>$novoEscopoQtd[$y],
                                        "tipoProd"=>$novoEscopoIdClasse[$y],
                                        "data_cadastro"=>date('Y-m-d H:i:s'),
                                        "idUsuario"=>$this->session->userdata('idUsuarios'),
                                        "ativo"=>1
                                    );
                                    $this->peritagem_model->add("catalogo_produto_itens",$data);
                                    $escopoItem = $this->peritagem_model->getEscopoItensByIdEscopoAndIdProduto($escopo->idServicoEscopo,$novoEscopoIdProduto[$y]);
                                
                                    if(empty($escopoItem)){
                                        //cria novo itens para o escopo
                                        $data = array(
                                            "idServicoEscopo"=>$escopoOrc->idServicoEscopo,
                                            "descricaoServicoItens"=>"Substituir ".$produtos->descricao,
                                            "idProduto"=>$novoEscopoIdProduto[$y],
                                            "idClasse"=>2,
                                            "tipoCampo"=>"input",
                                            "ativo"=>1
                                        ...
                                        //cria novo itens para o escopo do orçamento
                                        $data2 = array(
                                        ...
                                        $idItemServic = $this->peritagem_model->add("orc_servico_escopo_itens",$data2,true);
                                        $name = "newImage_".$orcItem[$x]."_".$contagemNovo[$y];
                                        //echo $_FILES[$name];
                                        //exit;
                                        if(isset($_FILES[$name])){
                                            $_allowed_ext = ['pdf','dwg','jpg','jpeg','png','gif','docx','xlsx','mp4','mov','jfif'];
                                            $qtd = count($_FILES[$name]["error"]);
                                            for($xd = 0; $xd < $qtd; $xd++){
                                                if(empty($_FILES[$name]["tmp_name"][$xd])) continue;
                                                $target_dir = "./assets/uploads/desenho/";
                                                $imageFileType = strtolower(pathinfo(basename($_FILES[$name]["name"][$xd]),PATHINFO_EXTENSION));
                                                if(!in_array($imageFileType, $_allowed_ext, true)){
                                                    $this->session->set_flashdata('error','Formato não aceito.');
                                                    continue;
                                                }
                                                $safe_name = uniqid('', true).".".$imageFileType;
                                                $target_file = $target_dir . $safe_name;
                                                if (move_uploaded_file($_FILES[$name]["tmp_name"][$xd], $target_file)) {
                                                    $data = array(
                                                        'nomeArquivo' =>  $safe_name,
                                                        'imagem' =>  $safe_name,
                                                        'caminho' => 'assets/uploads/desenho/',
                                                        'tipo' => 'DESENHO',
                                                        'tamanho' => filesize($target_file),
                                                        'extensao' => $imageFileType,
                                                        'statusAnexo' => 2,
                                                        'idProduto'=>$novoEscopoIdProduto[$y],
                                                        'data_cadastro' => date('Y-m-d H:i:s'),
                                                        'user_proprietario' => $this->session->userdata('idUsuarios'),
                                                        'idOrcamentos_item' => $orcItem[$x],
                                                        'idOrcServicoEscopoItens' => $idItemServic
                                                    );
                            ...
                            }else if(!empty($novoEscopoPN[$y])){
                                $produtos = $this->produtos_model->getByPn2($novoEscopoPN[$y]);
                                $catalogoItem = $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($produtos->idProdutos,$catalogo->idCatalogoProduto);
                                if(empty($catalogoItem)){   
                                    //cria novo itens para o catalogo                                 
                                    $data = array(
                                        "idCatalogoProduto"=>$catalogo->idCatalogoProduto,
                                ...
                                $escopoItem = $this->peritagem_model->getEscopoItensByIdEscopoAndIdProduto($escopo->idServicoEscopo,$produtos->idProdutos);
                        ...
                        $orcamentoItem = $this->orcamentos_model->getOrcItemWithOs($orcItem[$x]);
                        //cria um novo catalogo  
                        $data = array(
                            "idProduto"=>$orcamentoItem->idProdutos,
                            "descricaoCatalogo"=>$orcamentoItem->descricao_item,
                            "tipoProd"=>$orcamentoItem->tipoProd,
                            "data_cadastro"=>date('Y-m-d H:i:s'),
                            "idUsuario"=>$this->session->userdata('idUsuarios'),
                            "ativo"=>1
                        );
                        ...
                        for($y=0;$y<count($novoEscopoIdProduto);$y++){
                            if(!empty($novoEscopoIdProduto[$y])){
                                $catalogoItem = $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($novoEscopoIdProduto[$y], $idCatalogo);
                                if(empty($catalogoItem)){
                                ...
                                $produtos = $this->produtos_model->getByPn2($novoEscopoPN[$y]);
                                $catalogoItem = $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($produtos->idProdutos, $idCatalogo);
                                if(empty($catalogoItem)&&!empty($produtos)){   
                                ...
                                if(empty($escopoItem)){
                                    ...
                                    //cria novo itens para o escopo
                                    $data = array(
                                        "idServicoEscopo"=>$escopoItem->idServicoEscopo,
                                        "descricaoServicoItens"=>"Substituir ".$produtos->descricao,
                                        "idProduto"=>$produtos->idProdutos,
                                        "idClasse"=>2,
                                        "tipoCampo"=>"input",
                        ...
                        if(!empty($orcservescitens)){
                            for($y=0;$y<count($orcservescitens);$y++){
                                if(!empty($idpn[$y]) && !empty($catalogo)){
                                    if(!$this->peritagem_model->getItemCatalogoItemByIdProdutoByIdCatalogo($idpn[$y],$idCatalogo)){
        ...
        $idOs = $this->input->post("idOsCatalogo");
        //Fabricação
        if(!empty($idOs)){            
            for($x=0;$x<count($idOs);$x++){
                $novoCatalogoPN = $this->input->post("novoCatalogoPN_".$idOs[$x]);
                $novoCatalogoIdProduto = $this->input->post("novoCatalogoIdProduto_".$idOs[$x]);
                $novoCatalogoDescProd = $this->input->post("novoCatalogoDescProd_".$idOs[$x]);
                $novoCatalogoQtd = $this->input->post("novoCatalogoQtd_".$idOs[$x]);
                $novoEscopoIdClasse = $this->input->post("novoEscopoIdClasse_".$idOs[$x]);
                $contagemNovo = $this->input->post("contagemNovo".$idOs[$x]);
                $idCatalogoItem = 0;
                $objOrcItem = $this->os_model->getInfoOs($idOs[$x]);
                for($y=0;$y<count($novoCatalogoIdProduto);$y++){
                    if(!empty($novoCatalogoIdProduto[$y])){
                        /**/
                        //echo "01";
                        $data = array(
                            "idProduto"=>$novoCatalogoIdProduto[$y],
                            "quantidade"=>$novoCatalogoQtd[$y],
                            "tipoProd"=>$novoEscopoIdClasse[$y],
                            "data_cadastro"=>date('Y-m-d H:i:s'),
                            "idUsuario"=>$this->session->userdata('idUsuarios'),
                            "ativo"=>1,
                            "idCatalogoProduto"=>$objOrcItem->idCatalogo
                        );
                        $idCatalogoItem = $this->peritagem_model->add('catalogo_produto_itens',$data,true);
                    } else if(!empty($novoCatalogoPN[$y])){
                        $produtos = $this->produtos_model->getByPn2($novoEscopoPN[$y]);
                        //$objOrcItem = $this->os_model->getInfoOs($idOs[$x]);
                        //echo "02";
                        if(!empty($produtos)){
                    ...
                    if(!empty($idCatalogoItem)){
                        //echo "03";
                        
                        $objCatItem = $this->peritagem_model->getCatalogoItem($idCatalogoItem);
                        //echo json_encode($objCatItem);
                        $subOs = $this->orcamentos_model->getSubOsbyidOs($idOs[$x]); 
                        $produtos = $this->produtos_model->getById($objCatItem->idProduto);
                        $dataSubOs = array(
                            "idOs"=>$idOs[$x],
                            "idProduto_master"=>$objOrcItem->idProdutos,
                            "idProduto_sub"=>$objCatItem->idProduto,
                            "posicao"=>count($subOs),
                            "data_insert"=>date('Y-m-d H:i:s'),
                            "quantidade"=>$novoCatalogoQtd[$y]*$objOrcItem->qtd,
                            "idClasse"=>2,
                            "ativo"=>1,
                            "descricaoOsSub"=>$produtos->descricao
                        );
                        $idOsSub2 = $this->peritagem_model->add("os_sub",$dataSubOs,true);
                        
                        $name = "newImage_fab_".$idOs[$x]."_".$contagemNovo[$y];
                                       // echo $_FILES[$name];
                                       // exit;
                        if(isset($_FILES[$name])){
                            $_allowed_ext_fab = ['pdf','dwg','jpg','jpeg','png','gif','docx','xlsx','mp4','mov','jfif'];
                            $qtd = count($_FILES[$name]["error"]);
                            for($xd = 0; $xd < $qtd; $xd++){
                                if(empty($_FILES[$name]["tmp_name"][$xd])) continue;
                                $target_dir = "./assets/uploads/desenho/";
                                $imageFileType = strtolower(pathinfo(basename($_FILES[$name]["name"][$xd]),PATHINFO_EXTENSION));
                                if(!in_array($imageFileType, $_allowed_ext_fab, true)){
                                    $this->session->set_flashdata('error','Formato não aceito.');
                                    continue;
                                }
                                $safe_name_fab = uniqid('', true).".".$imageFileType;
                                $target_file = $target_dir . $safe_name_fab;
                                if (move_uploaded_file($_FILES[$name]["tmp_name"][$xd], $target_file)) {
                                    $data = array(
                                        'nomeArquivo' =>  $safe_name_fab,
                                        'imagem' =>  $safe_name_fab,
                                        'caminho' => 'assets/uploads/desenho/',
                                        'tipo' => 'DESENHO',
                                        'tamanho' => filesize($target_file),
                                        'extensao' => $imageFileType,
        ...
        redirect(base_url().'index.php/desenho/visualizardesenhos/'.$idOrc.'/'.$this->input->post('idOrcItemRedirect'));
    }
    function cadastrarDesenhoNoItem (){
        ...
        if(!empty($_FILES['file']["name"])){
            $_allowed_cdi = ['pdf','dwg','jpg','jpeg','png','gif','docx','xlsx','mp4','mov','jfif'];
            $target_dir = "./assets/uploads/desenho/";
            $nome = $_FILES['file']["name"];
            $imageFileType = strtolower(pathinfo(basename($_FILES['file']["name"]),PATHINFO_EXTENSION));
            if(!in_array($imageFileType, $_allowed_cdi, true)){
                echo json_encode(array("result"=>false,"error"=>"Formato não aceito."));
                return;
            }
            $safe_name_cdi = uniqid('', true).".".$imageFileType;
            $_FILES['file']["name"] = $safe_name_cdi;
            $target_file = $target_dir . $safe_name_cdi;
            ...
            if(!empty($idOrcSerItem)){
                ...
                $objOrcServItem2 = $this->peritagem_model->getEscopoItensOrcByidEscopoItem($idOrcSerItem);
            }else{
                $objOrcServItem = null;
                $objOrcServItem2 = null;
            }
            ...
            if (move_uploaded_file($_FILES['file']["tmp_name"], $target_file)) {
                ...
                $this->peritagem_model->add('anexo_desenho',$data);
                if(!empty($objOrcServItem2) && !empty($objOrcServItem->idProduto)){
                    $itens = $this->peritagem_model->getOrcServicoItensByIdOrcEscopoByIdProduto($objOrcServItem2->idOrcServicoEscopo,$objOrcServItem->idProduto);
                    if(!empty($itens)){
                        foreach($itens as $m){
                            if($m->idOrcServicoEscopoItens != $idOrcSerItem){
                                $data = array(
                                    'nomeArquivo' => $nome,
                                    'imagem' =>  $_FILES['file']["name"],
                                    'caminho' => 'assets/uploads/desenho/',
                                    ...
                                    'user_proprietario' => $this->session->userdata('idUsuarios'),
                                    'idOrcamentos_item' => $this->input->post('idOrcItem'),
                                    'idOrcServicoEscopoItens' => $m->idOrcServicoEscopoItens
                                );
                                $this->peritagem_model->add('anexo_desenho',$data);
                            }
                        }
                ...
                /*
                $objServItem = $this->peritagem_model->getServicoItensByIdOrcServicoEscopoItens($idOrcSerItem);
                $listServItem = $this->peritagem_model->getEscopoItensByIdEscopoAndIdProduto($objServItem->idServicoEscopo,$objServItem->idProdutos);
                if()*/
            }
        ...
        echo json_encode(array("result"=>true,"anexoGeral"=>$getAnexoOrcItem,"anexoItem"=>$getAnexoOrcServItem,"nome"=>$nome));
    }
    function cadastrarDesenhoNoItem2 (){
        $this->load->model('peritagem_model');
        $this->load->model('orcamentos_model');
        if(!empty($_FILES['file']["name"])){
            $_allowed_cdi2 = ['pdf','dwg','jpg','jpeg','png','gif','docx','xlsx','mp4','mov','jfif'];
            $target_dir = "./assets/uploads/desenho/";
            $imageFileType = strtolower(pathinfo(basename($_FILES['file']["name"]),PATHINFO_EXTENSION));
            if(!in_array($imageFileType, $_allowed_cdi2, true)){
                echo json_encode(array("result"=>false,"error"=>"Formato não aceito."));
                return;
            }
            $safe_name_cdi2 = uniqid('', true).".".$imageFileType;
            $_FILES['file']["name"] = $safe_name_cdi2;
            $target_file = $target_dir . $safe_name_cdi2;
            if(!empty($this->input->post('idOrcSerItem'))){
                $idOrcSerItem = $this->input->post('idOrcSerItem');
            }else{
                $idOrcSerItem = NULL;
    ...
    function cadastrarDesenhoNaSubOs (){
        ...
        $nome = "";
        if(!empty($_FILES['file']["name"])){
            $_allowed_sub = ['pdf','dwg','jpg','jpeg','png','gif','docx','xlsx','mp4','mov','jfif'];
            $target_dir = "./assets/uploads/desenho/";
            $nome = $_FILES['file']["name"];
            $imageFileType = strtolower(pathinfo(basename($_FILES['file']["name"]),PATHINFO_EXTENSION));
            if(!in_array($imageFileType, $_allowed_sub, true)){
                echo json_encode(array("result"=>false,"error"=>"Formato não aceito."));
                return;
            }
            $safe_name_sub = uniqid('', true).".".$imageFileType;
            $_FILES['file']["name"] = $safe_name_sub;
            $target_file = $target_dir . $safe_name_sub;
            ...
            if(!empty($objOsSub->idProduto_sub)){
                ...
                $listAllSubOs = $this->orcamentos_model->getAllSubOsbyIdOsAndIdProd($objOsSub->idOs,$idProduto);
            ...
            if (move_uploaded_file($_FILES['file']["tmp_name"], $target_file)) {
                ...
                if(!empty($listAllSubOs)){
                    foreach($listAllSubOs as $c){
                        $data = array(
                            'nomeArquivo' => $nome,
                            'imagem' =>  $_FILES['file']["name"],
                            'caminho' => 'assets/uploads/desenho/',
        ...
        echo json_encode(array("result"=>true,"anexoGeral"=>$getAnexoIdOs,"anexoItem"=>$getAnexoOrcServItem, "nomeArquivo"=>$nome));
    }
    ...
    function cadastrarDesenhoNoOrcECat(){
        ...
        $this->load->model('orcamentos_model');
        if(!empty($_FILES['file']["name"])){
            $_allowed_oec = ['pdf','dwg','jpg','jpeg','png','gif','docx','xlsx','mp4','mov','jfif'];
            $target_dir = "./assets/uploads/desenho/";
            $imageFileType = strtolower(pathinfo(basename($_FILES['file']["name"]),PATHINFO_EXTENSION));
            if(!in_array($imageFileType, $_allowed_oec, true)){
                echo json_encode(array("result"=>false,"error"=>"Formato não aceito."));
                return;
            }
            $safe_name_oec = uniqid('', true).".".$imageFileType;
            $_FILES['file']["name"] = $safe_name_oec;
            $target_file = $target_dir . $safe_name_oec;
    ...
    function finalizardesenho(){
        ...
        $data3 = array("statusDesenho"=>3,"idUserFinalizarDesenho"=>$this->session->userdata('idUsuarios'),"data_finalizado_desenho"=>date('Y-m-d H:i:s'));
        $verifyOs = false;
        $verifyOrcitem = false;
        ...
        if(!empty($idOs)){
            foreach($idOs as $r){
                ...
                if($objOrcItem->tipoOrc == 'serv'){
                    ...
                    if($objOrcItem->idStatusPeritagem == 1){
                        $this->peritagem_model->edit("orcamento_item",array("idStatusPeritagem"=>2),"idOrcamento_item", $objOrcItem->idOrcamento_item);
                    }
        ...
        redirect(base_url().'index.php/desenho/visualizardesenhos/'.$this->input->post('idOrcamento2').'/'.$this->input->post('idOrcItemRedirect'));   
    }
    ...
    function adicionar_pn(){
        ...
        if ($this->form_validation->run('produtos') == false) {
            ...
            }else if(empty($_FILES['imag_dwg']["name"]) && !empty($_FILES['imag_jpg']["name"])){                
    ...
    function findPn2(){
        ...
        $verifica = $this->peritagem_model->getCatalogoByPnAndTipo($pn,$tipo);
        ...
        $json = array('result'=>true,'resultado'=>$resultado, 'pn'=>$pn, 'tipo'=>$tipo);
        echo json_encode($json);
    ...
    function salvarpndesenho(){
        $this->load->model('produtos_model');
        $this->load->model('peritagem_model');
        $pn = $this->input->post("pn");
        $idpn = $this->input->post("idpn");
        $pnQtd = $this->input->post("pnQtd");
        $pnIdClasse = $this->input->post("pnIdClasse");
        $checkbox = ($this->input->post("checkbox")?1:0);

        $idServicoEscopoItens = $this->input->post("idServicoEscopoItens");
        
        $escopo = $this->peritagem_model->getEscopoByIdEscopoItem($idServicoEscopoItens);
        ...
        if(!empty($catalogo)){
            ...
            $produto = $this->produtos_model->getById($escopo->idProduto);
            $dataCat = array(
                "idProduto"=>$escopo->idProduto,
                "descricaoCatalogo"=>$produto->descricao,
                "tipoProd"=>$escopo->tipoServico,
                "data_cadastro"=>date("Y-m-d H:i:s"),
                "idUsuario"=>$this->session->userdata('idUsuarios'),
                "ativo"=>1
            );
            $idCatalogo = $this->peritagem_model->add("catalogo_produto",$dataCat,true);
        }

        if(!empty($idpn)){
            $catalogoitem = $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($idpn,$idCatalogo);
            if(empty($catalogoitem)){
                $data3 = array(
                    "idProduto"=>$idpn,
                    "idCatalogoProduto"=>$idCatalogo,
                ...
                $this->peritagem_model->add("catalogo_produto_itens",$data3);
            }
            $data = array(
                "idProduto"=>$idpn
            );
            $objpn = $this->produtos_model->getById($idpn);
            $objpn->item_comercial = $checkbox;
            $this->peritagem_model->edit("servico_escopo_itens",$data,"idServicoEscopoItens",$idServicoEscopoItens);
            $this->peritagem_model->edit("produtos",$objpn,"idProdutos",$objpn->idProdutos);
            echo json_encode(array("result"=>true,"produto"=>$objpn));
            return;
        }else if(!empty($pn)){
            $pn_principal = $this->produtos_model->getByPn($pn);
            
            if(!empty($pn_principal)){
                $catalogoitem = $this->peritagem_model->getCatalogoItensByIdProdutoAndIdCatalogo($pn_principal->idProdutos,$idCatalogo);
                if(empty($catalogoitem)){
                    $data3 = array(
                        "idProduto"=>$pn_principal->idProdutos,
                        "idCatalogoProduto"=>$idCatalogo,
                        "quantidade"=>$pnQtd,
                        "tipoProd"=>$pnIdClasse,
                        "data_cadastro"=>date("Y-m-d H:i:s"),
                ...
                $this->peritagem_model->edit("servico_escopo_itens",$data,"idServicoEscopoItens",$idServicoEscopoItens);
                $pn_principal->item_comercial = $checkbox;
                $this->peritagem_model->edit("produtos",$pn_principal,"idProdutos",$pn_principal->idProdutos);
                echo json_encode(array("result"=>true,"produto"=>$pn_principal));
                return;
            }
        }
        echo json_encode(array("result"=>false));
    ...
    function reavaliacaodesenho(){
        $this->load->model('peritagem_model');  
        $this->load->model('os_model');   
        foreach($this->input->post('idOrcItem') as $r){            
            $data2 = array (
                'statusDesenho'=>2
            );
            $this->peritagem_model->edit('orcamento_item',$data2,'idOrcamento_item',$r);
            $this->peritagem_model->edit('os',$data2,'idOrcamento_item',$r);     
            $os = $this->os_model->getByid_table($r, 'os', 'idOrcamento_item');
            $this->os_model->insertOSHis($os);      
            
        }
        if($this->input->post('idOrcItem')){
        ...
        $this->session->set_flashdata('success', 'Item disponível para anexar desenho!');
        redirect(base_url().'index.php/desenho/visualizardesenhos/'.$this->input->post('idOrcamento2').'/'.$this->input->post('idOrcItemRedirect'));
        
    }
    function catalogo(){ 
        $this->load->model('peritagem_model');
        $this->data['view'] = 'desenho/catalogo';
    ...
    function adicionarCatalogo(){
        $this->load->model('produtos_model');
        $this->data['produto'] = $this->produtos_model->getById($this->input->get("idProduto"));
        $this->data['tipo'] = $this->input->get("tipo");
    ...
    function downdes(){
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vdesenhoOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar Desenho.');
            redirect(base_url());
        }
        $this->load->model('os_model');
        $idDesenho = $this->uri->segment(3);
        $anexo_desenho = $this->os_model->getimagem($idDesenho);
        redirect(base_url() . $anexo_desenho->caminho . $anexo_desenho->imagem);
    }
}
