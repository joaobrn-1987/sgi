...
public function getEstatisticasEstoqueFiltrado()
{
    ...
    if (!empty($prode)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "descricaoInsumo LIKE '%" . $prode . "%'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "descricao LIKE '%" . $prode . "%'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
    if (!empty($pne)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "pn_insumo = '" . $pne . "'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "pn = '" . $pne . "'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
    if (!empty($idOse)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "idOs = '" . $idOse . "'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "idOs = '" . $idOse . "'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
    if (!empty($idEmpresae)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "emitente = '" . $idEmpresae . "'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "emitente = '" . $idEmpresae . "'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
    if (!empty($idDepartamentoe)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "departamento = '" . $idDepartamentoe . "'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "departamento = '" . $idDepartamentoe . "'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
    if (!empty($local)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "local LIKE '%" . $local . "%'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "local LIKE '%" . $local . "%'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
    if (!empty($unidades)) {
        $whereInsumos .= ($primeiroInsumo ? '' : ' AND ') . "quantidade = '" . $unidades . "'";
        $whereProdutos .= ($primeiroProduto ? '' : ' AND ') . "quantidadeEstoque = '" . $unidades . "'";
        $primeiroInsumo = false;
        $primeiroProduto = false;
    }
...
//////////////////////////////registra consulta contador/////////////////////
    ...
    public function Busca_Estoque_Filtro(){
        ...
        $this->load->model('estoque_model');
        $this->load->model('insumos_model');
		$this->load->model('produtos_model'); // Carregar o model de produtos para acessar getEstatisticasConsultasPorListaIds
        $primeiro = true;
        $where = "";
        $whereProd = "";
        ...
        if(!empty($this->input->post("idOs"))){
            if($primeiro){
                $where = $where." almo_estoque.idOs = ".$this->input->post("idOs");
                $whereProd = $whereProd." almo_estoque_produtos.idOs = ".$this->input->post("idOs");
                $primeiro = false;
            }else{
                $where = $where." and almo_estoque.idOs = ".$this->input->post("idOs");
                $whereProd = $whereProd." and almo_estoque_produtos.idOs = ".$this->input->post("idOs");
            }
        }
        ...
        if(!empty($this->input->post("local"))){
            if($primeiro){
                $where = $where." almo_estoque_locais.local like '%".$this->input->post("local")."%'";
                $whereProd = $whereProd." almo_estoque_locais.local like '%".$this->input->post("local")."%'";
                $primeiro = false;
            }else{
                $where = $where." and almo_estoque_locais.local like '%".$this->input->post("local")."%'";
                $whereProd = $whereProd." and almo_estoque_locais.local like '%".$this->input->post("local")."%'";
            }
        }
        if(!empty($this->input->post("unidades"))){
            if($primeiro){
                $where = $where." os.unid_execucao in (".$this->input->post("unidades").")";
                $whereProd = $whereProd." os.unid_execucao in (".$this->input->post("unidades").")";
                $primeiro = false;
            }else{
		...
		//////////////////////////nova contagem acessos estoque////////////////////
//////////////////////////nova contagem acessos estoque////////////////////
		$idProduto = null; // Inicialize a variável com null

		if ($this->input->post('pne')) {
			
			$idProduto = $this->input->post('pne'); // Pegar o ID do produto do filtro
			$estatisticas = $this->produtos_model->getEstatisticasConsulta($idProduto);
			$data['estatisticas'] = $estatisticas;
		} else {
			$data['estatisticas'] = []; // Ou alguma outra atribuição padrão, caso não haja ID de produto
		}
		//////////////////////////nova contagem acessos estoque////////////////////
		//////////////////////////nova contagem acessos estoque////////////////////
		
		
        /*$resultado = $this->unificarEstoques(array_merge(
		$this->almoxarifado_model->getEstoqueFilter($where),
		$this->almoxarifado_model->getEstoqueFilterProdutos($whereProd)));
        $json = array('result'=>true,'resultado'=>$resultado);*/

        $resultadoEstoque = $this->unificarEstoques(array_merge(
            $this->almoxarifado_model->getEstoqueFilter($where),
            $this->almoxarifado_model->getEstoqueFilterProdutos($whereProd))
        );
        $jsonEstoque = array('result'=>true,'resultado'=>$resultadoEstoque);

        $idsProdutos = array_unique(array_column($resultadoEstoque, 'idProduto'));

        // Busca as estatísticas de consulta para os produtos filtrados
        $estatisticas = $this->produtos_model->getEstatisticasConsultasPorListaIds($idsProdutos);

        // Adiciona as estatísticas ao array JSON de resposta
        $jsonEstoque['estatisticas'] = $estatisticas;

// Grava log de cada item consultado
        // Grava log de cada item consultado (APÓS A FILTRAGEM)
        if (!empty($resultadoEstoque)) {
            foreach ($resultadoEstoque as $r) {
                if (isset($r->idProduto)) {
                    $dadosLog = array(
                        'idProduto' => $r->idProduto,
                        'quantidade_estoque' => isset($r->quantidade) ? $r->quantidade : null,
    ...
    // Busca e adiciona as estatísticas individuais de cada produto
    $resultadoComEstatisticas = [];
    foreach ($resultadoEstoque as $item) {
        if (isset($item->idProduto)) {
            ...
            if ($estatisticas) {
                $item->total_consultas = $estatisticas->total_consultas;
                $item->estoque_zero = $estatisticas->estoque_zero;
                $item->estoque_maior_igual_1 = $estatisticas->estoque_maior_igual_1;
            } else {
                $item->total_consultas = 0;
                $item->estoque_zero = 0;
                $item->estoque_maior_igual_1 = 0;
            }
        } else {
            $item->total_consultas = 0;
            $item->estoque_zero = 0;
		...
		$jsonEstoque = array('result' => true, 'resultado' => $resultadoComEstatisticas);
        echo json_encode($jsonEstoque);
    }
...
public function buscarCnpj()
{
    ...
    // Validar o CNPJ antes de prosseguir
    if (!$this->validarCnpj($cnpj)) {
        $this->session->set_flashdata('error', 'CNPJ inválido.');
        redirect('almoxarifado');
    }

    // Configurações da API
    $url = "https://app.omie.com.br/api/v1/geral/clientes/";
    $app_key = "5477735855592";
    $app_secret = "a393489e1ea56cd4cd04db6e66762bb9";
    ...
    $data = [
        ...
        "param" => [
            [
                ...
                "codigo_cliente_integracao" => $cnpj
            ]
        ]
    ];
    ...
    // Chamada para a API usando cURL
    $ch = curl_init();
...
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
    ...
    if (curl_errno($ch)) {
        ...
        curl_close($ch);
        $this->session->set_flashdata('error', 'Erro na requisição: ' . $error_msg);
        redirect('almoxarifado/recebercompras');
    }
	...
	log_message('debug', 'Resposta da API: ' . print_r($response, true));
	
$response = json_decode($response, true); // Decodifica o JSON para um array associativo

if (isset($response['codigo_cliente_omie'])) {
    // Sucesso: Cliente encontrado
    $this->session->set_flashdata('success', 'Cliente encontrado: ' . $response['razao_social']);
} else {
    // Erro: Cliente não encontrado
    $this->session->set_flashdata('error', 'CNPJ não encontrado.');
}


    if (isset($result['faultstring'])) {
        // Erro na API
        $this->session->set_flashdata('error', 'Erro na consulta: ' . $result['faultstring']);
        redirect('almoxarifado/recebercompras');
    } elseif (!empty($result['clientes_cadastro'])) {
        // Cliente encontrado
        $cliente = $result['clientes_cadastro'][0]; // Pega o primeiro cliente da resposta
        $this->session->set_flashdata('success', 'Cliente encontrado: ' . $cliente['razao_social']);
        redirect('almoxarifado/recebercompras');
    } else {
...
private function validarCnpj($cnpj)
{
    ...
    function imprimircodbarras(){
        ...
        $this->data['result'] = json_decode($_GET["my_data"]);
