...
class Peritagem extends CI_Controller
{
    ...
    function listaperitagem(){ 
        ...
        if(!empty($idOrcamento)){
            $where .= " and orcamento.idOrcamentos = ".(int)$idOrcamento;
        }
        $idOs = $this->input->post('idOs');
        if(!empty($idOs)){
            $where .= " and os.idOs = ".(int)$idOs;
        }
        $pn = $this->input->post('pn');
        if(!empty($pn)){
            $where .= " and produtos.pn like '%".$this->db->escape_like_str($pn)."'";
        }
        $descricaoOrc = $this->input->post('descricaoOrc');
        if(!empty($descricaoOrc)){
            $where .= " and orcamento_item.descricao_item like '%".$this->db->escape_like_str($descricaoOrc)."'";
        }
        $statusPeritagem = $this->input->post('statusPeritagem');
        if($this->input->post('verificarForm')){
            if(!empty($statusPeritagem)){
                $where .= " and orcamento_item.idStatusPeritagem = ".(int)$statusPeritagem;
                $statusPeritagem = 0;
            }
        }else{
            if($this->permission->checkPermission($this->session->userdata('permissao'),'cPeritagem') && empty($where)){
                $where .= " and orcamento_item.idStatusPeritagem = 3";
                $statusPeritagem = 3;
            }else{
                $where .= " and orcamento_item.idStatusPeritagem = 2";
                $statusPeritagem = 2;
            }
        }
        
        $this->data['idStatusPeritagem2'] = $statusPeritagem;
        $this->data['statusPeritagem'] = $this->peritagem_model->getStatusPeritagem2();
        $this->data['aguardandoPer'] = $this->peritagem_model->aguardandoPeritagem3($tipo,$where);
        foreach($this->data['aguardandoPer'] as $c){
            if($c->unid_execucao){
                $c->objOrcamento = $this->peritagem_model->aguardandoPeritagem2($tipo,$where." and os.unid_execucao = ".$c->unid_execucao);
            }else{
                $c->objOrcamento = $this->peritagem_model->aguardandoPeritagem2($tipo,$where." and (os.unid_execucao is null or os.unid_execucao = \"\")");
            }
        }
    ...
    function criarescopoorc($id){
        ...
        if(!empty($escopoOrc)){
            ...
            if($this->input->post('idStatusEscopo')<=2){
                $data = array('idStatusEscopo'=>$this->input->post('idStatusEscopo'));
            ...
            if($this->input->post('idStatusEscopo')<=2){
                $data = array(
                    ...
                    'idStatusEscopo'=>$this->input->post('idStatusEscopo')
                );
            }else{
                $data = array(
                    'idServicoEscopo'=>$id,
    ...
    */
    ...
    function finalizarPeritagem(){
        ...
        foreach($this->input->post('idOrcServEscopo') as $r){
            ...
            $this->peritagem_model->edit("orcamento_item",array("idStatusPeritagem"=>4),"idOrcamento_item",$orcServicoEscopo->idOrcItem);
            $vendedor = $this->orcamentos_model->getVendedor($orc->idVendedor);
    ...
    function solicitarconfirmacao(){
        ...
        foreach($this->input->post('idOrcServEscopo') as $r){
            ...
            $this->peritagem_model->edit("orcamento_item",array("idStatusPeritagem"=>3),"idOrcamento_item",$orcServicoEscopo->idOrcItem);
            $orc = $this->orcamentos_model->get_item_orc2($orcServicoEscopo->idOrcItem);
            $os = $this->orcamentos_model->getos_item2($orcServicoEscopo->idOrcItem);
            foreach($os as $d){
    ...
    function salvarCatalogo2(){
        ...
        for($x=0;$x<count($listaProd);$x++){
            $data2 = array(
                "idCatalogoProduto"=>$idCatalogo,
                "idProduto"=>$listaProd[$x],
                "tipoProd"=>$this->input->post('tipoProd2')[$x],
                "quantidade"=>$this->input->post('quantidade2')[$x],
                "data_cadastro"=>date('Y-m-d H:i:s'),
                "idUsuario"=>$this->session->userdata('idUsuarios'),
                "ativo"=>1
            );
        ...
        redirect( base_url().'index.php/desenho/visualizarCatalogo/'.$idCatalogo );
    }
    ...
    function solicitarperitagem(){
        ...
        $data = array("idStatusEscopo"=>5);
        $itemOrcamento = $this->orcamentos_model->get_item_orc2($idOrcItem);
        if($itemOrcamento->idStatusPeritagem == 1){
            $data2 = array("statusDesenho"=>1,"data_solicitar_desenho"=>date('Y-m-d H:i:s'),"idStatusPeritagem"=>2);
        }else{
    ...
    function ordemservico(){
        ...
        if (!empty($idStatusOs)) {
            $ids_safe = implode(',', array_filter(array_map('intval', (array)$idStatusOs)));
            if (!empty($ids_safe)) {
                $query_status_producao = "($ids_safe)";
            }
        }
        $query_unid_execucao = "";
        if (!empty($unid_execucao)) {
            $ids_safe = implode(',', array_filter(array_map('intval', (array)$unid_execucao)));
            if (!empty($ids_safe)) {
                $query_unid_execucao = "($ids_safe)";
            }
        }
        $query_unid_faturamento = "";
        if (!empty($unid_faturamento)) {
            $ids_safe = implode(',', array_filter(array_map('intval', (array)$unid_faturamento)));
            if (!empty($ids_safe)) {
                $query_unid_faturamento = "($ids_safe)";
            }
        }
        $query_tipoos = "";
        if (!empty($id_tipo)) {
            $ids_safe = implode(',', array_filter(array_map('intval', (array)$id_tipo)));
            if (!empty($ids_safe)) {
                $query_tipoos = "($ids_safe)";
            }
        }
        $query_desenho = "";
        if (!empty($desenho)) {
            $desenho_safe = implode(',', array_filter(array_map('intval', (array)$desenho)));
            if (!empty($desenho_safe)) {
                $query_desenho = " and os.statusDesenho IN($desenho_safe)";
            }
        }

        $query_verifControl = "";
        if (!empty($verificacaoControle)) {
            $verificacaoControle_safe = implode(',', array_filter(array_map('intval', (array)$verificacaoControle)));
            if (!empty($verificacaoControle_safe)) {
                $query_verifControl = " and os.idVerificacaoControle IN($verificacaoControle_safe)";
            }
        }
    ...
    function recusarperitagem(){
        ...
        foreach($this->input->post('idOrcServEscopo') as $r){
            ...
            $idOrcServicoEscopo = $r;
            $orcServicoEscopo = $this->peritagem_model->getOrcEscopo2($idOrcServicoEscopo);
            $this->peritagem_model->edit('orcamento_item',$data2,'idOrcamento_item',$this->input->post('idOrcItemRedirect'));
