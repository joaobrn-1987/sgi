...
class fornecedores extends CI_Controller {
	...
	function gerenciar(){
        ...
        if (!empty($field) && !empty($search)) {
            ...
            $this->data['results'] = $this->fornecedores_model->get('fornecedores','idFornecedores,nomeFornecedor,documento,telefone,celular,email,rua,numero,bairro,cidade,estado,cep,site,endcomp,tipofor','',$config['per_page'],$this->uri->segment(3));
...
echo $ret = json_encode($retorno, JSON_PRETTY_PRINT);
    ...
    function adicionar() {
            ...
            if( $this->fornecedores_model->getByDocumento(set_value('documento')) == true)
        {
           ...
           redirect(base_url().'index.php/fornecedores/editar/'); 
    ...
    function adicionarInt() {
        ...
        if ($this->form_validation->run('fornecedores') == false) {
            ...
            if( $this->fornecedores_model->getByDocumento(set_value('endcomp')) == false)
        {
            $this->session->set_flashdata('error','Fornecedor ja possui cadastro.');
           redirect(base_url()); 
        }
			...
			if (is_numeric($id = $this->fornecedores_model->add('fornecedores', $data, true)) ) {
                ...
                //$this->salvarlog($this->session->userdata('idUsuarios'),'fornecedores','inserir',$descricao );
                $this->session->set_flashdata('success','Fornecedor adicionado com sucesso!');
                redirect(base_url() . 'index.php/fornecedores/editarInt/'.$id);
            } else {
                $this->data['custom_error'] = '<div class="form_error"><p>Ocorreu um erro.</p></div>';
            }
        }
        $this->data['view'] = 'fornecedores/adicionarFornecedorInt';
        $this->load->view('tema/topo', $this->data);
    ...
    function editar() {
        ...
        if ($this->form_validation->run('fornecedores') == false) {
            ...
            if ($this->fornecedores_model->edit('fornecedores', $data, 'idFornecedores', $this->input->post('idFornecedores')) == TRUE) {
                ...
                $this->session->set_flashdata('success','Fornecedor editado com sucesso!');
                redirect(base_url() . 'index.php/fornecedores/editarInt/'.$this->input->post('idFornecedores'));
            } else {
                $this->data['custom_error'] = '<div class="form_error"><p>Ocorreu um erro</p></div>';
            }
        }
        ...
        $this->data['result'] = $this->fornecedores_model->getById($this->uri->segment(3));
        $this->data['view'] = 'fornecedores/editarFornecedorInt';
        $this->load->view('tema/topo', $this->data);
