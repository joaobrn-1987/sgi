    ...
    function confirmarrecebimento(){
        ...
        if(!empty($idControleEtapa)){
            for($x=0;$x<sizeof($idControleEtapa);$x++){
                ...
                $itemOrc = $this->orcamentos_model->getitem($idOrcamento_item);
                if($itemOrc->idStatusPeritagem == 1){
                    $this->producao_model->edit("orcamento_item",array("statusDesenho"=>1,"idStatusPeritagem"=>2,"data_solicitar_desenho"=>date('Y-m-d H:i:s')),"idOrcamento_item",$controle_etapa_result->idOrcamento_item);
                }

                $orc_escopo = $this->peritagem_model->getOrcEscopoActiveByOrcItem($idOrcamento_item);
                if($orc_escopo->idStatusEscopo == 1){
                    $idStatusEscopo = 5;
                    //$this->producao_model->edit("orc_servico_escopo",array("idStatusEscopo"=>$idStatusEscopo),"idOrcServicoEscopo",$orc_escopo->idOrcServicoEscopo);
                }
