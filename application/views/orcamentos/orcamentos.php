                        ...
                        <div class="tab-content" style="background-color: #F9F9F9;border: 1px #cdcdcd solid;">
                            <div class="tab-pane active" id="tab1">
                                <div class="span12" id="divCadastrarOs" style="padding: 1%;">
                                    <form action="<?php echo base_url() ?>index.php/orcamentos" method="post" id="formOrc">
                                        ...
                                        <div class="span12" >
                                            <div class="span1" >
                                                ...
                                                <input id="verificar" class="span12" type="hidden" name="verificar" value="1" />
                                            </div>
                                            <div class="span1" >
                                        ...
                                        <div class="span12" style=" margin-left: 0">
                                            ...
                                            <div class="spanAjust" style="height: 51px;">
                                                <label style="height: 51px;display: flex;align-items: center;">Exibir Orç. Venc: <input type="checkbox" name="exibirVenc" <?php if(isset($exibirVenc)){if($exibirVenc==1){echo "checked";}}?> value="1"></label>
                                            </div>
                                            <div class="spanAjust">
                                                <br>
                                            ...
                                            <div class="spanAjust">
                                                <br>
                                                <a class="btn btn-success" type="button" onclick="exibirAntigas()">Orçamentos Antigos</a>
                                            </div>
...
// NÃO consultamos novamente o banco aqui. Usamos $orcitem->idOs que vem do JOIN correto no Model.
...
// Verificamos se o $orcitem já possui um ID de OS (vido do getorc_itemB)
if (!empty($orcitem->idOs)) {
    
    if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
        // Usamos $orcitem->idOs em vez de results3
        $ositem = '<font color="red"><a target="_blank" href="' . base_url() . 'index.php/os/visualizar/' . $orcitem->idOs . '" style="color: red;" onMouseOver="this.style.color=\'blue\'"
        onMouseOut="this.style.color=\'red\'"><b> OS:</b>' . $orcitem->idOs .'</a></font>';
    } else {
        $ositem = "<font color='red'><b> OS:</b>" . $orcitem->idOs . "</font>";
    }
} else {
    $ositem = '';
}							
...
if ($r->enviado_cliente == 0){
    $color = "style='color:red'";
    echo '<td ' . $color . '><strong>NÃO</strong></td>';
} elseif ($r->enviado_cliente == 1){
    $color = "style='color:green'";
    echo '<td ' . $color . '><strong>SIM</strong></td>';
}

if ($r->data_envio != null) {
    $color = "style='color:green'";
    echo '<td ' . $color . '>' .  date("d-m-Y H:i:s", strtotime($r->data_envio)) . '</td>';
} else {
    $color = "style='color:red'";
    echo '<td></td>';
}
						echo '<td style="color:grey">' . $r->nomeVendedor . '</td>';
                        echo '<td>';
