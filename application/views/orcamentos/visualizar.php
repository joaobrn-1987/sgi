...
<div class="widget-box">
	<div class="widget-title">
		<ul class="nav nav-tabs">
			...
			<div class="buttons">
				<?php //echo json_encode($itens_orcamento);
				if ($this->permission->checkPermission($this->session->userdata('permissao'), 'APOrcamento') && $itens_orcamento) {
									...
									<table width='70%' border='0'>
										<tr>
											<td colspan='2' align='center' height='50'>
												<b>
													<font size='5'><?php echo $dados_emitente->nome; ?></font>
												</b>
											</td>
										</tr>
										<tr>
	...
	font-size:12px;">
										...
										<tr>
											...
											<td width='50%'><?php echo $dados_clientes->nomeCliente; ?></td>
											<td align='left'>CNPJ:</td>
											<td><?php echo $dados_clientes->documento; ?></td>
										</tr>

										<tr>
											<td align='left' width='13%'>Email solicitante: </td>
											<td><?php echo $dados_solicitante->email_solici; ?></td>
											<td align='left' width='13%'> Nome solicitante: </td>
											<td><?php echo $dados_solicitante->nome; ?></td>

										</tr>

										<td align='left'>Telefone:</td>
										<td><?php echo $dados_clientes->telefone; ?></td>
										<td align='left'>Referência:</td>
										<td colspan='3'><?php echo $result->referencia ?></td>
...
$valorsub = 0;
$valortot = 0;
$valoripi_total = 0; // CORRIGIDO: soma de todos os ipi dos itens
$valordesconto = 0;
...
foreach ($itens_orcamento as $item) {
    ...
    $valorsub += $subtotal;
    $valoripi_total += $valoripi_item;
    $valortot += $valor_total_item;
    ...
    <tr>
        ...
        <td><?php echo $item->tipoOrc; ?></td>
        <?php
        if (!empty($item->tipoProd)) {
            if ($item->tipoProd == "pec") {
                echo '<td>Peça</td>';
            }
            if ($item->tipoProd == "cil") {
                echo '<td>Cilindro</td>';
            }
            if ($item->tipoProd == "sub") {
                echo '<td>Subconjunto</td>';
            }
            if ($item->tipoProd == "maq") {
                echo '<td>Maquina</td>';
            }
        } else {
            echo '<td></td>';
        ...
        <td align='center'><?php echo number_format($valor_total_item, 2, ",", "."); ?></td>
    </tr>
    <tr>
        <td colspan='14'><?php echo nl2br($item->detalhe); ?></td>
    </tr>
    <?php
    $count_item++;
}
								...
								<td>
									<table width='100%' class='comBordas'>
										...
										<tr>
											...
											<td align='center' rowspan='2'>VALOR IPI R$: <?php echo number_format($valoripi_total, 2, ",", "."); ?></td>
