...
<?php 
	if($redirecionarNovoOrc){
		echo "<script type='text/javascript'>window.open('".base_url()."index.php/orcamentos/editar/".$redirecionarNovoOrc."','_blank')</script>";
	}
?>
...
-->
	...
	<div align='right'>
		...
		Valor:<select name="exibirValor"><option value="0">Não</option><option value="1">Sim</option></select>
			<button class="btn btn-inverse tip-top" name="orcCustomIngles" value="orcCustomIngles"><i class="icon-print icon-white" ></i> Inglês</button>
			...
			if ($result->status_orc == 0) {
				...
				if ($this->permission->checkPermission($this->session->userdata('permissao'), 'APOrcamento') && $dados_item) {
						...
						<div class="span12 widget-box" id="divProdutosServicos" style="padding: 0.2%; margin-left: 0">
							<ul class="nav nav-tabs">
								...
								<li class="active vertical-center" id="">
									...
									<span> <strong>  Orçamento enviado: </strong> </span> 
									<input style="width:60px" type="checkbox" for="enviado_cliente" name="enviado_cliente" class='check span12' value="1" title="Marque esta opção se o orçamento foi enviado" 
										
									<?php if($result->enviado_cliente == 1) echo 'checked'; ?>  
											<td>
												<?php
													if (($result->data_envio != '0000-00-00 00:00:00') && ($result->enviado_cliente == 1)){
														$color = "style='color:green'";
														echo '<span ' . $color . '>' .  date("d-m-Y H:i:s", strtotime($result->data_envio)). '</span>';
														
													}
													if (($result->data_envio != '0000-00-00 00:00:00') && ($result->enviado_cliente == 0)){
														echo '';
													}	
													if (($result->data_envio == '0000-00-00 00:00:00') && ($result->enviado_cliente == 0)){
														echo '';
									...
									<?php 
									if(($result->enviado_cliente == 1) || ($result->data_envio <> '0000-00-00 00:00:00')){ ?>
										<input id="data_envio" type="hidden" name="data_envio" value="<?php echo $result->data_envio; ?>" />
									<?php }
									if(($result->enviado_cliente == 0) || ($result->data_envio == '0000-00-00 00:00:00')){ ?> 
										<input id="data_envio" type="hidden" name="data_envio" value="<?php echo date('Y-m-d H:i:s'); ?>" />
									<?php } ?>
...
<?php 
$idsParaExcluir = [2, 3, 4, 6, 7]; // IDs que não devem ser exibidos
?>
        ...
        <?php foreach ($dados_vendedor as $v) { ?>
            <?php if (!in_array($v->idVendedor, $idsParaExcluir)) { ?>
                <option value="<?php echo $v->idVendedor; ?>" 
                    <?php echo ($result->idVendedor == $v->idVendedor) ? 'selected="selected"' : ''; ?>>
                    <?php echo htmlspecialchars($v->nomeVendedor, ENT_QUOTES, 'UTF-8'); ?>
                </option>
            <?php } ?>
        <?php } ?>
    </select>
	 <input type="hidden" name="regiao" id="regiao" value="<?php echo htmlspecialchars($result->regiao, ENT_QUOTES, 'UTF-8'); ?>" readonly />
</div>
...
<div class="span2 control-group">
    ...
    <!--<pre><?php //var_dump($result->idVendedorAuxiliar); ?></pre>-->
...
<select class="span12 form-control" name="idVendedorAuxiliar" id="idVendedorAuxiliar">
    ...
    <?php foreach ($dados_vendedorAuxiliar as $v) { ?>
        <option value="<?php echo $v->idVendedorAuxiliar; ?>"
            <?php echo ($result->idVendedorAuxiliar == $v->idVendedorAuxiliar) ? 'selected' : ''; ?>>
            <?php echo htmlspecialchars($v->nomeVendedor, ENT_QUOTES, 'UTF-8'); ?>
        </option>
    <?php } ?>
</select>
    ...
    <select class="span12 form-control" name="idGerente">
        ...
        <?php } ?>
											...
											<div class="span3" class="control-group line-through" title="CIF: O fornecedor é resposável pelo custo e seguro do frete e entrega do produto. &#13 FOB: O cliente é resposável pelo custo e seguro do frete e entrega do produto.">
												...
												<select name="entrega" class="span12">
													...
													<?php foreach($dados_tipoentrega as $c){ 
														if($c->idTipoEntrega == $result->entrega){
															echo '<option selected value="'.$c->idTipoEntrega.'">'.$c->descricaoTipoEntrega.'</option>';
														}else{
												...
												<input class="span6" id="entregaoutros" type="hidden" name="entregaoutros" value="<?php echo $result->entregaoutros; ?>" size="50" />
											</div>
											<div class="span3" class="control-group">
										...
										<div class="span12" style="padding: 0.2%; margin-left: 0">
											...
											<div class="span2" class="control-group" >
												<label for="num_pedido" class="control-label">Moeda</label>

												<input id="moeda_cotacao" class="span12" type="text" name="moeda_cotacao" value="<?php echo $result->moedaCotacao; ?>" />
											</div>
											<div class="span2" class="control-group" >
												<label for="num_pedido" class="control-label">Câmbio</label>

												<input id="valor_cotacao" class="span12" onKeyPress="FormataValor2(this,event,12,2);" type="text" name="valor_cotacao" value="<?php echo number_format((float)$result->valorCotacao, 2, ",", "."); ?>" />
											</div>
											<div class="span4" class="control-group" alt="Observações." title="Observações.">
		...
		<div style="text-align:center">
			<a href="#modal-transferir" style="margin-right: 1%" role="button" data-toggle="modal" class="btn btn-warning"><i class="icon-plus icon-white"></i>Transferir Itens</a>
			<a href="#modal-desmembrar" style="margin-right: 1%" role="button" data-toggle="modal" class="btn btn-warning"><i class="icon-plus icon-white"></i>Desmembrar Orç.</a>
		...
		<div class="widget-box">
<div class="widget-title" style="padding: 10px; display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between;">
    <div>
        <span class="icon"><i class="icon-user"></i></span>
        <h5 style="display: inline-block; margin: 0 10px;">Cadastro de Itens</h5>
        <a href="#modal-adicionaritem" role="button" data-toggle="modal" class="btn btn-success btn-small" style="margin-left: 10px;">
            <i class="icon-plus icon-white"></i>Adicionar Itens
        </a>
    </div>
    <div>
...
<div class="widget-content" style="padding: 10px 15px;">
						...
						foreach ($dados_item as $orc_item) {
							...
							if ($orc_item->tipoOrc == "serv") {
								...
								if (!empty($escopo)) {
									...
									$idStatusEscopo = $escopo->idStatusEscopo;
							...
							echo '<td style="/* padding: 50px 5px 50px 5px */ "><div id="origem" class="span12" style="padding: 0.2%; margin-left: 0">';
								echo '<input type="hidden" id="id_orc_item_' . $contador_local_autocomplete . '" name="id_orc_item[]"   value="' . $orc_item->idOrcamento_item . '"/>' .
									'<div class="span12" style="padding: 0.2%; margin-left: 0">'.
										'<div class="span1">' .
											'<label>O.S.</label>' .
											'<input readonly type="text" class="span12" name="idOs2[]" value="' . $orc_item->idOs . '" />'.											
										'</div>';
										//if($orc_item->idOs){
                                echo '<div class="span1">' .
                                    '<label>Un. Ex.</label>' .
                                    '<select class="span12 unex-item" name="unex[]" data-index="'.$contador_local_autocomplete.'">',
                                    '<option value="">Selecione</option>';
                                    foreach($unid_exec as $v){
                                        echo '<option '.($v->id_unid_exec == $orc_item->unid_execucao?"selected":"").' value="'.$v->id_unid_exec.'">'.$v->status_execucao.'</option>';
                                    }
                                    echo '</select>'.
                                ...
                                echo '<div class="span1">' .
                                    ...
                                    '<select class="span12 tipoos-item" name="tipoos[]" data-index="'.$contador_local_autocomplete.'">',
                                    '<option value="">Selecione</option>';
                                    ?>
													<?php foreach ($tipo_os as $ostipo) { ?>

														<option value="<?php echo $ostipo->id_tipo; ?>" <?php if ($ostipo->id_tipo == $orc_item->id_tipo) {
																										echo "selected='selected'";
										...
										'<div class="span3" alt="' . $orc_item->descricao_item . '" title="' . $orc_item->descricao_item . '">' .
											...
											'<input type="text" style="width: 85%" class="span8" id="descricao_item_' . $contador_local_autocomplete . '" name="descricao_item[]"  value="' . $orc_item->descricao_item . '" />' .
											'<input type="button" class="span4" style="width: 13%;margin-left: 2%;" onclick="$(\'#modal_descricao_'.$orc_item->idOrcamento_item.'\').modal(\'show\')">'.
										'</div>' .	
										'<div id="modal_descricao_'.$orc_item->idOrcamento_item.'" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">'.
											'<div class="modal-header">'.
												'<h5 id="myModalLabel">Descrição para o cliente</h5>'.
											'</div>'.
											'<div class="modal-body">'.
												'<div class="span12">'.
													'<label>Descrição:</label>'.
													'<input type="text" class="span12" id="descricaoImprimir_'.$orc_item->idOrcamento_item.'" value="'.(!empty($orc_item->descricao_item_imprimir)?$orc_item->descricao_item_imprimir:$orc_item->descricao_item).'">'.
												'</div>'.
											'</div>'.
											'<div class="modal-footer">'.
												'<button class="btn" data-dismiss="modal" aria-hidden="true">Cancelar</button>'.
												'<a class="btn btn-warning" onclick="salvarDescricaoImprimir('.$orc_item->idOrcamento_item.')">Confirmar</a>'.
											'</div>'.
										'</div>'.
									'</div>';
								...
								if($orc_item->tipoOrc == 'serv'){
									...
									if($orc_item->idStatusPeritagem == 1){
										echo '<td style="text-align: center; /* padding: 75px 5px 75px 5px */ display: table-cell;min-height: 10em;vertical-align: middle;width: 100px;"><a onclick="solicitarPeritagem('.$orcServicoEscopo2->idOrcServicoEscopo.','. $orc_item->idOrcamento_item .','.$contador_local_autocomplete.',this)" role="button" data-toggle="modal" class="btn btn-warning"  class="span12" style="/* margin-right: 10px;*/">Validar desenho</a>';
							...
							echo '<div class="span12" style="padding: 0.2%; margin-left: 0">' .
								...
								'<div class="span1">' .
								'<label>QTD:</label>' .
								'<input type="text" class="span5 number" id="qtd_' . $contador_local_autocomplete . '" name="qtd[]" onblur="calculaSubTotal(' . $contador_local_autocomplete . ');" onclick="this.select();" value="' . $orc_item->qtd . '" />' .
								'<select class="span7" id="tipoqtd_' . $contador_local_autocomplete . '" name="tipoqtd[]">';
								foreach ($dados_tipoqtd as $v) {
									echo '<option '.($v->idTipoQtd == $orc_item->idTipoQtd?"selected":"").' value="'.$v->idTipoQtd.'">'.$v->descricaoTipoQtd.'</option>';
								}
								echo '</select>'.
								'</div>' .
								'<div class="span1">' .
							...
							if ($orc_item->tipoOrc == 'serv') {
								echo '<div class="span12" style="padding: 0.2%; margin-left: 0">';
								echo '<div class="span2" "style="margin-top: 20px;">' .
									'<label>Status Peritagem</label>' .
									'<input class="span12" type="text" id="statusPeritagem_' . $contador_local_autocomplete . '" readonly value="' . $orc_item->descricaoPeritagem . '">';
									echo '</div>';
								echo '<div class="span2" "style="margin-top: 20px;">' .
									'<label>Status Desenho</label>' .
									'<input class="span12" type="text" id="statusDesenho_' . $contador_local_autocomplete . '" readonly value="' . ($orc_item->statusDesenho == null || $orc_item->statusDesenho == 0 ?"Não Solicitado":($orc_item->statusDesenho == 1?"Sem Desenho":($orc_item->statusDesenho == 2?"Incompleto":"Completo"))) . '">';
							...
							if (!empty($escopo)) {
								...
								echo '</table>'.
								'<div>' .
								//'<a href="#" onclick="adicionarItemCatalogo(' . $contador_local_autocomplete . ');" class="btn btn-success"><i class="icon-plus icon-white"></i> Adicionar Item </a>' .
								'</div>';
							}else{
								echo '<h5>Itens</h5>';
								echo '<table class="table table-bordered " id="tableEscopo_' . $contador_local_autocomplete . '">' .
	...
	<div id="modal-desmembrar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<form action="<?php echo base_url() ?>index.php/orcamentos/desmembrarorcamento" method="post" id="formDesmembrar">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">

			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<h5 id="myModalLabel">Desmembrar Orçamento <b id="bLiberarPCP"><?php echo $result->idOrcamentos;?></b></h5>
				<input type="hidden" value="<?php echo $result->idOrcamentos; ?>" name="idOrcDesmembrar">
			</div>
			<div class="modal-body">
				<h5 style="text-align: center">Selecione os itens que serão destinados ao novo orçamento.</h5>
				<table class="table table-bordered ">
					<thead>
						<tr>
							<th></td>
					...
					<tbody>
						<?php 
							foreach($dados_item as $orc_item){
								echo '<tr>';
									echo '<td style="text-align:center"><input type="checkbox" name="checkDesmembrar[]" value="'.$orc_item->idOrcamento_item.'"></td>';
									echo '<td>'.$orc_item->idOs.'</td>';
									echo '<td>'.$orc_item->pn.'</td>';
									echo '<td>'.$orc_item->descricao_item.'</td>';
									echo '<td>'.$orc_item->qtd.'</td>';
								echo '</tr>';
							}
						?>
					</tbody>
	...
	<div id="modal-transferir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<form action="<?php echo base_url() ?>index.php/orcamentos/transferiritens" method="post" id="formTransferir">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">

			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<h5 id="myModalLabel">Transferir Itens <b id="bLiberarPCP"><?php echo $result->idOrcamentos;?></b></h5>
				<input type="hidden" value="<?php echo $result->idOrcamentos; ?>" name="idOrcTransferir">
			</div>
			<div class="modal-body">
				<h5 style="text-align: center">Informe o número do orçamento de destino e selecione os itens que serão transferidos.</h5>
				<div class="span12" style="margin-left:0px;margin-bottom:10px">
					<div class="span3">
						<label>Orç. destino:</label>
						<input class="span12 number" type="text" name="idOrcTransferirDestino" id="idOrcTransferirDestino">
					</div>
				</div>
				<table class="table table-bordered ">
					<thead>
					...
					<tbody>
						<?php 
							foreach($dados_item as $orc_item){
								echo '<tr>';
									echo '<td style="text-align:center"><input type="checkbox" name="checkTransferir[]" value="'.$orc_item->idOrcamento_item.'"></td>';
									echo '<td>'.$orc_item->idOs.'</td>';
									echo '<td>'.$orc_item->pn.'</td>';
									echo '<td>'.$orc_item->descricao_item.'</td>';
									echo '<td>'.$orc_item->qtd.'</td>';
	   ...
	   $(document).on('click', 'a', function(event) {
		...
		function calculaSubTotal(x) {
			...
			for (i = 0; i < contador_global_autocomplete; i++) {
				...
				$('#val_ipi_rs_' + i).val(Formata_Dinheiro(total2));
	...
	<script type="text/javascript">
		...
		*/
		...
		function solicitarPeritagem(idOrcEscopo,idOrcItem,pos,item){
			$.ajax({
				...
				success: function(data) {
					...
					if(data.result){
						$(item).remove();
						$("#statusPeritagem_"+pos).val("AGUARDANDO PERITAGEM");
						$("#statusDesenho_"+pos).val("Sem Desenho");
		...
		function salvarDescricaoImprimir(idOrcamentoItem){
			$.ajax({
				url: "<?php echo base_url(); ?>index.php/orcamentos/editarDescricaoImprimir",
				dataType: 'json',
				type: 'POST',
				async:false,
				data:{
					descricao: $("#descricaoImprimir_"+idOrcamentoItem).val(),
					idOrcItem: idOrcamentoItem,
				},
				success:function(data2){
					alert("Descrição alterada com sucesso");
					$("#modal_descricao_"+idOrcamentoItem).modal("hide");
				},
				error: function(xhr, textStatus, error) {
					console.log(xhr.responseText);
					console.log(xhr.statusText);
...
$(document).ready(function() {
    var idVendedorAuxiliarSelecionado = "<?php echo $result->idVendedorAuxiliar; ?>";
    var idVendedorSelecionado = "<?php echo $result->idVendedor; ?>";

    function carregarVendedoresAuxiliares(idVendedor) {
        if (!idVendedor) return; // Se não houver vendedor, não executa o AJAX
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(index, item) {
                    var selected = (item.idVendedorAuxiliar == idVendedorAuxiliarSelecionado) ? 'selected' : '';
                    options += '<option value="' + item.idVendedorAuxiliar + '" ' + selected + '>' + item.nomeVendedorAuxiliar + '</option>';
                });
    ...
    $('#idVendedor').change(function() {
        ...
        carregarVendedoresAuxiliares(idVendedor);
    });

    // 🚀 FORÇAR O CARREGAMENTO AUTOMÁTICO APENAS SE JÁ HOUVER UM VENDEDOR AUXILIAR SALVO
    if (idVendedorSelecionado) {
        carregarVendedoresAuxiliares(idVendedorSelecionado);
    }
});
...
document.getElementById('formOrcamento').addEventListener('submit', function(event) {
    event.preventDefault(); // Previne o envio automático do formulário para evitar envios duplicados

    var form = this;
    var checkbox = document.getElementById('enviado_cliente_checkbox');
    var enviadoCliente = checkbox.checked ? 1 : 0;

    // Remover campo anterior se já existir
    var existingField = document.querySelector('input[name="enviado_cliente"]');
    if (existingField) {
        existingField.remove();
    }
    
    if (<?php echo ($result->enviado_cliente == 0) || (is_null($result->data_envio)) ? 'true' : 'false'; ?> || checkbox.checked) {
        var enviadoClienteInput = document.createElement('input');
        enviadoClienteInput.type = 'hidden';
        enviadoClienteInput.name = 'enviado_cliente';
        enviadoClienteInput.value = enviadoCliente;
        form.appendChild(enviadoClienteInput);

        // Se o checkbox for marcado, adicione a data atual
        if (checkbox.checked) {
            var dataEnvioInput = document.createElement('input');
            dataEnvioInput.type = 'hidden';
            dataEnvioInput.name = 'data_envio';
            dataEnvioInput.value = new Date().toISOString().slice(0, 19).replace('T', ' ');
            form.appendChild(dataEnvioInput);
        }
    }

    // Remover campo de região antes de enviar
    var regiaoField = document.getElementById('regiao');
    if (regiaoField) {
        regiaoField.removeAttribute
		</script>
<script>
$(document).ready(function () {
