<div class="row-fluid" style="margin-top:0">
    ...
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                    ...
                    <div class="control-group">
                        <label for="user" class="control-label">Usuario<span class="required">*</span></label>
                        <div class="controls">
                            <input id="user" type="text" name="user" value="<?php echo $result->user; ?>"  />
                        </div>
                    </div>
                    <div class="control-group">
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <?php foreach ($dados_emitente as $p) {
                                $verificar = false;
                                foreach($empresas_user as $d){
                                    if($p->id == $d->idEmpresa){
                                        $verificar = true;
                                    }
                                }
                                echo '<div class="control-group"><label  class="control-label"  style="width: 15px; padding-top: 10px;"><input id="idempresa" '.($verificar?"checked":"").'  type="checkbox" name="idempresa[]" value="'.$p->id.'"  /></label>';
                                echo '<div class="controls" style="margin-left: 25px;">'.$p->nome.'</div></div>';
                            } ?>
                        </div>
                    </div>
                    <div class="control-group">
                        <label for="Empresa"  class="control-label">Departamento</label>
                        <div class="controls">
                            <?php foreach ($dados_departamento as $p) {
                                $verificar = false;
                                foreach($departamento_user as $d){
                                    if($p->idAlmoEstoqueDep == $d->idDepartamento){
                                        $verificar = true;
                                    }
                                }
                                echo '<div class="control-group"><label  class="control-label"  style="width: 15px; padding-top: 10px;"><input id="iddepartamento" '.($verificar?"checked":"").'  type="checkbox" name="iddepartamento[]" value="'.$p->idAlmoEstoqueDep.'"  /></label>';
                                echo '<div class="controls" style="margin-left: 25px;">'.$p->descricaoDepartamento.'</div></div>';
                            } ?>
                        </div>
                    </div>
