...
?>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="tab1">
                                ...
                                <div class="span12" id="divCadastrarOs">
                                    ...
                                    <form class="form-inline" action="<?php echo base_url() ?>index.php/os/ordemservico_pcp" method="post" name="form8" id="form8">
    ...
    <div class="row-fluid" style="margin-top:0">
        <div class="span12" style="width: 100%; overflow-y: scroll;">
            <div class="widget-box">
                <div class="widget-title">
                    <span class="icon">
                        <i class="icon-tags"></i>
...
echo '<td>'; // Coluna da Produção (F)
if (!empty($r->data_producao)) {
    // Se a data de produção existe, exibe ela em um span com um identificador
    $data_producao_formatada = date("d/m/Y", strtotime($r->data_producao));
    echo '<span class="data-producao-display" data-idos="' . $r->idOs . '">' . $data_producao_formatada . '</font></span>';
} else {
    // Se não existe, exibe o formulário. Usamos classes e um data-id para identificação.
    echo '
    <form action="' . base_url('index.php/os/dataproducaoPCP') . '" class="form-dataproducao-ajax" method="post" data-idos="' . $r->idOs . '">
        <input type="hidden" name="idOs" value="' . $r->idOs . '"/>
        <input type="date" name="data_producao" class="input-data-producao span12" required />
        <button type="submit" class="btn btn-mini btn-primary">Salvar</button>
    </form>';
}
echo '</td>'; // Fechamento da tag <td>
                                            ...
                                            $vale= $this->almoxarifado_model->getOnlyValeByOsOrigem($r->idOs);
                                            if(!empty($vale)){
                                        ...
                                        <?php
                                            ...
                                            echo '<td><font size="1">';
                                            //if($r->statusDesenho == 3){
                                                if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vDistribuirOs') || $this->permission->checkPermission($this->session->userdata('permissao'), 'eDistribuirOs')) {
    ...
    <div class="widget-box" style="overflow:auto;">
        ...
        <div class="widget-content nopadding">
            ...
            <table class="comBordas" id="tableOs">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan='7' align='right'>
                            <?php echo "Somatória: " . number_format($valor, 2, ",", "."); ?>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<div id="modal-alterarOS" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<div id="modal-confirmacao" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="position:absolute;">
    ...
    <div class="modal-body">
        Serão alteradas as seguintes O.S.: <input type="text" id="confirmOS" disabled/>
    </div>
    <div class="modal-footer">
        <button class="btn" data-dismiss="modal" aria-hidden="true">Cancelar</button>
...
<script>
    ...
    function confirmacaoAlterar(){
        ...
        document.querySelector('#confirmOS').value = input;
        if(input == null || input == ''){
...
<script>
    /*document.getElementById("dataproducaoPCP").addEventListener("submit", function(e) {
        console.log(document.getElementById("data_producao").value);
    });*/
	
	$(document).ready(function() {
    // ... suas funções PrintElem, Popup, exportTableToCSV e handlers existentes ...

    // Listener para submissão dos formulários de data de produção
    $(document).on('submit', '.form-dataproducao', function(e) {
        e.preventDefault(); // Impede o comportamento padrão de recarregar a página

        var form = $(this);
        var idOs = form.data('idos'); // Pega o idOs do atributo data-idos do formulário
        var dataProducaoInput = form.find('.input-data-producao');
        var dataProducao = dataProducaoInput.val();
        ...
        $.ajax({
            url: form.attr('action'), // Pega a URL de action do formulário
            type: form.attr('method'), // Pega o método do formulário (post)
            data: {
                idOs: idOs,
            ...
            success: function(response) {
                if (response.status === 'success') {
                    // Encontra a célula <td> que contém este formulário
                    var td = form.closest('td');
                    // Substitui o formulário pela data formatada
                    td.html('<span class="data-producao-exibida" data-idos="' + response.idOs + '">' + response.data_producao_formatada + '</span>');
                    // Opcional: mostrar uma mensagem de sucesso (ex: com noty/toastr se você tiver)
                    // alert(response.message);
                } else {
                    alert('Erro: ' + response.message);
                    submitButton.prop('disabled', false).text('Salvar'); // Reabilita em caso de erro
                }
            },
...
<script>
$(document).ready(function() {
    // --- Funções de Impressão e Exportação (Mantidas como já ajustamos) ---

    // Exemplo:
    // function PrintElem(elemSelector) { /* ... */ }
    // function Popup(data) { /* ... */ }
    // function exportTableToCSV($table, filename, type) { /* ... */ }

    // Handlers para os botões de impressão e exportação (da sua implementação anterior)
    // Exemplo:
    // $("#imprimir").click(function(e) { e.preventDefault(); PrintElem('#printOs'); });
    // $(".export-csv").on('click', function(event) { /* ... */ });
    // $("#imprimirRelTable").click(function(e) { e.preventDefault(); PrintElem('#printRelTable'); });
    // $(".export-csv-rel").on('click', function(event) { /* ... */ });

    // --- NOVA LÓGICA PARA ATUALIZAÇÃO DA DATA DE PRODUÇÃO VIA AJAX ---
    $(document).on('submit', '.form-dataproducao-ajax', function(e) {
        e.preventDefault(); // Impede o envio padrão do formulário (que recarregaria a página)

        var $form = $(this); // O formulário que foi submetido
        var idOs = $form.data('idos'); // Pega o idOs do atributo data-idos do formulário
        var dataProducao = $form.find('.input-data-producao').val(); // Pega o valor do input de data
        ...
        var $submitButton = $form.find('button[type="submit"]');
        $submitButton.prop('disabled', true).text('Salvando...'); // Desabilita o botão para evitar cliques múltiplos

        $.ajax({
            url: $form.attr('action'), // Pega a URL do atributo 'action' do formulário
            type: $form.attr('method'), // Pega o método ('post') do formulário
            dataType: 'json', // Espera uma resposta JSON do servidor
            data: {
                idOs: idOs,
                data_producao: dataProducao
            ...
            success: function(response) {
                if (response.status === 'success') {
                    // Encontra a célula <td> pai do formulário
                    var $td = $form.closest('td');
                    // Substitui o formulário pela data formatada recebida do PHP
                    $td.html('<font size="1"><span class="data-producao-display" data-idos="' + response.idOs + '">' + response.data_producao_formatada + '</span></font>');
                    ...
                    alert('Erro: ' + response.message);
                    $submitButton.prop('disabled', false).text('Salvar'); // Reabilita o botão
                }
            },
            error: function(xhr, status, error) {
                // Em caso de erro na requisição (rede, servidor, etc.)
    ...
    // ... Seus outros scripts, como o autocomplete de PN
    // $("#pn").autocomplete({ ... });
});
