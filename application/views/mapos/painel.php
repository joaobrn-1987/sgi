      ...
      <ul class="quick-actions">
       <!-- <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vCliente')){ ?>
		...
		<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vFornecedor')){ ?>
            <li class="bg_am"> <a href="<?php echo base_url()?>index.php/fornecedores"> <i class="icon-group"></i> Fornecedor</a> </li>
        <?php } ?>
        <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vProduto')){ ?>
            <li class="bg_lg"> <a href="<?php echo base_url()?>index.php/produtos"> <i class="icon-barcode"></i> PN</a> </li>
        <?php } ?>-->
		<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vOrcamento')){ ?>
            <li class="bg_ls"> <a href="<?php echo base_url()?>index.php/orcamentos"><i class="icon-fixed-width icon-book"></i> Orçamento</a></li>
        <?php } ?>
		<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vOs')){ ?>
            <li class="bg_lo"> <a href="<?php echo base_url()?>index.php/os"> <i class="icon-fixed-width icon-cogs"></i> Ordem de Serviço</a> </li>
        <?php } ?>
		<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vOs')){ ?>
            <li class="bg_lo2"> <a href="<?php echo base_url()?>index.php/os/carteiraservico"> <i class="icon-tasks"></i> Carteira de Serviços</a> </li>
        <?php } ?>
		<?php if($this->session->userdata('permissao') <> 6) { ?>
            <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vCotacao')){ ?>
                <li class="bg_am"> <a href="<?php echo base_url()?>index.php/suprimentos"> <i class="icon icon-paste"></i> Compra Solicitada</a> </li> 
            <?php } ?>
            <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vPedCompra')){ ?>
                <li class="bg_lg"> <a href="<?php echo base_url()?>index.php/suprimentos"> <i class="icon icon-file"></i> Gerar Ordem de Compra</a> </li>
            <?php } ?>
        <?php } ?>
        
		<!--
        <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vInsumo')){ ?>
            <li class="bg_lb"> <a href="<?php echo base_url()?>index.php/insumos"> <i class="icon-shopping-cart"></i> Insumos</a> </li>
        <?php } ?>-->
		<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vAlmoxarifado')){ ?>
            <li class="bg_lg2"> <a href="<?php echo base_url()?>index.php/almoxarifado"> <i class="icon-tags"></i> Almoxarifado</a> </li>
        <?php } ?>
		<!--<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vPedidocompraalmox')){ ?>
            <li class="bg_lg2"> <a href="<?php echo base_url()?>index.php/pedidocompraalmox"> <i class="icon-truck"></i> Almoxarifado P.C.</a> </li>
        <?php } ?>
        <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vMaquina')){ ?>
            <li class="bg_ly"> <a href="<?php echo base_url()?>index.php/maquinas"> <i class="icon-wrench"></i> Maquinas</a> </li>
        <?php } ?>
		<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'vMaquinausuario')){ ?>
            <li class="bg_ly"> <a href="<?php echo base_url()?>index.php/maquinasusuarios"> <i class="icon-wrench"></i> Maquinas Usuarios</a> </li>
        <?php } ?>-->
		<li class="bg_lb"> <a href="https://www.ubertec.ind.br/apresentacao-UBERTEC-09-06-2025.pptx"><i class="icon-folder-open "></i>Apresentação Ubertec 2025</a> </li>
...
<div class="span12" style="margin-left: 0">
        <?php if($this->permission->checkPermission($this->session->userdata('permissao'),'fComercial')){ ?>
            <div class="widget-box">
                <div class="widget-title"><span class="icon"><i class="icon-signal"></i></span><h5>O.S. Reagendadas</h5></div>
                <div class="widget-content" style="height:300px; overflow:auto;">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <?php foreach ($alteracoes as $r) {
                            echo '<tr>';
                            echo '<td>';
                            echo $r->idOs;
                            echo '</td>';
                            echo '<td>';
                            $date = new DateTime($r->data_reagendada);
                            echo $date->format('d/m/Y');
                            echo '</td>';
                            echo '<td>';
                            $date = new DateTime($r->data_anterior);
                            echo $date->format('d/m/Y');
                            echo '</td>';
                            echo '<td>';
                            echo $r->nomeUser;
                            echo '</td>';
                            echo '<td>';
                            $date = new DateTime($r->data_ocorrencia);
