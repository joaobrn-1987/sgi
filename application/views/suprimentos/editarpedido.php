...
?>
    ...
    <body onLoad="calculaSubTotal();">
        ...
        <div class="widget-box">
            ...
            <div>
                ...
                <form action="<?php echo base_url() ?>index.php/suprimentos/salvaritemcompra" method="post">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr>
                            ...
                            <th>Status<?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePedCompra')) { ?><a href="#modal-ipi" style="margin-right: 1%" role="button" data-toggle="modal" title="Editar IPI">
                                    <font color='red'><i class="icon-pencil icon-white"></i></font>
                                </a><?php } ?></th>
                            <th>Data <br>Entregue <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePedCompra')) { ?><a href="#modal-ipi" style="margin-right: 1%" role="button" data-toggle="modal" title="Editar IPI">
                                        <font color='red'><i class="icon-pencil icon-white"></i></font>
                                    </a> <?php } ?></th>
                            <th>Grupo</th>
                            <th width='70'>QTD<br>recebida</th>
                            <th><b>Verif. <br> Estq. <br>/ Res.</b></th>
                            <th>Valor Unit.</th>
                            <th width='40'>IPI% <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePedCompra')) { ?><a href="#modal-ipi" style="margin-right: 1%" role="button" data-toggle="modal" title="Editar IPI">
                                        <font color='red'><i class="icon-pencil icon-white"></i></font>
                                    </a> <?php } ?></th>
                            <th>ICMS</th>
                            <th>Valor <br>total</th>

                            <th>N° NF <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePedCompra')) { ?><a href="#modal-ipi" style="margin-right: 1%" role="button" data-toggle="modal" title="Editar IPI">
                                        <font color='red'><i class="icon-pencil icon-white"></i></font>
                                    </a> <?php } ?></th>
                        ...
                        foreach ($results as $r) {
                            ...
                            $anexoImagens3 = $this->pedidocompra_model->getAnexoCotacaoSuprimentosByIdPedidoCompra($r->idPedidoCompra);
                            $color = '';
                        ...
                        ?>
                            ...
                            // Se mudou o ID ou a Previsão, imprime o cabeçalho (Botão do Modal)
                            if ($idPedidoCompra != $r->idPedidoCompra || $previsaoEntrega != $r->previsao_entrega) {
                                ...
                                // ======================================================================
                                // FIX DE PRODUÇÃO: LIMPEZA DE ID PARA O BOTÃO
                                // Remove espaços, traços e dois pontos da data para criar um ID válido
                                // ======================================================================
                                $data_limpa_link = preg_replace('/[^0-9]/', '', $r->previsao_entrega); 
                                $id_modal_link = "modal_oc_" . $r->idPedidoCompra . "_" . $data_limpa_link;
                            ?>

                                <tr>
                                    <td style="width:100%; border:1px solid #EEEEEE" class="table-bordered" COLSPAN="13">
                                        <a href="#<?php echo $id_modal_link; ?>" data-toggle="modal">
                                            <?php echo ' <b>OC:</b> ' . $r->idPedidoCompra . ' <b>Empresa:</b> ' . $nomeEmitente . '/ <b>Fornecedor:</b> ' . $nomeFornecedor . ' <b>Previsão de entrega:</b> ' . $r->previsao_entrega; ?>
                                        </a>
                                        
                                        <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePedCompra')) { ?>
                                            <a href="#<?php echo $id_modal_link; ?>" style="margin-right: 1%" role="button" data-toggle="modal" title="Editar">
                                                <font color='red'><i class="icon-pencil icon-white"></i></font>
                                            </a>
                                        <?php } ?>
                                        
                                        <a href="#modal-anexar-cotacoes<?php echo $r->idPedidoCompra; ?>" data-toggle="modal" class="btn btn-success"><i class="icon-plus icon-white"></i> Anexar Cotações</a>
                                        <?php

                                        if (!empty($anexoImagens3)) {
                                            if (count($anexoImagens3) < 2 && $r->sem_cotacao == 0) {
                                                echo '<p><b style="color:red">A alteração do status e dos campos serão liberados após a anexação de duas ou mais cotações</b></p>';
                                            }
                                        } else {
                                            if ($r->sem_cotacao == 0) {
                                                echo '<p><b style="color:red">A alteração do status e dos campos serão liberados após a anexação de duas ou mais cotações</b></p>';
                            ...
                            <tr style="color:<?php echo $color; ?>" class="table-bordered">
                                ...
                                <td width='10'>
                                    <font size='1'><?php echo $r->cod_fornecedor; ?></font>
                                </td>
                                <td>
                                    <font size='1'><?php
                                ...
                                <td>
                                    <input type="hidden" id="idstatus_<?php echo $contador; ?>" value="<?php echo $r->idStatuscompras; ?>" size="1" />
                                    <input type="hidden" id="aprovadoPCP_<?php echo $contador; ?>" value="<?php echo $r->aprovacaoPCP; ?>" size="1" />
                                    <input type="hidden" id="aprovacaoSUP_<?php echo $contador; ?>" value="<?php echo $r->aprovacaoSUP; ?>" size="1" />
                                    <input type="hidden" id="autorizadoCompra_<?php echo $contador; ?>" value="<?php echo $r->autorizadoCompra; ?>" size="1" />
                                    <input type="hidden" id="aprovacaoDir_<?php echo $contador; ?>" value="<?php echo $r->aprovacaoDir; ?>" size="1" />
                                    <input type="hidden" id="idOs_<?php echo $contador; ?>" value="<?php echo $r->idOs; ?>" size="1" />
                                    <select style="<?php echo ((empty($anexoImagens3) || count($anexoImagens3) < 2)  && $r->idStatuscompras == 3 && $r->sem_cotacao == 0 ? "pointer-events:none" : "") ?>" class="recebe-solici" class="controls" style="font-size: 10px;" name="idStatuscompras[]" id="idStatuscompras<?php echo $contador; ?>">

                                        <?php foreach ($dados_statuscompra as $so) { ?>

                                            <?php
                                            echo '<script>console.log("Etapa: ' . $r->etapa . ' Aprovacao PCP: ' . $r->aprovacaoPCP . ' Aprovacao FIN: ' . $r->aprovacaoSUP . ' Aprovacao Dir: ' . $r->autorizadoCompra . ' Aprovacao DirTec: ' . $r->aprovacaoDir . ' IDDistribuir ' . $r->idDistribuir . '")</script>';
                                            if ($r->idOs <= 19999) {

                                                if ($r->autorizadoCompra == 1 && $r->aprovacaoPCP == 1 && $r->aprovacaoSUP == 1 && $r->aprovacaoDir == 1) {
                                            ?>
                                                    ...
                                                    if ($r->etapa <= 3 && ($r->autorizadoCompra == 0 || $r->aprovacaoPCP == 0 || $r->aprovacaoSUP == 0 || $r->aprovacaoDir == 0)) {
                                                        if ($so->etapa <= 3 && $so->etapa != 0) {
                                                    ?>
                                                            <option value="<?php echo $so->idStatuscompras; ?>" <?php if ($so->idStatuscompras == $r->idStatuscompras) {
                                                                                                                    echo "selected='selected'";
                                                                                                                } else if ($r->etapa == 3 && $so->etapa == 3) {
                                                                                                                    echo "disabled";
                                                                                                                } ?>>
                                                                <?php echo $so->nomeStatus; ?>
                                                            </option>
                                ...
                                <td>
                                    <font size='1'>
                                        ...
                                        ?>
                                            ...
                                            <input <?php echo ((empty($anexoImagens3) || count($anexoImagens3) < 2)  && $r->idStatuscompras == 3 && $r->sem_cotacao == 0 ? "readonly" : "") ?> id="dataentregue" class="data span14" type="text" name="dataentregue[]" value="" style="font-size: 10px;" size='17' />
                                        <?php
                                        }
                                        ?>
                                    </font>
                                ...
                                ?>
                                <td><input <?php echo ((empty($anexoImagens3) || count($anexoImagens3) < 2)  && $r->idStatuscompras == 3 && $r->sem_cotacao == 0 ? "readonly" : "") ?> class="span6" onclick="this.select();" type="text" style="font-size: 10px;" name="qtdrecebida[]" value="<?php if ($r->qtdrecebida) {
                                                                                                                                                                                                                                                                                                        echo $r->qtdrecebida;
                                                                                                                                                                                                                                                                                                    } else {
                                                                                                                                                                                                                                                                                                        echo $r->quantidade;
                                                                                                                                                                                                                                                                                                    } ?>">
                                ...
                                <td>
                                    <?php
                                    if (!empty($estoque2)) { ?>
                                        <a style="position: absolute;" href="#modal-estoque-<?php echo $r->idPedidoCompraItens; ?>" style="margin-right: 1%" role="button" data-toggle="modal" class="btn tip-top"><i class="icon-eye-open"></i></a>
                                    <?php
                                    } ?>
                                </td>
                                <td>
                                    <input <?php if ($r->etapa == 3 || ($r->etapa == 4 && $r->idStatuscompras != 5)) {
                                                echo 'readonly';
                                            } ?> <?php echo ((empty($anexoImagens3) || count($anexoImagens3) < 2)  && $r->idStatuscompras == 3 && $r->sem_cotacao == 0 ? "readonly" : "") ?> style="font-size: 10px;width: 80px;" class="span12" onclick="this.select();" type="text" onkeypress="alterarStatus(<?php echo $contador; ?>)" id="valor_unitario<?php echo $contador; ?>" name="valor_unitario[]" 'onKeyPress="FormataValor2(this,event,10,3);"
                        ...
                        <td>
                            <input style="font-size: 10px;" class="span12" onclick="this.select();" type="text"
                                ...
                                onKeyUp="calculaSubTotal(<?php echo $contador; ?>);" <?php echo ((empty($anexoImagens3) || count($anexoImagens3) < 2)  && $r->idStatuscompras == 3 && $r->sem_cotacao == 0 ? "readonly" : "") ?>>
                        </td>
                        <td style="text-align: center"> 
                            
                            <input style="font-size: 10px;width: 40px;" class="span12" onclick="this.select();" type="text"
                                id="valor_icms<?php echo $contador; ?>" onKeyPress="FormataValor2(this,event,10,2);"
                                name="valor_icms[]" onKeyUp="calculaSubTotal(<?php echo $contador; ?>);" <?php echo ((empty($anexoImagens3) || count($anexoImagens3) < 2)  && $r->idStatuscompras == 3 && $r->sem_cotacao == 0 ? "readonly" : "") ?> value="<?php if ($r->porcentagem) {
                                                                                                                                                                                                                                                                        echo number_format($r->icmsPorcentagem, 2, ",", ".");
                                                                                                                                                                                                                                                                    } else {
                                                                                                                                                                                                                                                                        echo number_format($r->icms, 2, ",", ".");
                                                                                                                                                                                                                                                                    } ?> "
                                >
                                <br>
                                <input type="checkbox" <?php if ($r->porcentagem) {
                                                            echo 'checked';
                                                        } ?> id="checkPercentagem<?php echo $contador; ?>" name="checkPercentagem[]" value="<?php echo $r->idPedidoCompraItens; ?>"style="z-index:999"> %
                        </td>
                        <td> <?php
                                ...
                                ?>
                            <input style="font-size: 10px;width: 80px;" type="text" id="valor_produtos<?php echo $contador; ?>"
                                name="valor_produtos[]"
                    ...
                    <?php
                    // ======================================================================
                    // CORREÇÃO CRÍTICA (FIX DE PRODUÇÃO):
                    // 1. Array de controle para evitar duplicação de modais
                    // 2. Limpeza do ID do modal para garantir que o link funcione (Sem espaços na data)
                    // ======================================================================
                    
                    $modais_processados = array(); // Array para controlar duplicidade
                    $contadorOS = 0;
                    
                    foreach ($results as $r) { 
                        // Cria chave única
                        $chave_unica = $r->idPedidoCompra . '_' . $r->previsao_entrega;
                        
                        // Verifica se já processou essa OC. Se não, entra no IF.
                        if (!in_array($chave_unica, $modais_processados)) {
                            
                            $modais_processados[] = $chave_unica; // Adiciona a chave ao array para não repetir
                            $contadorOS++;
                            
                            // Mantém a lógica do ID único para os campos para o Javascript
                            $uniqID = $r->idPedidoCompra . "_" . $contadorOS;
                            
                            // FIX DE ID: Cria o ID do HTML do modal sem espaços ou caracteres especiais
                            $data_limpa_modal = preg_replace('/[^0-9]/', '', $r->previsao_entrega); 
                            $id_modal_html = "modal_oc_" . $r->idPedidoCompra . "_" . $data_limpa_modal;
                        ?>
    ...
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                            ...
                            <div id="<?php echo $id_modal_html; ?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                    <h5 id="myModalLabel">Dados Orçamento (OC: <?php echo $r->idPedidoCompra; ?>)</h5> 
                                    <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePedCompra')) { ?>
                                        <a href="#modalCadastrarFornecedor" role="button" data-toggle="modal" onclick="openmodal('<?php echo $id_modal_html; ?>')" class="btn btn-success" style="height: 20px"><i class="icon-plus icon-white"></i> Cadastrar Fornecedor</a>
                                    <?php } ?>
                                </div>
                                <div class="modal-body">
                                    <div style="size:20px">
                                        ...
                                        <?php
                                        // IMPORTANTE: Iterar novamente para pegar APENAS os itens desta OC específica
                                        // A verificação extra da data ($rs->previsao_entrega) garante que itens de datas diferentes não se misturem
                                        foreach ($results as $rs) {
                                            if($rs->idPedidoCompra == $r->idPedidoCompra && $rs->previsao_entrega == $r->previsao_entrega){
                                        ?>
                                            <input type="hidden" name="idDistribuir_[]" value="<?php echo $rs->idDistribuir; ?>" />
                                            <input type="hidden" name="idPedidoCompraItens[]" value="<?php echo $rs->idPedidoCompraItens; ?>" />
                                        <?php
                                            }
                                        }
                                        ?>
                                        ...
                                        ?>
                                            <input id="emitente_<?php echo $uniqID; ?>" onclick="this.select();" class="span12 controls emitente" type="text" name="emitente" value="<?php echo $r->nome; ?>" size='50' />
                                            <input id="emitente_id_<?php echo $uniqID; ?>" type="hidden" name="emitente_id" class="emitente_id" value="<?php echo $r->idEmitente; ?>" />

                                        <?php
                                        } else {
                                        ?>

                                            <input id="emitente_<?php echo $uniqID; ?>" class="span12 emitente controls" type="text" name="emitente" value="" size='50' />
                                            <input id="emitente_id_<?php echo $uniqID; ?>" type="hidden" name="emitente_id" value="" class="emitente_id" />
                                        <?php
                                        }
                                        ?>
                                        ...
                                        ?>
                                            <input id="fornecedor_txt_<?php echo $uniqID; ?>" onclick="this.select();" class="span12 controls fornecedor" type="text" name="fornecedor" value="<?php echo $r->nomeFornecedor; ?>" size='50' />
                                            <input id="fornecedor_id_<?php echo $uniqID; ?>" type="hidden" name="fornecedor_id" class="fornecedor_id" value="<?php echo $r->idFornecedores; ?>" />

                                        <?php
                                        } else {
                                        ?>

                                            <input id="fornecedor_txt_<?php echo $uniqID; ?>" class="span12 fornecedor controls" type="text" name="fornecedor" value="" size='50' />
                                            <input id="fornecedor_id_<?php echo $uniqID; ?>" type="hidden" name="fornecedor_id" value="" class="fornecedor_id" />
                                    ...
                                    <table>
                                        <tr>
                                            <td> <input id="idPedidoCompraipi" type="hidden" name="idPedidoCompraipi" value="<?php echo $results[0]->idPedidoCompra; ?>" />
                                                Previsão de entrega:<input size='6' id="previsao_entrega_<?php echo $uniqID; ?>" class=" data" type="text" name="previsao_entrega" value="<?php if (!empty($r->previsao_entrega)) {
                                            ...
                                            <td>Pagamento:
                                                <select class=" recebe-solici" class=" controls" name="idCondPgto" id="idCondPgto_<?php echo $uniqID; ?>">
                                        ...
                                        <tr>
                                            <td colspan='6'>OBS:
                                                <textarea id="obs_<?php echo $uniqID; ?>" rows="5" cols="100" class="" name="obs"><?php echo ($r->obscompras); ?></textarea>
                                            </td>
                                        </tr>
                                    </table>
    ...
    <?php
    foreach ($results as $r) {
        $anexoImagens2 = $this->pedidocompra_model->getAnexoCotacaoSuprimentosByIdPedidoCompra($r->idPedidoCompra);
        ?>
        <?php echo '<div id="modal-anexar-cotacoes' . $r->idPedidoCompra . '" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">' .
            '<div class="modal-header">' .
                '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>' .
                '<h5 id="myModalLabel">Anexar Cotações. O.C.: ' . $r->idPedidoCompra . '</h5>' .
            '</div>' .
            '<form action="' . base_url() . 'index.php/suprimentos/anexarCotacao/' . $r->idPedidoCompra . '" id="formAlterar" enctype="multipart/form-data" method="post" class="form-horizontal">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
' .
            '<div class="modal-body">' .
            '<div class="span12">' . /*
            ...
            '<div class="span3">' .
            '<label> Arquivo </label>' .
            '<input type="file"  name="imag_' . $r->idPedidoCompra . '[]" id="imag_' . $r->idPedidoCompra . '" accept=".pdf,image/png, image/jpeg"  multiple>' .
            '</div>' .
            '<div class=""span3>' .
            '<button type="submit" style="margin-right: 1%" class="btn btn-success" ><i class="fa fa-plus" style="color:white"></i> Salvar Desenho</button>' .
            '</div>' .
            '</div>' .
        ...
        if ($anexoImagens2) {
            echo '<div class="row-fluid" style="margin-top:0">' .
                ...
                '<table class="table table-bordered " id="tableDesenhos_' . $r->idPedidoCompra . '_">' .
                '<thead>' .
                '<tr>' .
                '<th>Arquivo</th>' .
                '<th>Excluir</th>' .
            ...
            foreach ($anexoImagens2 as $anex) {
                ...
                echo '<td><a href="' . base_url() .  $anex->caminho . $anex->imagem . '" style="margin-right: 1%" class="btn tip-top" ><i class="icon-eye-open"></i></a>
                    ...
                    '<td><a onclick="excluirAnexo('.$anex->idAnexo.')" style="margin-right: 1%" role="button"                                                                         
                                                                        class="btn btn-warning tip-top"><i class="icon-remove icon-white"></i></a></td>';
        ...
        foreach ($results as $rs) {
            ...
            echo '<input type="hidden" name="idDistribuir_[]" value="' . $rs->idDistribuir . '" />';
        }
    ...
    <div id="modal-imprimiritem" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <form action="<?php echo base_url() ?>index.php/suprimentos/imprimiritem" method="post">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
    ...
    <div id="modal-ipi" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <form action="<?php echo base_url() ?>index.php/suprimentos/editar_ipi" method="post">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
    ...
    <div id="modal-editar_1" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <form action="<?php echo base_url() ?>index.php/suprimentos/editar_1" method="post">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
    ...
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
            ...
            <div class="modal-body">
                ...
                Você confirma a entrada no estoque desse item?
            </div>
    ...
    <div id="modalremoveranexo" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <form action="<?php echo base_url(); ?>index.php/suprimentos/removeranexocotacao" id="formCadastrar" enctype="multipart/form-data" method="post" class="form-horizontal">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h3 id="myModalLabel">Remover Anexo</h3>
            </div>
            <div class="modal-body">
                <h5>Confirmar a remoção do anexo: <b id="bRemoverAnexo"></b></h5>
                <input type="hidden" id="idAnexoRemover" name="idAnexoRemover">
            </div>
            <div class="modal-footer">
                <button class="btn" data-dismiss="modal" aria-hidden="true" id="btnCancelExcluir">Cancelar</button>
                <button class="btn btn-success" id="saida3">Confirmar</button>
            </div>
        </form>
    </div>
    <script type="text/javascript">
        function alterarStatus(valor){
            ...
            var aprovadoPCP = $("#aprovadoPCP_"+valor).val();
            var aprovadoSUP = $("#aprovacaoSUP_"+valor).val();
            var aprovadoDir = $("#aprovacaoDir_"+valor).val();
            var statusCompra = $("#idstatus_"+valor).val();
            var os = $("#idOs_"+valor).val();
            var autorizadoCompra = $("#autorizadoCompra_"+valor).val();     
            if(statusCompra!=5){
                $("#idStatuscompras"+valor).val(10);
            }         
    ...
    <script type="text/javascript">
        ...
        function cadastrarFornecedor() {
            var nomeFornecedor = document.querySelector(' #nomeFornecedor').value; var documento=document.querySelector('#documento').value; var telefone=document.querySelector('#telefone').value; var celular=document.querySelector('#celular').value; var email=document.querySelector('#email').value; var estado=document.querySelector('#estado').value; var cep=document.querySelector('#cep').value; var cidade=document.querySelector('#cidade').value; var bairro=document.querySelector('#bairro').value; var rua=document.querySelector('#rua').value; var numero=document.querySelector('#numero').value; if(documento=="" || documento==null || nomeFornecedor=="" || nomeFornecedor=="" || cep=="" || cep==null || estado=="" || estado==null || cidade=="" || cidade==null || bairro=="" || bairro==null || rua=="" || rua==null || numero=="" || numero==null){ alert("Verifique os dados."); return; } $.ajax({ url: "<?php echo base_url(); ?>index.php/fornecedores/cadastrarFornecedor" , type: 'POST' , dataType: 'json' , data: { nomeFornecedor: nomeFornecedor, documento: documento, telefone: telefone, celular: celular, email:email, estado:estado, cep:cep, cidade:cidade, bairro:bairro, rua:rua, numero:numero }, success: function(data) { alert(data.msg); if(data.result){ document.querySelector("#fornecedor-"+globalVariable).value=nomeFornecedor.toUpperCase(); document.querySelector("#fornecedor_id-"+globalVariable).value=data.idFornecedor $('#modalCadastrarFornecedor').modal('hide'); $('#modal-fornec-emitente' + globalVariable).modal('show'); } }, error: function(xhr, textStatus, error) { console.log("4"); console.log(xhr.responseText); console.log(xhr.statusText); console.log(textStatus); console.log(error); }, }) } function openmodal(id) { globalVariable=id; $('#modal-fornec-emitente' + id).modal('hide'); $('#modalCadastrarFornecedor').modal('show'); } $(document).ready(function() { $(document).on('click', 'a' , function(event) { var idInsumos_e=$(this).attr('idInsumos_e'); var idPedidoCompra_e=$(this).attr('idPedidoCompra_e'); var idPedidoCompraItens_e=$(this).attr('idPedidoCompraItens_e'); var quantidade_e=$(this).attr('quantidade_e'); var dimensoes_e=$(this).attr('dimensoes_e'); var valor_unitario_e=$(this).attr('valor_unitario_e'); var idEmitente_e=$(this).attr('idEmitente_e'); $('#idInsumos_e1').val(idInsumos_e); $('#idPedidoCompra_e1').val(idPedidoCompra_e); $('#idPedidoCompraItens_e1').val(idPedidoCompraItens_e); $('#quantidade_e1').val(quantidade_e); $('#dimensoes_e1').val(dimensoes_e); $('#valor_unitario_e1').val(valor_unitario_e); $('#idEmitente_e1').val(idEmitente_e); }); }); </script>
                ...
                <script type="text/javascript">
                    ...
                    function calculaSubTotal(x) {
                        ...
                        for (i = 0; i < contador_global_autocomplete; i++) {
                            ...
                            var checkPercentagem = $('#checkPercentagem'+i).is(":checked");
                            valor_icms = valor_icms.toString().replace(".", "");
                            valor_icms = valor_icms.toString().replace(",", ".");
                            if(checkPercentagem){
                                valor_icms = (valorunit*qtd)*(valor_icms/100);
                            }
                        ...
                        for (i2 = 1; i2 <= contador_os; i2++) {
                            ...
                            desconto_ = (desconto_?desconto_:"0,00");
                    ...
                    function retorna_formatado(num) {
                        ...
                        for (var i = 3; i < Math.floor((num.length - (1 + i)) / 3); i++)
                ...
                <script type="text/javascript">
                    function excluirAnexo(idAnexo){
                        $.ajax({
                            url: "<?php echo base_url(); ?>index.php/suprimentos/getAnexoCotacao",
                            type: 'POST',
                            dataType: 'json',
                            async:false,
                            data: {
                            ...
                            success: function(data2) {
                                $("#idAnexoRemover").val(idAnexo);
                                $("#bRemoverAnexo").empty();
                                $("#bRemoverAnexo").append(data2.obj.nomeArquivo);
                                $("#modalremoveranexo").modal("show");
                            },
