...
<div  class="control-group" style="margin-top: 5">
    ...
    <label for="dataReprog" class="control-label"><b>Filtrar por data de Reprogramação/Motivos:</b></label><br>
    De: <input id="dataReprogInicial" class="data" type="text" name="dataReprogInicial" value="<?php echo $this->input->get('dataReprogInicial'); ?>" /> 
    Até: <input id="dataReprogFinal" class="data" type="text" name="dataReprogFinal" value="<?php echo $this->input->get('dataReprogFinal'); ?>" />
</div>		
...
<div class="buttons">
    ...
    <?php 
        // Captura todos os filtros aplicados na URL atual
        $filtros_atuais = $this->input->get() ?: []; 
        // Gera a string de consulta (query string) para anexar ao link
        $query_string = http_build_query($filtros_atuais);
    ?>
    
    <a href="<?php echo base_url(); ?>index.php/os/exportarMotivosReagendamento?<?php echo $query_string; ?>" class="btn btn-mini btn-warning" title="Exportar Motivos de Reprogramação">
        <i class="icon-list icon-white"></i> Exportar Motivos
    </a>
</div>
...
<div class="widget-content nopadding" id="printOs2">
    ...
    <table style="border-collapse: collapse;font-family:Arial, Helvetica, sans-serif; font-size:11.5px; line-height:normal;" border="1" width='100%' id="fixed_table">
        ...
        <tr>
			...
			<td align='center'><b>Motivo Reprog.</b></td>
			<td align='center'><b>Data Canhoto</b></td>
			<td align='center'><b>Mat. Prod?</b></td>
            <td align='center' class="no-print"><b>Ver OS</b></td>
        </tr>
        ...
        foreach ($results as $r) {
            ...
            // Exibindo a Data de Criação (Data Insert)
            ...
            echo "<td align='center'>" . (!empty($r->data_reagendada) && $r->data_reagendada != '0000-00-00' ? date('d/m/Y', strtotime($r->data_reagendada)) : '-') . "</td>";
			...
			$motivos_lista = !empty($r->todos_motivos) ? $r->todos_motivos : "---";
			echo "<td align='left' style='font-size: 10px; color: #444;'>{$motivos_lista}</td>";			
			// --- Lógica Data Canhoto ---
			$data_canhoto = !empty($r->data_canhoto) ? date("d/m/Y", strtotime($r->data_canhoto)) : "";
			echo "<td align='center'>{$data_canhoto}</td>";
			
// --- Lógica para verificar se a OS possui material de produção ---
$ci = &get_instance();
$checkProd = $ci->db->get_where('distribuir_os', array('idOs' => $r->idOs, 'material_producao' => 1))->num_rows();

echo "<td align='center'>";
echo ($checkProd > 0) ? '<b style="color: red;">SIM</b>' : 'NÃO';
echo "</td>";			
			// ---------------------------
...
<table style="border-collapse: collapse; font-family:Arial, Helvetica, sans-serif; font-size:11.5px; line-height:normal;" border="1" width='100%' id="fixed_table">
    ...
    foreach ($results as $r) {
        ...
        // --- CORREÇÃO: Definição da data de produção que faltava ---
        if (!empty($r->data_producao)) {
            $data_producao_formatada = date("d/m/Y", strtotime($r->data_producao));
        } else {
            $data_producao_formatada = "";
