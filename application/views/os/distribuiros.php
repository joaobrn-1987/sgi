...
<form action="<?php echo base_url() ?>index.php/os/planejadoPCP" id="planejadoPCP" method="post">
    ...
    <span>&nbsp;&nbsp;&nbsp;&nbsp;<strong> PLANEJAR OS:</strong></span>
    ...
    <input style="width:60px" 
           ...
           value="1" 
           class="check span12"
           title="Marque esta opção se OS foi planejada"
           id="planejadoCheckbox"
           <?php if ($result->planejadoPCP == 1) { echo 'checked disabled'; } ?> />

    <input type="hidden" name="idOs" value="<?php echo $result->idOs ?>" />

    <!-- Botão de Planejar -->
    <?php if ($result->planejadoPCP == 1) {?>
        <strong>O.S. já planejada</strong>
    <?php } else { ?>
        <button type="submit" class="btn btn-success" id="planejarButton">
            <i class=""></i> PLANEJAR
        </button>
    <?php } ?>
</form>
...
<script>
    document.getElementById('planejadoPCP').addEventListener('submit', function(event) {
        var checkbox = document.getElementById('planejadoCheckbox');
        if (!checkbox.checked) {
            alert('Você deve marcar o checkbox antes de planejar.');
																			...
																			echo '<div style="margin: 20px;margin-top: 0px;">';
																				echo '<table class="table table-bordered ">';
																					...
																					echo '<tr>';
																						...
																						// --- FIM DA ALTERAÇÃO ---	
                                                                                        echo '<th>Prod?</th> <th>Ações</th>';
																						echo '<th>Revisão PCP</th>';
																						echo '<th>Data Ped. Revisão</th>';		
																						echo '<th></th>';
																						echo '<th></th>';
																					echo '</tr>';
																					...
																					echo '<tbody>';
																						foreach($a->distribuiros as $d){
																							echo '<tr>';
                                                                                                ...
                                                                                                // --- INÍCIO DA ALTERAÇÃO ---
echo '<td>';
echo '<select class="tipo-compra-select span12" data-id="' . $d->idDistribuir . '">';

$necessario_selected = ($d->tipo_compra == 'Necessário' ? 'selected' : '');
$reposicao_selected = ($d->tipo_compra == 'Reposição' ? 'selected' : '');
$placeholder_selected = ($necessario_selected == '' && $reposicao_selected == '') ? 'selected' : '';

echo '<option value="" ' . $placeholder_selected . '>Escolha o tipo</option>';
echo '<option value="Necessário" ' . $necessario_selected . '>Necessário</option>';
echo '<option value="Reposição" ' . $reposicao_selected . '>Reposição</option>';

echo '</select>';
echo '</td>';
                                                                                                // --- FIM DA ALTERAÇÃO ---
// --- INCLUSÃO: Exibição do campo Material para Produção ---
echo '<td style="text-align: center; vertical-align: middle;">';
if (isset($d->material_producao) && $d->material_producao == 1) {
    echo '<span class="label label-important" style="font-weight: bold;" title="Material destinado à Produção">SIM</span>';
} else {
    echo '<span class="label" style="background-color: #bbbbbb;" title="Material Comum / Não Produção">NÃO</span>';
}
echo '</td>';  
		...
		<div class="modal-body">
			...
			<div class="span12" style="margin-left:0px">
                ...
                <div class="span2">
                    ...
                    <select name="tipo_compra" id="tipo_compra_modal" class="span12">
...
<div class="control-group">
    <label class="control-label" style="font-weight: bold; color: #2d335b;">Material p/ Produção?</label>
    <div class="controls">
        <select name="material_producao" class="span12" style="border: 1px solid #ccc;">
            <option value="0">NÃO</option>
            <option value="1">SIM</option>
        </select>
        <span class="help-block"><small>* Defina se este item é essencial para a linha de produção.</small></span>
    </div>
</div>			
...
<script type="text/javascript">
	...
	function verificarDadosMateriais(){
		...
		var tipoCompraModal = document.querySelector('#tipo_compra_modal');
		...
		if(!tipoCompraModal || tipoCompraModal=="0"){
			$(tipoCompraModal).css('border-color','red');
			verificar = false;
		}		
