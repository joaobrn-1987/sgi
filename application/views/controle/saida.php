										...
										<tbody>
                                            <?php
                                                ...
                                                foreach($objControleSubItem as $d){
                                                    echo '<tr class="trpai'.$d->idControleEtapa.'">';
                                                            
                                                        if($d->tipoOrc == "serv"){
                                                            echo '<td onclick="openclose(this,'.$d->idControleEtapa.')"  style="text-align: center; /* padding: 75px 5px 75px 5px */ display: table-cell;min-height: 10em;vertical-align: middle;"><a class="detail-icon"><i class="fa fa-plus"></i></a></td>';
                                                        }else{
                                                            echo '<td></td>';
                                                        }
                                                        ...
                                                        echo '<td ><div id="origem" class="span12" style="padding: 0.2%; margin-left: 0">';
                                                            ...
                                                            echo '</div>'.
                                                                ...
                                                                '</div>' ;
                                                            echo '<div class="span1">';
                                                                echo '<a onclick="receberMercadoria('.$contador_local_autocomplete.')" role="button" data-toggle="modal" class="btn btn-warning"  class="span12" style="/* margin-right: 10px;*/padding: 14px;">Saída</a>';
                                                            echo '</div>';
                                                            echo '</div>';
                                                            
                                                        echo '</div></td>';
                                                    echo '</tr>';
                                                    echo '<tr class="trfilho'.$d->idControleEtapa.'" style="display:none">';
                                                        echo '<td>';
                                                        echo '</td>';
                                                        echo '<td>';
                                                                        ...
                                                                        echo ' <tbody>';
                                                                            echo '<tr>';
                                                                                ...
                                                                                if($c->idStatusEtapaServico == 3 ){
                                                                                    ...
                                                                                    echo '<td style="width:70px">'.$c->quantidade.'</td>';
                                                                                }
                                                                            ...
                                                                            foreach($c->controleEtapaSubitem as $d){
                                                                                echo '<tr>';
                                                                                    ...
                                                                                    if($d->idStatusEtapaServico==3){
                                                                                        ...
                                                                                        echo '<td style="width:70px">'.$d->quantidade.'</td>';
                                                                                    }
...
<div id="modal-observacao" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url(); ?>index.php/controle/confirmarsaida"  enctype="multipart/form-data" id="formConfirmarSaida" method="post" class="form-horizontal">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">

        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            <h5 id="myModalLabel">Saída</h5>
        </div>
        <div class="modal-body">
            <h5 style="text-align: center">Confirmar a saída dos itens</h5><!--
            ...
            <div class="row-fluid" style="margin-top:0">
                <div class="span12">
                    <div class="span6">
                        <label>Etapa</label>
                        <select class="span12 form-control" id="statusServico" name="statusServico">
            ...
            <a onclick="limparAssinatura()" style="margin-top:15px" class="btn">Limpar</a>
        </div>
        
        <div class="modal-footer">
            <button class="btn" data-dismiss="modal" aria-hidden="true" id="btnCancelExcluir">Cancelar</button>
            <a class="btn btn-success" onclick="confirmarSaida()">Confirmar</a>
        </div>
    </form>
</div>
...
<script type="text/javascript">
    ...
    function receberMercadoria(valor){
        ...
        var local = document.querySelectorAll("#local_"+valor);
        var linha = document.querySelectorAll("#linha_"+valor);
        var coluna = document.querySelectorAll("#coluna_"+valor);
        ...
        var localSub = document.querySelectorAll("#localSub_"+valor);
        var linhaSub = document.querySelectorAll("#linhaSub_"+valor);
        var colunaSub = document.querySelectorAll("#colunaSub_"+valor);
        ...
        for(x=0;x<controleEtapa.length;x++){
            if(controleEtapa[x].checked){
                arrayControleEtapa.push({"id":controleEtapa[x].value,"local":local[x].value,"linha":linha[x].value,"coluna":coluna[x].value});
            }
        }
        for(x=0;x<controleEtapaSubitem.length;x++){
            if(controleEtapaSubitem[x].checked){
                ...
                arrayControleEtapaSubitem.push({"id":controleEtapaSubitem[x].value,"quantidade":quantidadeSubitem[x].value,"local":localSub[x].value,"linha":linhaSub[x].value,"coluna":colunaSub[x].value});
            }
        }
        if(arrayControleEtapaSubitem.length <= 0 && arrayControleEtapa.length <= 0){
            alert("Nenhum item selecionado.");
            return;
        }
        $.ajax({
            url: "<?php echo base_url(); ?>index.php/controle/entradamercadoria",
            type: 'POST',
            dataType: 'json',
            async:false,
            data: {
    ...
    function carregarmodalconfirmacao(itens){
                ...
                for(x=0;x<itens.arrayControleEtapa.length;x++){
                    html += '<tr>';
                        ...
                        html += '<td>';
                            ...
                            html += '<input type="hidden" name="localModal[]" value="'+(itens.arrayControleEtapa[x].local?itens.arrayControleEtapa[x].local:"")+'">';
                        html += '</td>';
                        html += '<td>';
                            if(itens.arrayControleEtapa[x].linha)
                                html += itens.arrayControleEtapa[x].linha;
                            html += '<input type="hidden" name="linhaModal[]" value="'+(itens.arrayControleEtapa[x].linha?itens.arrayControleEtapa[x].linha:"")+'">';
                        html += '</td>';
                        html += '<td>';
                            if(itens.arrayControleEtapa[x].coluna)
                                html += itens.arrayControleEtapa[x].coluna;
                            html += '<input type="hidden" name="colunaModal[]" value="'+(itens.arrayControleEtapa[x].coluna?itens.arrayControleEtapa[x].coluna:"")+'">';
                        html += '</td>';
                    html += '</tr>';
                }
                html += '</tbody>'+
        ...
        if(itens.arrayControleEtapaSubitem.length>0){
            html += '<table id="table_id" class="table table-bordered " id="dadosTlbOsOc">'+
                '<thead>'+
                    '<tr>'+
                        '<th></th>'+
                ...
                for(x=0;x<itens.arrayControleEtapaSubitem.length;x++){
                    html += '<tr>';
                        ...
                        html += '<td>';
                            ...
                            html += '<input type="hidden" name="localModalSubitem[]" value="'+(itens.arrayControleEtapaSubitem[x].local?itens.arrayControleEtapaSubitem[x].local:"")+'">';
                        html += '</td>';
                        html += '<td>';
                            if(itens.arrayControleEtapaSubitem[x].linha)
                                html += itens.arrayControleEtapaSubitem[x].linha
                            html += '<input type="hidden" name="linhaModalSubitem[]" value="'+(itens.arrayControleEtapaSubitem[x].linha?itens.arrayControleEtapaSubitem[x].linha:"")+'">';
                        html += '</td>';
                        html += '<td>';
                            if(itens.arrayControleEtapaSubitem[x].coluna)
                                html += itens.arrayControleEtapaSubitem[x].coluna
                            html += '<input type="hidden" name="colunaModalSubitem[]" value="'+(itens.arrayControleEtapaSubitem[x].coluna?itens.arrayControleEtapaSubitem[x].coluna:"")+'">';
                        html += '</td>';
                    html += '</tr>';
                }
                html += '</tbody>'+
    ...
    function confirmarSaida(){
        var etapa = document.querySelector("#statusServico").value;
        var idUser = document.querySelector("#idUser").value;
        var usuario = document.querySelector("#usuario").value;
        if(!idUser || !usuario){
            alert("Informe o usuário que está retirando o item");
            return;
        }
        if(!etapa){
            alert("informe a etapa para qual o item está sendo retirado.");
            return;
        }
