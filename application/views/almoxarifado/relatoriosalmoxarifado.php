...
<div id="estoqueAtual" style="display:block">
  ...
  <div class="widget-box">
	...
	<div class="buttons">                    
      ...
      <a href="javascript:void(0)" class="btn btn-mini btn-success" onclick="exportarAtualParaExcelBackend()"><i class="icon-list-alt icon-white"></i> Excel Total</a>
    </div>
      ...
      <table class="table table-bordered " id="table_estoque" style="border-collapse: collapse;font-family:Arial, Helvetica, sans-serif;
        ...
        <thead>
          <tr>
			...
			<th>Consultas com estoque = 0:</th>
			<th>Consultas com estoque >= 1:</th>	 		
            <th>O.S.</th>
            <th>Empresa</th>
            <th>Departamento</th>
            <th>Local</th>       
...
<?php 
$estat = isset($estatisticas[$r->idProduto]) ? $estatisticas[$r->idProduto] : null;
?>

<td>
  <?php echo $estat && $estat->total_consultas > 0 ? $estat->total_consultas : ''; ?>
</td>
<td>
  <?php echo $estat && $estat->estoque_zero > 0 ? $estat->estoque_zero : ''; ?>
</td>
<td>
  <?php echo $estat && $estat->estoque_maior_igual_1 > 0 ? $estat->estoque_maior_igual_1 : ''; ?>
</td>	
...
<div id="estoqueGeral" style="display:none">
  ...
  <div class="widget-box">
	...
	<div class="buttons">
      ...
      <a href="javascript:void(0)" class="btn btn-mini btn-success" onclick="exportarGeralParaExcelBackend()"><i class="icon-list-alt icon-white"></i> Excel Total</a>
    </div>
                  ...
                  <div>
                    <div class="span12" style="margin-left: 0">
                      ...
                      <div class="span2" class="control-group" >
                        <label for="idEmpresa" class="control-label">Unid. Exec.:</label>
                        <?php foreach ($unid_exec as $exec) { ?>
                            <input type="checkbox" name="unid_execucaoeEntr" id="unid_execucaoeEntr" class='check' value="<?php echo $exec->id_unid_exec; ?>"> &nbsp;<?php echo $exec->status_execucao; ?>
    ...
    function filterEstoqueAtual(){
       ...
       success: function(data) {
            if (data.result == true) {
                ...
                // Agora, solicite as estatísticas
                $.ajax({
                    url: "<?php echo base_url(); ?>index.php/almoxarifado/getEstatisticasEstoqueFiltrado",
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        prode: prode,
                    ...
                    success: function(stats) {
                        atualizarContadores(stats); // Crie uma função para exibir os contadores na view
                    },
    ...
    function preencherTabelaEstoqueAtual(resultado){
      ...
      for(y=0;y<resultado.length;y++){
        ...
        if(resultado[y].metrica == 0){
            ...
            newCell.innerHTML = resultado[y].quantidade+" | "+resultado[y].comprimento+" MM";
...
newCell.innerHTML = (resultado[x].total_consultas && resultado[x].total_consultas != 0 ? resultado[x].total_consultas : "");

newCell = newRow.insertCell(5);
newCell.innerHTML = (resultado[x].estoque_zero && resultado[x].estoque_zero != 0 ? resultado[x].estoque_zero : "");

newCell = newRow.insertCell(6);
newCell.innerHTML = (resultado[x].estoque_maior_igual_1 && resultado[x].estoque_maior_igual_1 != 0 ? resultado[x].estoque_maior_igual_1 : "");
    ...
    function filtrarEntradas(){
        ...
        var unid_execu = Array.apply(null,document.querySelectorAll("#unid_execucaoeEntr"));
        var unidades = "";
        for(var x=0;x<unid_execu.length;x++){
          if(unid_execu[x].checked){
            if(unidades != ""){
    ...
    function preencherTabelaSaida(resultado){
        ...
        for(y=0;y<resultado.length;y++){
            ...
            if(resultado[y].assinatura)
              newCell.innerHTML = '<a href="<?php  echo base_url()?>'+resultado[y].assinatura+'" target="_blank" style="margin-right: 1%" class="btn tip-top" ><i class="icon-eye-open"></i></a>';
...
<?php if(isset($dadosRelatorio)){
    ...
    function build_sub_table(subData, table) {
        var data = JSON.parse(JSON.stringify(window[subData]))
    ...
    function detailFormatter(index, row, element){ 
        var html = []
        var data = JSON.parse(JSON.stringify(window["sub_data"+row.id]))
        var data2 = JSON.parse(JSON.stringify(window["sub_entrada"+row.id]))
        var data3 = JSON.parse(JSON.stringify(window["sub_saida"+row.id]))
        var htmlA = "";
        var htmlB = "";
        var htmlC = "";
...
function exportarAtualParaExcelBackend() {
        var empresa = document.getElementById('idEmpresae').value;
        var departamento = document.getElementById('idDepartamentoe').value;
        var prode = document.getElementById('prode').value;
        var pn = document.getElementById('pne').value;
        var idOse = document.getElementById('idOse').value;
        var local = document.getElementById('localatual').value;

        // Monta o link para a nova função do PHP
        var url = "<?php echo base_url(); ?>index.php/almoxarifado/exportarEstoqueAtualExcel" + 
            "?empresa=" + empresa +
            "&departamento=" + departamento +
            "&prode=" + prode +
            "&pn=" + pn +
        ...
        // Abre o link para forçar o download do Excel
        window.open(url, '_blank');
    }

    function exportarGeralParaExcelBackend() {
        var empresa = document.getElementById('idEmpresag').value;
        var departamento = document.getElementById('idDepartamentog').value;
        var descricao = document.getElementById('prodg').value;
        var png = document.getElementById('png').value;

        // Monta o link para a nova função do PHP
        var url = "<?php echo base_url(); ?>index.php/almoxarifado/exportarEstoqueGeralExcel" + 
            "?empresa=" + empresa +
            "&departamento=" + departamento +
            "&descricao=" + descricao +
            "&pn=" + png;
