...
<div>
  <a href="#modal-usuario" role="button" data-toggle="modal" class="btn btn-success"><i class="icon-plus icon-white"></i> Cadastrar Pessoa</a>
</div>
<?php
        }?>


<div id="insumos" style="display:<?php if($dados_departamento[0]->tipo == "INSUMO"){echo "block";}else{echo "none";}?>">
                    ...
                    <div class="span12">
                        <div class="widget-box">
                            <div class="widget-title">
                                ...
                                <input class="span12" class="form-control" id="departamento2" type="text" name="departamento2" value="<?php if($dados_departamento[0]->idAlmoEstoqueDep != 1){echo $dados_departamento[0]->descricaoDepartamento;}?>" disabled style="border:0px;width:50%;margin:3px" />
                            </div>
                                                    ...
                                                    <div class="span12" style="padding: 1%; margin-left: 0">
                                                        ...
                                                        <div class="tamanho">
                                                            <div class="span1" class="control-group"
                                                                ...
                                                                <label for="cliente"
                                                                    class="control-label">Comprimento(mm):</label>
                    ...
                    <div class="span12">
                        <div class="widget-box">
                            <div class="widget-title">
                                ...
                                <h5 style="padding-right: 0px;">Saída de Produtos</h5><input class="span12" class="form-control" id="departamentoS2"  type="text" name="departamentoS2" value="<?php if($dados_departamento[0]->idAlmoEstoqueDep != 1){echo $dados_departamento[0]->descricaoDepartamento;}?>" disabled style="border:0px;width:50%;margin:3px" />
                            </div>
                            <div class="widget-content nopadding">
                                <div class="span12" id="divProdutosServicos" style=" margin-left: 0">
                                    <div class="tab-content">
    ...
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                            ...
                            <input type="hidden" value="" name="assinatura" id="assinatura">
                ...
                <div align="center">
                  ...
                  <a type="" onclick="limparAssinatura()" style="margin-top:15px" class="btn btn-warning">Limpar</a>
                </div>
            </div>
        </div>
    </div>
...
<div id="produtos" style="display:<?php if($dados_departamento[0]->tipo == "PRODUTO"){echo "block";}else{echo "none";}?>;">
    <div class="tabbable" style="margin-top: 30px;">
        <!-- Only required for left/right tabs -->
        <ul class="nav nav-tabs">
    ...
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
                            ...
                            <input type="hidden" value="" name="assinatura2" id="assinatura2">
                        </form>
                    </div>
                </div>  
                <div align="center">
                  <div class="wrapper">
                    <div style="border-bottom: 2px solid black;height: 125px;margin-bottom: 10px;"></div>
                    <canvas id="signature-pad2" name="signature-pad2" class="signature-pad" width=800 height=200></canvas>
                    <div style="text-align:center">
                    <h4>Assinatura</h4> 
                    </div> 
                  </div>
                  ...
                  <a type="" onclick="limparAssinatura2()" style="margin-top:15px" class="btn btn-warning">Limpar</a>
                </div>              
            </div>
        </div>
    </div>
...
<div id="modal-usuario" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
    aria-hidden="true">
  <div class="row-fluid" style="margin-top:0">
    <div class="span12">
      <div class="widget-box">
...
<script type="text/javascript">
    ...
    function inserirLinhaTabelaEntradaProduto(){
      ...
      if(typeof idDepartamento != "UNDEFINED" && idDepartamento != null && idDepartamento != "" && typeof pn != "UNDEFINED" && pn != null && pn != "" && typeof prod != "UNDEFINED" && prod != null && prod != "" && typeof idMedicao != "UNDEFINED" && idMedicao != null && idMedicao != ""  && typeof qtd != "UNDEFINED" && qtd != null && qtd != "" )
      {
        if(isNaN(qtd)== true){
          a.disabled = false;
          return alert("O campo quantidade deve possuir apenas números.");
        }
        if(isNaN(valorUnit.replace(',','.'))== true){
          a.disabled = false;
          return alert("O campo valor unitário deve possuir apenas números.");
        }
        if(idOs != null){
          if(isNaN(idOs)== true){
            a.disabled = false;
            return alert("O campo OS deve possuir apenas números.");
          }
        }
        if(nf != null){
          if(isNaN(nf)== true){
            a.disabled = false;
            return alert("O campo NF deve possuir apenas números.");
          }
        }
        arrayTabelaEntrada = {
          "pn":pn,
	...
	function inserirLinhaTabelaSaidaProduto(){
	  ...
	  // Validação mantida: Descrição/Item, Quantidade e Usuário são obrigatórios.
	  if( (idEstoque != "" || prod != "") && qtd != "" && idUser != "")
	  {
		if(isNaN(qtd)== true){
		  a.disabled = false;
		  return alert("O campo quantidade deve possuir apenas numeros.");
		...
		// <-- CORREÇÃO: Bloco de validação que tornava a OS obrigatória foi REMOVIDO. -->
		// O campo OS agora é opcional.
		...
		a.disabled = false;
		alert("Para dar saída, preencha no mínimo a Descrição do Produto, a Quantidade e o Usuário.");
	  }
	}
      ...
      $.ajax({
        ...
        success: function(data2) {
          if(data2.result){
            //var arrayList = JSON.parse(getCookie('tabelaEntradaProduto'));
            arrayList = { };
            window.open('<?php echo base_url()?>index.php/almoxarifado/imprimircodbarras?my_data='+JSON.stringify(data2.entrada));
    ...
    function cadastrarSaidaEstoqueProduto(){
      ...
      var obj = {};
      var data = new Array();
      verificarAssinatura2()/*
      if(!verificarAssinatura2()){
        a.disabled = false;
        alert("Assinatura não preenchida");
      ...
      $.ajax({
        ...
        data: {
          arraySaidas: data,
          assinatura2: document.querySelector("#assinatura2").value
        },
        success: function(data2) {
          if(data2.result){
            var arrayList = [];
    ...
    function alterarEstoque(value){
      ...
      $("#localS").autocomplete({
        source: "<?php echo base_url(); ?>index.php/almoxarifado/autoCompleteEstoqueSaidaLocal/"+value+'/'+idDepartamento,
        minLength: 1,
        select: function( event, ui ) {
          $('#idAlmoEstoqueS').val(ui.item.id);
          $('#localS').val(ui.item.local);
    ...
    function alterarEstoque1(value){
      ...
      $("#localS").autocomplete({
        source: "<?php echo base_url(); ?>index.php/almoxarifado/autoCompleteEstoqueSaidaLocal/"+idEmpresa+'/'+value,
        minLength: 1,
        select: function( event, ui ) {
          $('#idAlmoEstoqueS').val(ui.item.id);
          $('#localS').val(ui.item.local);
      ...
      for(var x=0;x<numOfRows;x++){
        ...
        if(typeof colum[0] != "undefined"){
          if(typeof colum[0].innerHTML != "undefined"){
            ...
            if(colum[0].value==id){
              somQtd = parseFloat(somQtd) + parseFloat(colum[2].value);
            }
          }
        }
      }
          ...
          if(data.result == true){
            if(data.resultado.length > 0){
              ...
              for(x=0;x<data.resultado.length;x++){
                ...
                newCell = newRow.insertCell(3);   
                newCell.innerHTML = (data.resultado[x].local!=null?data.resultado[x].local:"")+"<input value='"+data.resultado[x].local+"' name='descricaoLocalP' id='descricaoLocalP' type='hidden'/>";
                newCell = newRow.insertCell(4);   
                ...
                newCell = newRow.insertCell(5);  
                if(data.resultado[x].vale != null && data.resultado[x].vale != ""){
                  html = "";
                  primeiro = true;
                  for(y=0;y<data.resultado[x].vale.length;y++){
                    if(primeiro){
                      html += data.resultado[x].vale[y].idOsOrig+" >> "+data.resultado[x].vale[y].idOsDest+";"
                      primeiro = false;
                    }else{
                      html += "</br>"+data.resultado[x].vale[y].idOsOrig+" >> "+data.resultado[x].vale[y].idOsDest+";"
                    }
    ...
    function adicionarItensTabelaSaida(){
      ...
      var descEmpresa = empresaDoc.options[empresaDoc.selectedIndex].label;
      itemSel = false;
      ...
      for(x=0;x<idAlmoEstoque.length;x++){
        if(idAlmoEstoque[x].checked){
          ...
          newCell = newRow.insertCell(3);   
          newCell.innerHTML = (descricaoLocalP[x].value!=null&&descricaoLocalP[x].value!="null"?descricaoLocalP[x].value:"");
          newCell = newRow.insertCell(4);   
    ...
    $(document).ready(function(){
      ...
      $("#localS").autocomplete({
        source: "<?php echo base_url(); ?>index.php/almoxarifado/autoCompleteEstoqueSaidaLocal/"+getidEmpresaS2()+"/"+getidDepartamentoS2(),
        minLength: 1,
        select: function( event, ui ) {
          $('#idAlmoEstoqueS').val(ui.item.id);
          $('#pnS4').val(ui.item.pn);
